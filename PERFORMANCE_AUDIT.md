...
| 60 | **MEM-C3: PrefixMatch clone KV cache on hit — deep clone MB per call** — changed `KVCacheEntry.k/v: Vec<f32>` → `Arc<Vec<f32>>`; cloning KVCacheEntry is now O(1) instead of O(KV_size). All construction/read sites updated accordingly | `gqa.rs:643-645` | ✅ Done | Memory -80% per prefix hit |
| 61 | **CPU-H2: MoE Router CPU path — O(batch×experts×hidden) sequential dot products** — replaced `for i { for j { dot } }` with single `input.dot(&w.t().to_owned())` + row-wise softmax in-place. Uses ndarray's optimized matmul. Removed dead `apply_softmax_row`. | `routing.rs:197-219` | ✅ Done | CPU routing -5x latency |
| 62 | **CPU-H1: MoE Expert CPU forward — token-level sequential, no batching** — `forward_batched` replaced token loop (`for i { forward(token_slice) }`) with batched ndarray ops: `input @ fc1.T + b1 → gelu → @ fc2.T + b2`. Eliminates per-token fc1/fc2 dispatch overhead. | `experts.rs:343-363` | ✅ Done | CPU expert -10x throughput |
| 63 | **MEM-H4: PagedKVCache → to_flat_cache alloc — Array2 zeros + intermediate copy** — replaced `Array2::zeros` + `from_shape_vec` + `slice_mut().assign()` with direct `vec![0f32; total]` + `copy_from_slice`. Eliminates `Array2` wrapper and `s!` overhead for each block slice. | `paged_cache.rs:730-757` | ✅ Done | Memory temp -30% |
| 64 | **MEM-H5: share_prefix_in_blocks Vec\<Vec\> clone of entire block table** — removed `src_rows: Vec<Vec<Option<usize>>>` clone of src block table. Processed layer-by-layer with lightweight `layer_data: Vec<(usize, Vec<(usize, usize)>)>` containing only non-None entries. Removed separate `refcount_increments` vec (refcount ++ inline). | `paged_cache.rs:668-710` | ✅ Done | Memory -80% per prefix share |
