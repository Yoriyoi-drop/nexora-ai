...
impl CodeParser {
    ...
    fn parse_python(&self, code: &str) -> Result<CodeAst> {
        ...
        // Extract functions
        let function_regex = Regex::new(r"def\s+(\w+)\s*\(([^)]*)\)\s*:").unwrap();
    ...
    fn parse_javascript(&self, code: &str) -> Result<CodeAst> {
        ...
        // Extract classes (ES6)
        let class_regex = Regex::new(r"class\s+(\w+)\s*(\s+extends\s+(\w+))?\s*\{").unwrap();
        for cap in class_regex.captures_iter(code) {
            let name = cap.get(1).unwrap().as_str().to_string();
    ...
    fn parse_java(&self, code: &str) -> Result<CodeAst> {
        ...
        // Extract methods
        let method_regex = Regex::new(r"(public|private|protected)?\s*(static)?\s*(\w+)\s+(\w+)\s*\(([^)]*)\)\s*\{").unwrap();
        for cap in method_regex.captures_iter(code) {
            ...
            let params = cap.get(6).unwrap().as_str().to_string();
            ast.add_function(CodeFunction {
        ...
        // Extract classes
        let class_regex = Regex::new(r"(public\s+)?class\s+(\w+)\s*(\s+extends\s+(\w+))?\s*\{").unwrap();
        for cap in class_regex.captures_iter(code) {
    ...
    fn parse_cpp(&self, code: &str) -> Result<CodeAst> {
        ...
        // Extract functions
        let function_regex = Regex::new(r"(\w+)\s+(\w+)\s*\(([^)]*)\)\s*\{").unwrap();
        for cap in function_regex.captures_iter(code) {
            let return_type = cap.get(1).unwrap().as_str().to_string();
            let name = cap.get(2).unwrap().as_str().to_string();
            let params = cap.get(3).unwrap().as_str().to_string();
            ast.add_function(CodeFunction {
                name: format!("{}: {}", name, return_type),
                parameters: params,
                line_start: 0,
        ...
        // Extract classes
        let class_regex = Regex::new(r"class\s+(\w+)\s*(\s*:\s*(public|private|protected)\s+(\w+))?\s*\{").unwrap();
        for cap in class_regex.captures_iter(code) {
            let name = cap.get(1).unwrap().as_str().to_string();
            let inheritance = cap.get(4).map(|m| m.as_str().to_string()).unwrap_or_default();
            ast.add_class(CodeClass {
    ...
    fn parse_generic(&self, code: &str) -> Result<CodeAst> {
        ...
        let function_regex = Regex::new(r"(\w+)\s*\(").unwrap();
        for cap in function_regex.captures_iter(code) {
            let name = cap.get(1).unwrap().as_str().to_string();
            ast.add_function(CodeFunction {
                name,
...
impl CodeMetrics {
    ...
    fn count_functions(&self, code: &str) -> usize {
        ...
        match self.language.as_str() {
            "python" => {
                let regex = Regex::new(r"def\s+\w+\s*\(").unwrap();
                count = regex.find_iter(code).count();
            },
            "javascript" | "js" => {
                let regex = Regex::new(r"function\s+\w+\s*\(").unwrap();
                count = regex.find_iter(code).count();
            },
            "java" => {
    ...
    fn count_classes(&self, code: &str) -> usize {
        ...
        match self.language.as_str() {
            "python" => {
                let regex = Regex::new(r"class\s+\w+").unwrap();
                count = regex.find_iter(code).count();
            },
        ...
        match language {
            "python" => {
                ...
                for cap in regex.captures_iter(code) {
                    imports.push(cap.get(1).unwrap().as_str().to_string());
                }
            },
            "javascript" => {
