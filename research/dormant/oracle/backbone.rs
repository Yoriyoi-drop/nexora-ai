...
impl SparseMoELayer {
    ...
    pub fn forward(&self, x: &Array2<f32>) -> Result<Array2<f32>> {
        ...
        let x_reshaped = x.view((batch_size * seq_len, d_model))?;
        ...
        let mut output = Array2::zeros((batch_size * seq_len, d_model));
        
        for (i, &expert_idx) in expert_indices.iter().enumerate() {
            let expert_input = x_reshaped.slice(s![i, ..]);
            let expert_output = self.experts[expert_idx].forward(&expert_input.into_dimensionality()?)?;
            
            let mut output_row = output.slice_mut(s![i, ..]);
            output_row.assign(&expert_output);
        }
        
        // Reshape back
        Ok(output.view((batch_size, seq_len, d_model))?.to_owned())
    }
...
impl MLPExpert {
    pub fn new(input_dim: usize, hidden_dim: usize) -> Self {
        Self {
            w1: Array2::random((input_dim, hidden_dim), rand::distributions::Standard),
            w2: Array2::random((hidden_dim, input_dim), rand::distributions::Standard),
            b1: Array1::random(hidden_dim, rand::distributions::Standard),
            b2: Array1::random(input_dim, rand::distributions::Standard),
        }
    }
...
impl MultiHeadLatentAttention {
    ...
    pub fn forward(&self, x: &Array3<f32>, mask: Option<&Array2<f32>>) -> Result<Array3<f32>> {
        ...
        let latent_3d = latent.view((batch_size, seq_len, self.config.latent_dim))?;
        
        // Compress latent representation
        let compressed = self.latent_compression.compress(&latent_3d)?;
        ...
        let output_reshaped = concatenated.view((batch_size * seq_len, self.config.latent_dim))?;
...
impl LatentAttentionHead {
    ...
    pub fn forward(&self, x: &Array3<f32>, mask: Option<&Array2<f32>>) -> Result<Array3<f32>> {
        ...
        // Project to Q, K, V
        let x_reshaped = x.view((batch_size * seq_len, self.latent_dim))?;
        
        let q = self.q_proj.forward(&x_reshaped)?;
        let k = self.k_proj.forward(&x_reshaped)?;
        let v = self.v_proj.forward(&x_reshaped)?;
        
        let q_3d = q.view((batch_size, seq_len, self.head_dim))?;
        let k_3d = k.view((batch_size, seq_len, self.head_dim))?;
        let v_3d = v.view((batch_size, seq_len, self.head_dim))?;
        
        // Scaled dot-product attention
        let attention_output = self.scaled_dot_product_attention(&q_3d, &k_3d, &v_3d, mask)?;
        
        // Project output
        let output_reshaped = attention_output.view((batch_size * seq_len, self.head_dim))?;
        let output = self.out_proj.forward(&output_reshaped)?;
        
        Ok(output.view((batch_size, seq_len, self.latent_dim))?.to_owned())
    }
    ...
    fn scaled_dot_product_attention(
        ...
        let q_transposed = q.t().swap_axes(1, 2)?;
        let k_transposed = k.t().swap_axes(1, 2)?;
        let scores = q_transposed.dot(&k_transposed) / (head_dim as f32).sqrt();
        
        // Apply mask if provided
        let scores = if let Some(mask) = mask {
            let mask_expanded = mask.broadcast((batch_size, seq_len, seq_len))?;
            scores * &mask_expanded
        } else {
            scores
        };
        ...
        let v_transposed = v.t().swap_axes(1, 2)?;
        let output = attention_weights.dot(&v_transposed);
        
        Ok(output.t().swap_axes(1, 2)?)
    }
...
impl LatentCompression {
    ...
    pub fn compress(&self, x: &Array3<f32>) -> Result<Array3<f32>> {
...
impl LinearLayer {
    pub fn new(input_dim: usize, output_dim: usize) -> Self {
        Self {
            weight: Array2::random((input_dim, output_dim), rand::distributions::Standard),
            bias: Array1::random(output_dim, rand::distributions::Standard),
        }
    }
...
impl OracleBackbone {
    ...
    pub fn forward(&self, input_ids: &Array2<i32>, mask: Option<&Array2<f32>>) -> Result<Array3<f32>> {
        ...
        for i in 0..self.moe_layers.len() {
            ...
            // MoE layer
            let moe_output = self.moe_layers[i].forward(&hidden)?;
        ...
        // Output projection
        let hidden_reshaped = hidden.view((batch_size * seq_len, self.config.d_model))?;
        let logits = self.output_projection.forward(&hidden_reshaped)?;
        
        Ok(logits.view((batch_size, seq_len, self.output_projection.weight.dim().1))?.to_owned())
    }
    ...
    pub fn get_expert_usage_stats(&self, input_ids: &Array2<i32>) -> Result<Vec<Vec<f32>>> {
        ...
        for moe_layer in &self.moe_layers {
            let usage = moe_layer.get_expert_usage(&hidden)?;
...
impl EmbeddingLayer {
    pub fn new(vocab_size: usize, d_model: usize) -> Self {
        Self {
            embeddings: Array2::random((vocab_size, d_model), rand::distributions::Standard),
        }
    }
...
pub mod utils {
    ...
    pub fn calculate_flops(config: &OracleBackboneConfig, batch_size: usize, seq_len: usize) -> u64 {
        ...
        ((moe_flops + attention_flops) * n_layers * batch_size) as u64
    }
