...
impl OracleTrainer {
    ...
    pub fn new(config: OracleConfig, vocab_size: usize) -> Result<Self> {
        ...
        let reference_model = self::alignment::CodeModel::new(vocab_size, config.training.max_seq_len);
        let current_model = self::alignment::CodeModel::new(vocab_size, config.training.max_seq_len);
    ...
    fn run_pretraining(&mut self, training_data: &[TrainingExample]) -> Result<PretrainingResult> {
        ...
        for epoch in 0..self.config.training.pretraining_epochs {
            ...
            if epoch % self.config.training.eval_interval == 0 {
                let eval_metrics = self.evaluate_pretraining(training_data)?;
                result.epoch_metrics.push(eval_metrics);
                
                println!("  Eval Loss: {:.6}", eval_metrics.avg_loss);
                
                // Early stopping
                if self.check_early_stopping(eval_metrics.avg_loss) {
    ...
    fn run_alignment(&mut self, training_data: &[TrainingExample]) -> Result<AlignmentResult> {
        ...
        for epoch in 0..self.config.training.alignment_epochs {
            ...
            let alignment_stats = self.dpo_trainer.get_alignment_stats(batch);
    ...
    /// Final evaluation
    fn final_evaluation(&mut self, training_data: &[TrainingExample]) -> Result<EvaluationMetrics> {
        ...
        for batch in pretraining_batches.iter().take(10) { // Sample 10 batches
            let loss = self.pretrainer.dual_loss_calculator.compute_dual_loss(batch)?;
        ...
        for example in training_data.iter().take(100) { // Sample 100 examples
            ...
            let score = self.verifier.verify_code(&code)?;
            verification_scores.push(score);
        }
    ...
    /// Save checkpoint
    fn save_checkpoint(&self, checkpoint_name: String) -> Result<()> {
...
pub struct AlignmentResult {
    pub final_loss: f32,
    pub total_epochs: usize,
    pub alignment_stats: Vec<self::alignment::AlignmentStats>,
}
...
pub mod utils {
    ...
    pub fn estimate_training_time(
        ...
        let flops_per_step = estimate_flops_per_step(&config.backbone, vocab_size);
        let total_flops = total_steps * flops_per_step;
        
        // Time estimation (assuming 1 TFLOP/s)
        let estimated_seconds = total_flops / 1e12;
