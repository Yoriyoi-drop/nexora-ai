...
impl NexoraConfig {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> NexoraResult<Self> {
        ...
        if !path_ref.exists() {
            return Err(NexoraError::not_found(format!(
                "Configuration file not found: {:?}",
                path_ref
            )));
        }

        let content = std::fs::read_to_string(path_ref).map_err(|e| NexoraError::io(e))?;

        let config: NexoraConfig =
            toml::from_str(&content).map_err(|e| NexoraError::serialization(e))?;
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> NexoraResult<()> {
        ...
        if let Some(parent) = path_ref.parent() {
            std::fs::create_dir_all(parent).map_err(|e| NexoraError::io(e))?;
        }

        let content = toml::to_string_pretty(self).map_err(|e| NexoraError::serialization(e))?;

        std::fs::write(path_ref, content).map_err(|e| NexoraError::io(e))?;
    ...
    pub fn validate(&self) -> NexoraResult<()> {
        ...
        if self.core.max_concurrent_requests == 0 {
            ...
            warn!(
                "core.max_concurrent_requests is very high ({}), consider reducing",
        ...
        } else if self.core.request_timeout_ms > 600000 {
            // 10 minutes
            warn!(
                "core.request_timeout_ms is very high ({}ms), consider reducing",
        ...
        if self.tokenizer.vocab_size == 0 {
            ...
            warn!(
                "tokenizer.vocab_size is very large ({}), this may impact performance",
        ...
        if self.tokenizer.min_frequency == 0 {
            ...
            warn!(
                "tokenizer.min_frequency is high ({}), may reduce vocabulary coverage",
    ...
    pub fn summary(&self) -> String {
        ...
        let summary = format!(
            "📋 Nexora AI Configuration Summary:\n\
             ...
             └─────────────────────────────────────────────────────────────┘",
            // Core
            if self.core.enable_ml_intent {
                "✅ Enabled"
            } else {
                "❌ Disabled"
            },
            if self.core.enable_coordination {
                "✅ Enabled"
            } else {
                "❌ Disabled"
            },
    ...
    pub fn to_safe_json(&self) -> NexoraResult<serde_json::Value> {
        ...
        let json_value = serde_json::to_value(self).map_err(|e| NexoraError::serialization(e))?;
