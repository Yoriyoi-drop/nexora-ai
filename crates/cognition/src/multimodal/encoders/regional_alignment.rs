...
impl RegionalAlignment {
            ...
            for b in 0..batch_size {
                for p in start_patch..end_patch {
                    for d in 0..embed_dim {
                        ...
                        let dst_idx = b * (end_patch - start_patch) * embed_dim
                            + (p - start_patch) * embed_dim
                            + d;
    ...
    pub fn compute_regional_contrastive_loss(
        ...
        if image_regions.len() != text_phrases.len() {
            return Err(crate::multimodal::error::CaffeineError::input_validation(
        ...
        self.alignment_matrix = Some(ArrayD::from_shape_vec(
            matrix_shape,
