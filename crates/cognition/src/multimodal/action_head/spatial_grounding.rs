...
use crate::multimodal::config::BBoxFormat as ConfigBBoxFormat;
...
impl SpatialGroundingModule {
        ...
        for token in tokens {
            if token.modality == ModalityType::Image {
                ...
                if let Some((x, y, w, h)) = token.spatial_coords {
                    ...
                    let distance = ((center_x - bbox_center_x).powi(2)
                        + (center_y - bbox_center_y).powi(2))
                    .sqrt();
                    let max_distance =
                        (bbox.coords.2 - bbox.coords.0).max(bbox.coords.3 - bbox.coords.1);
    ...
    fn token_id_to_label(&self, token_id: usize) -> Result<String> {
        let common_labels = vec![
            "person", "car", "tree", "house", "sky", "cloud", "sun", "moon", "star", "mountain",
            "river", "ocean", "beach", "forest", "road", "building", "animal", "dog", "cat",
            "bird", "flower", "grass", "rock", "sand", "phone", "computer", "book", "table",
            "chair", "door", "window",
        ];
                ...
                if let Some((x, y, w, h)) = token.spatial_coords {
                    ...
                    let coords = match self.config.bbox_format {
                        ...
                        ConfigBBoxFormat::CXCYWH => {
                            (norm_x + norm_w / 2.0, norm_y + norm_h / 2.0, norm_w, norm_h)
                        }
                    };
