...
impl AgenticActionHead {
    ...
    fn grounding_to_action(&self, grounding: SpatialGrounding) -> Result<ActionOutput> {
        let action = Action {
            ...
            parameters: {
                ...
                let bbox_list: Vec<serde_json::Value> = grounding
                    .bounding_boxes
                    .into_iter()
                    .map(|bbox| {
                        serde_json::json!({
                ...
                params.insert(
                    ...
                    serde_json::Value::Array(bbox_list),
                );
                params
            },
            timestamp: std::time::SystemTime::now()
                ...
                .as_secs_f32(),
            confidence: grounding.confidence_scores.iter().sum::<f32>()
                / grounding.confidence_scores.len() as f32,
        };
    ...
    pub fn update_config(&mut self, config: crate::multimodal::config::ActionConfig) -> Result<()> {
        self.config = config;
