...
pub struct TriQueryFormer {
    ...
    config: crate::multimodal::config::QFormerConfig,
}

impl TriQueryFormer {
    /// Create new Tri-Query Former with validation
    pub fn new(config: crate::multimodal::config::QFormerConfig) -> Result<Self> {
    ...
    pub fn transform(&mut self, features: &EncodedFeatures) -> Result<QueryFeatures> {
        ...
        let spatial_features = self.process_spatial_queries_safe(image_features, video_features)?;

        // Process temporal queries (only for modalities that support temporal features)
        let temporal_features =
            self.process_temporal_queries_safe(audio_features, video_features)?;
    ...
    fn validate_cross_modality_consistency(&self, features: &EncodedFeatures) -> Result<()> {
        ...
        if !batch_sizes.is_empty() {
            ...
            for (i, &batch_size) in batch_sizes.iter().enumerate() {
                if batch_size != first_batch_size {
                    return Err(crate::multimodal::error::CaffeineError::tensor_operation(
                        &format!(
                            "Inconsistent batch sizes: modality {} has batch size {}, expected {}",
        ...
        // embed_dims is guaranteed non-empty by the `is_empty` check above
        if !embed_dims.is_empty() {
            let max_embed_dim = *embed_dims.iter().max().unwrap_or(&0);
            let min_embed_dim = *embed_dims.iter().min().unwrap_or(&0);

            if max_embed_dim > min_embed_dim * 4 {
                return Err(crate::multimodal::error::CaffeineError::tensor_operation(
                    &format!(
                        "Embedding dimensions too different: min={}, max={}",
                        min_embed_dim, max_embed_dim
                    ),
                ));
            }
        }
    ...
    fn process_semantic_queries_safe(
        ...
        self.semantic_queries.forward(&semantic_input).map_err(|e| {
            crate::multimodal::error::CaffeineError::qformer(&format!(
                "Semantic query processing failed: {}",
                e
            ))
        })
    }
    ...
    fn global_pool_safe(&self, features: &ArrayD<f32>) -> Result<ArrayD<f32>> {
        ...
        if batch_size > 1024 || seq_len > 4096 || embed_dim > 8192 {
            warn!(
                "Large tensor dimensions detected: batch={}, seq={}, embed={}",
        ...
        ArrayD::from_shape_vec(pooled_shape, pooled).map_err(|e| {
            crate::multimodal::error::CaffeineError::tensor_operation(&format!(
                "Failed to create pooled tensor: {}",
                e
            ))
        })
    }
    ...
    fn process_spatial_queries_safe(
        ...
        self.spatial_queries.forward(&spatial_input).map_err(|e| {
            crate::multimodal::error::CaffeineError::qformer(&format!(
                "Spatial query processing failed: {}",
                e
            ))
        })
    }
    ...
    fn extract_spatial_features_safe(&self, features: &ArrayD<f32>) -> Result<ArrayD<f32>> {
        ...
        if spatial_size.0 * spatial_size.1 != seq_len {
            warn!(
                "Spatial size {}x{} doesn't match sequence length {}",
                spatial_size.0, spatial_size.1, seq_len
            );
        }

        let mut spatial_features =
            vec![0.0f32; batch_size * spatial_size.0 * spatial_size.1 * embed_dim];
                    ...
                    for d in 0..embed_dim {
                        ...
                        if seq_idx < seq_len {
                            if let Some(&val) = features.get([b, seq_idx, d]) {
                                if val.is_finite() {
                                    let output_idx =
                                        b * spatial_size.0 * spatial_size.1 * embed_dim
                                            + i * spatial_size.1 * embed_dim
                                            + j * embed_dim
                                            + d;
        ...
        ArrayD::from_shape_vec(spatial_shape, spatial_features).map_err(|e| {
            crate::multimodal::error::CaffeineError::tensor_operation(&format!(
                "Failed to create spatial tensor: {}",
                e
            ))
        })
    }
    ...
    fn process_temporal_queries_safe(
        ...
        self.temporal_queries.forward(&temporal_input).map_err(|e| {
            crate::multimodal::error::CaffeineError::qformer(&format!(
                "Temporal query processing failed: {}",
                e
            ))
        })
    }
    ...
    fn extract_temporal_features_safe(&self, features: &ArrayD<f32>) -> Result<ArrayD<f32>> {
        ...
        ArrayD::from_shape_vec(temporal_shape, temporal_features).map_err(|e| {
            crate::multimodal::error::CaffeineError::tensor_operation(&format!(
                "Failed to create temporal tensor: {}",
                e
            ))
        })
    }
    ...
    fn compute_attention_weights_safe(
        ...
        let semantic_flat = semantic
            .view()
            .into_dimensionality::<ndarray::Ix2>()
            .map_err(|e| {
                crate::multimodal::error::CaffeineError::qformer(&format!(
                    "Failed to convert semantic to 2D: {}",
                    e
                ))
            })?;
        let spatial_flat = spatial
            .view()
            .into_dimensionality::<ndarray::Ix2>()
            .map_err(|e| {
                crate::multimodal::error::CaffeineError::qformer(&format!(
                    "Failed to convert spatial to 2D: {}",
                    e
                ))
            })?;
        let temporal_flat = temporal
            .view()
            .into_dimensionality::<ndarray::Ix2>()
            .map_err(|e| {
                crate::multimodal::error::CaffeineError::qformer(&format!(
                    "Failed to convert temporal to 2D: {}",
                    e
                ))
            })?;
        ...
        if combined.len() != total_queries * self.config.hidden_dim {
            return Err(crate::multimodal::error::CaffeineError::qformer(&format!(
                "Combined features size {} doesn't match expected {}x{}",
                combined.len(),
                total_queries,
        ...
        if attention_weights.len() != total_queries * total_queries {
            return Err(crate::multimodal::error::CaffeineError::qformer(&format!(
                "Attention weights size {} doesn't match expected {}x{}",
        ...
        ArrayD::from_shape_vec(shape, attention_weights).map_err(|e| {
            crate::multimodal::error::CaffeineError::qformer(&format!(
                "Failed to create attention weights tensor: {}",
                e
            ))
        })
    }
