...
impl SamplingEngine {
    ...
    pub fn new(config: SamplingConfig) -> SACAResult<Self> {
        ...
        info!(
            "Sampling Engine initialized with {} candidates",
    ...
    pub async fn sample(
        ...
        debug!(
            "Starting large-scale sampling for {} modules",
            modules.len()
        );
        ...
        for module in modules {
            let module_candidates = self
                .generate_module_candidates(module, context, cot_result)
                .await?;
        ...
        match self.config.sampling_strategy {
            SamplingStrategy::Random => {
                self.random_strategy
                    .sample(module, context, cot_result, self.config.num_candidates)
                    .await
            }
            SamplingStrategy::Diverse => {
                self.diverse_strategy
                    .sample(module, context, cot_result, self.config.num_candidates)
                    .await
            }
            SamplingStrategy::QualityFocused => {
                self.quality_focused_strategy
                    .sample(module, context, cot_result, self.config.num_candidates)
                    .await
            }
            SamplingStrategy::PerformanceFocused => {
                self.performance_focused_strategy
                    .sample(module, context, cot_result, self.config.num_candidates)
                    .await
            }
            SamplingStrategy::Hybrid => {
                ...
                if hybrid_count > 0 {
                    if let Ok(candidates) = self
                        .random_strategy
                        .sample(module, context, cot_result, hybrid_count)
                        .await
                    {
    ...
    async fn apply_quality_filter(
        &self,
        mut candidates: Vec<SamplingCandidate>,
    ) -> SACAResult<Vec<SamplingCandidate>> {
    ...
    /// Ensure we have the required number of candidates
    async fn ensure_candidate_count(
        &self,
        mut candidates: Vec<SamplingCandidate>,
    ) -> SACAResult<Vec<SamplingCandidate>> {
    ...
    async fn calculate_diversity_scores(
        &self,
        mut candidates: Vec<SamplingCandidate>,
    ) -> SACAResult<Vec<SamplingCandidate>> {
        for i in 0..candidates.len() {
            let diversity_score = self
                .diversity_calculator
                .calculate_diversity(&candidates[i], &candidates)
                .await?;
            // Update complexity score to include diversity
            candidates[i].complexity_score =
                (candidates[i].complexity_score + diversity_score) / 2.0;
        }
...
impl DiversityCalculator {
    ...
    async fn calculate_diversity(
        ...
        all_candidates: &[SamplingCandidate],
    ) -> SACAResult<f32> {
