...
impl ContextEngine {
    ...
    pub fn new(config: ContextConfig) -> SACAResult<Self> {
        ...
        info!(
            "Context Engine initialized with max {} files to analyze",
    ...
    pub async fn analyze(
        ...
        let context = self
            .perform_repository_analysis(&repo_path, modules, task)
            .await?;
        ...
        info!(
            "Repository context analysis completed: {} files analyzed",
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if entry_path.is_dir() {
                ...
                if self.should_scan_directory(&entry_path) {
                    let sub_files =
                        Box::pin(self.scan_repository_files(
    ...
    fn should_analyze_file(&self, path: &Path) -> bool {
        ...
        if let Some(extension) = path.extension().and_then(|e| e.to_str()) {
            ...
            let is_test_file = path
                .to_str()
                .map_or(false, |s| s.contains("test") || s.contains("spec"));
    ...
    fn update_naming_conventions(
        &self,
        conventions: &mut NamingConventions,
        samples: &NamingSamples,
    ) {
    ...
    async fn detect_architectural_patterns(
        ...
        if patterns.contains_key("repository") || patterns.contains_key("dao") {
            context
                ...
                .push("Repository Pattern".to_string());
        }

        if patterns.contains_key("factory") {
            context
                .architectural_patterns
                .push("Factory Pattern".to_string());
        }

        if patterns.contains_key("observer") {
            context
                .architectural_patterns
                .push("Observer Pattern".to_string());
        }

        if patterns.contains_key("singleton") {
            context
                .architectural_patterns
                .push("Singleton Pattern".to_string());
        }
    ...
    async fn detect_test_frameworks(&self, context: &mut RepositoryContext) -> SACAResult<()> {
        ...
        for dep in &context.dependencies {
            match dep.to_lowercase().as_str() {
                "tokio-test" | "criterion" => {
                    context.test_frameworks.push("Rust Testing".to_string())
                }
                "pytest" | "unittest" => context.test_frameworks.push("Python Testing".to_string()),
                "jest" | "mocha" | "chai" => context
                    .test_frameworks
                    .push("JavaScript Testing".to_string()),
                _ => {}
            }
        }
    ...
    /// Analyze module-specific context
    async fn analyze_module_context(
        &self,
        context: &mut RepositoryContext,
        modules: &[Module],
    ) -> SACAResult<()> {
