...
impl IpoTrainer {
    ...
    pub fn update_constraints(&mut self) -> Result<()> {
        for constraint in self.identity_constraints.values_mut() {
            let new_response = self.generate_current_response(&constraint.prompt)?;
            constraint.current_response = new_response;
            constraint.current_logprob = self.model.log_probability(
                &constraint.prompt, 
                &constraint.current_response
            )?;
        }
...
impl PolicyModel {
    fn reference_log_probability(&self, input: &str, output: &str) -> Result<f32> {
