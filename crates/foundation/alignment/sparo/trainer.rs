...
impl SparoTrainer {
    ...
    pub fn get_training_stats(&self) -> TrainingStats {
        ...
        let loss_trend = if self.training_state.loss_history.len() >= 2 {
            ...
            let earlier = self.training_state.loss_history.iter().rev().skip(5).take(5).sum::<f32>() / 5.0.max(1.0);
            recent - earlier
        } else {
            0.0
        };
...
pub mod utils {
    ...
    pub fn analyze_metrics(metrics: &[TrainingMetrics]) -> MetricsAnalysis {
        ...
        let best_loss = total_losses.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(&0.0);
