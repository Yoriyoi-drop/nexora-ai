...
impl SpectraCoordinator {
    ...
    fn setup_default_routing_rules(&mut self) {
        ...
        self.add_routing_rule(visual_rule);
        self.add_routing_rule(style_rule);
        self.add_routing_rule(innovation_rule);
        self.add_routing_rule(artistic_rule);
    }
    
    /// Setup agent weights based on creative capabilities
    fn setup_agent_weights(&mut self) {
        self.add_agent_weight("creative_muse".to_string(), 0.3);
        self.add_agent_weight("artistic_weaver".to_string(), 0.2);
        self.add_agent_weight("style_adapter".to_string(), 0.25);
        self.add_agent_weight("innovation_engine".to_string(), 0.25);
    }
    ...
    fn setup_communication_channels(&mut self) {
        ...
        self.add_communication_channel(inspiration_channel);
        self.add_communication_channel(style_channel);
        self.add_communication_channel(multimodal_channel);
    }
...
mod tests {
    ...
    fn test_spectra_coordinator_creation() {
        ...
        assert!(coordinator.communication_channels.len() == 3);
    }
