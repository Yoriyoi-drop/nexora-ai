...
impl Caffeine {
    ...
    pub fn new(config: CaffeineConfig) -> crate::caffeine::error::Result<Self> {
        ...
        let atqs_compression = if config.enable_atqs_compression {
            Some(crate::atqs::compression::adaptive_rank::CompressionEngine::new(config.atqs_config.clone().unwrap_or_default())?)
        } else {
            None
        };
        ...
        let has_moe_router = if config.enable_has_moe_routing {
            Some(crate::has_moe_ffn::routing::Router::new(
        ...
        let routed_tokens = if self.has_moe_router.is_some() {
            ...
            if let Some(ref mut router) = self.has_moe_router {
                ...
                let routing_decisions: Vec<crate::has_moe_ffn::types::RoutingDecision> = routing_decisions_raw
                    ...
                    .flatten()
                    .enumerate()
                    .map(|(i, expert_id)| crate::has_moe_ffn::types::RoutingDecision {
                        expert_id,
