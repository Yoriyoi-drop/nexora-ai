...
use crate::erp::{ERPConfig, ERPError, ResonanceGroup};
...
impl SuperpositionCompressor {
    pub fn new(config: ERPConfig) -> Self {
        let compression_method = match config.compression_mode {
            crate::erp::CompressionMode::Conservative => CompressionMethod::WeightedSuperposition,
            crate::erp::CompressionMode::Balanced => CompressionMethod::LowRankApproximation { rank: 16 },
            crate::erp::CompressionMode::Aggressive => CompressionMethod::SparseResidual { sparsity: 0.1 },
        };
