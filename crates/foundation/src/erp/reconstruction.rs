...
impl ContextReconstructor {
    pub fn new(config: ERPConfig) -> Self {
        let reconstruction_method = match config.compression_mode {
            crate::erp::CompressionMode::Conservative => ReconstructionMethod::FullDecompression,
            crate::erp::CompressionMode::Balanced => ReconstructionMethod::SparseGated { target_sparsity: 0.3 },
            crate::erp::CompressionMode::Aggressive => ReconstructionMethod::AdaptiveReconstruction { budget: 64 },
        };
    ...
    fn compute_layer_gates(&self, layer: &CompressedLayer, input: &Array1<f32>) -> Result<GatePattern, ERPError> {
        ...
        for (i, status) in layer.neuron_status.iter().enumerate() {
            if matches!(status, crate::erp::compression::NeuronStatus::Original) {
...
impl GateNetwork {
    ...
    pub fn compute_gate_score(&self, context_embedding: &Array1<f32>, resonance_rep: &crate::erp::compression::ResonanceRepresentation) -> f32 {
