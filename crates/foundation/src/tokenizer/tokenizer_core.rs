...
impl TokenizerCore {
    ...
    pub fn tokenize(&self, text: &str) -> Result<Vec<u32>> {
        ...
        for ch in chars {
            ...
            if let Some(&id) = self.vocab.get(s) {
                ...
                tokens.push(
                    self.special_tokens
                        .get_id(&crate::tokenizer::special_tokens::SpecialTokenID::Unk),
                );
            }
        }
    ...
    pub fn decode(&self, token_ids: &[u32]) -> Result<String> {
        ...
        for &id in token_ids {
            if let Some(token_str) = self.get_token_str(id) {
                ...
                result.push_str(
                    self.special_tokens
                        .get_token(&crate::tokenizer::special_tokens::SpecialTokenID::Unk),
                );
            }
        }
    ...
    pub fn get_vocab_stats(&self) -> VocabStats {
        ...
        VocabStats {
            ...
            total_frequency,
            average_frequency: if self.vocab.is_empty() {
                0.0
            } else {
                total_frequency as f64 / self.vocab.len() as f64
            },
        }
    }
    ...
    pub fn validate(&self) -> Result<()> {
            ...
            if let Some(&vocab_id) = self.vocab.get(token_str) {
                if vocab_id != id {
                    return Err(anyhow::anyhow!(
                        "ID mismatch for token '{}': vocab={}, reverse={}",
                        token_str,
                        vocab_id,
                        id
                    ));
                ...
                return Err(anyhow::anyhow!(
                    "Token '{}' in reverse vocab not found in vocab",
                    token_str
                ));
            }
        }
        ...
        for merge in &self.merges {
            if !self.reverse_vocab.contains_key(&merge.left) {
                return Err(anyhow::anyhow!(
                    "Merge rule references non-existent left token ID: {}",
                    merge.left
                ));
            }
            if !self.reverse_vocab.contains_key(&merge.right) {
                return Err(anyhow::anyhow!(
                    "Merge rule references non-existent right token ID: {}",
                    merge.right
                ));
            }
            if !self.reverse_vocab.contains_key(&merge.new_id) {
                return Err(anyhow::anyhow!(
                    "Merge rule references non-existent new token ID: {}",
                    merge.new_id
                ));
            }
        }
