...
use crate::tokenizer::tokenizer_core::TokenizerCore;
use anyhow::Result;
...
impl VocabBuilder {
    ...
    pub fn build(mut self) -> Result<Vec<VocabEntry>> {
        ...
        self.vocab
            .retain(|_, entry| entry.is_special || entry.frequency >= self.config.min_frequency);
    ...
    fn normalize_scores(&mut self) {
        let non_special_entries: Vec<&mut VocabEntry> =
            self.vocab.values_mut().filter(|e| !e.is_special).collect();

        if non_special_entries.is_empty() {
            return;
        }

        let min_score = non_special_entries
            .iter()
            .map(|e| e.score)
            .min_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
            .unwrap_or(0.0);

        let max_score = non_special_entries
            .iter()
            .map(|e| e.score)
...
mod tests {
    ...
    fn test_byte_level_vocab() {
        ...
        let byte_tokens: Vec<&VocabEntry> =
            vocab.iter().filter(|entry| !entry.is_special).collect();
    ...
    fn test_build_vocab_from_texts() {
        ...
        let vocab_strings: Vec<String> = vocab
            .iter()
            .filter(|e| !e.is_special)
            .map(|e| e.token.clone())
            .collect();
