...
impl SpecialTokenID {
    ...
    pub fn is_start(&self) -> bool {
        matches!(
            self,
            SpecialTokenID::CodeStart
                | SpecialTokenID::MathStart
                | SpecialTokenID::DocStart
                | SpecialTokenID::UserStart
                | SpecialTokenID::AssistantStart
                | SpecialTokenID::SystemStart
                | SpecialTokenID::AnswerStart
                | SpecialTokenID::BugStart
                | SpecialTokenID::FixStart
                | SpecialTokenID::DiffStart
                | SpecialTokenID::TracebackStart
    ...
    pub fn is_end(&self) -> bool {
        matches!(
            ...
            SpecialTokenID::Eos
                | SpecialTokenID::Eod
                | SpecialTokenID::CodeEnd
                | SpecialTokenID::MathEnd
                | SpecialTokenID::DocEnd
                | SpecialTokenID::UserEnd
                | SpecialTokenID::AssistantEnd
                | SpecialTokenID::SystemEnd
                | SpecialTokenID::AnswerEnd
                | SpecialTokenID::BugEnd
                | SpecialTokenID::FixEnd
                | SpecialTokenID::DiffEnd
                | SpecialTokenID::TracebackEnd
...
impl SpecialTokens {
    ...
    pub fn get_token_str(&self, id: u32) -> Option<&str> {
        Self::builtin_str_by_id(id).or_else(|| self.custom_id_to_token.get(&id).map(|s| s.as_str()))
    }
    ...
    pub fn is_special_str(&self, token_str: &str) -> bool {
        Self::builtin_id_by_str(token_str).is_some()
            || self.custom_token_to_id.contains_key(token_str)
    }
    ...
    pub fn get_all_tokens(&self) -> Vec<&str> {
        (0..SpecialTokenID::COUNT)
            .map(|i| {
                SpecialTokenID::try_from_id(i as u32)
                    .map(|t| t.as_str())
                    .unwrap_or("<unk>")
            })
            .collect()
        ...
        for (start, end) in pairs {
            if used_positions.contains(&start) || used_positions.contains(&end) {
                return Err(anyhow::anyhow!(
                    "Overlapping special token pairs at positions {} and {}",
                    start,
                    end
                ));
            }
        ...
        for (i, &id) in token_ids.iter().enumerate() {
            if !used_positions.contains(&i) {
                if let Some(token) = Self::get_special_token_by_id(id) {
                    if token.is_start() {
                        return Err(anyhow::anyhow!(
                            "Unmatched start token at position {}: {}",
                            i,
                            token.as_str()
                        ));
                    }
...
mod tests {
    ...
    fn test_token_matching() {
        assert_eq!(
            SpecialTokenID::CodeStart.matching_end(),
            Some(SpecialTokenID::CodeEnd)
        );
        assert_eq!(
            SpecialTokenID::CodeEnd.matching_start(),
            Some(SpecialTokenID::CodeStart)
        );
