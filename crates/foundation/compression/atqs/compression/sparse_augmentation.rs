...
fn reshape_to_2d(tensor: &ArrayD<f32>) -> Result<Array<f32, ndarray::Ix2>, crate::ATQSError> {
    ...
    if shape.len() == 2 {
        return Ok(tensor.to_owned().into_dimensionality().unwrap());
    }
    ...
    Ok(tensor.clone().into_shape((rows, cols)).unwrap())
}
...
fn reshape_from_2d(
    ...
    Ok(array_2d.clone().into_shape(original_shape).unwrap().into_dyn())
}

/// Compute truncated SVD (placeholder implementation)
