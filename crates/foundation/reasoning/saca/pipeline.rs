...
impl SACAPipeline {
    ...
    async fn execute_single_iteration(&self, task: &CodingTask, session: &mut PipelineSession) -> SACAResult<SACASolution> {
        ...
        let modules = self.execute_phase_with_metrics(
            ...
            self.decompose_engine.decompose(pipeline_data.cot_result.as_ref().unwrap())
        ).await?;
        ...
        let context = self.execute_phase_with_metrics(
            ...
            self.context_engine.analyze(pipeline_data.modules.as_ref().unwrap(), task)
        ).await?;
        ...
        let candidates = self.execute_phase_with_metrics(
            ...
            self.sampling_engine.sample(
                pipeline_data.modules.as_ref().unwrap(),
                pipeline_data.context.as_ref().unwrap(),
                pipeline_data.cot_result.as_ref().unwrap()
            )
        ).await?;
        ...
        let executed_candidates = self.execute_phase_with_metrics(
            ...
            self.execute_engine.execute_all(pipeline_data.candidates.as_ref().unwrap().clone(), pipeline_data.context.as_ref().unwrap())
        ).await?;
        ...
        let solution = self.execute_phase_with_metrics(
            ...
            self.rerank_engine.rerank(pipeline_data.executed_candidates.as_ref().unwrap().clone(), pipeline_data.context.as_ref().unwrap())
        ).await?;
        session.phase_history.push(session.current_phase);
...
mod tests {
    ...
    async fn test_phase_metrics() {
        ...
        let _solution = pipeline.execute(task).await.unwrap();
