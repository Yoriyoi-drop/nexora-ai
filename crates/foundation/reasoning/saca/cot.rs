...
mod tests {
    ...
    async fn test_cot_reasoning() {
        ...
        let cot = CoTEngine::new(config).unwrap();
        ...
        let result = cot.reason(&task).await.unwrap();
    ...
    async fn test_cot_cache() {
        ...
        let result1 = cot.reason(&task).await.unwrap();
        
        // Second call should use cache
        let result2 = cot.reason(&task).await.unwrap();
