...
use crate::saca::{types::*, config::*, error::*, prelude::*};
use crate::atqs::compression::CompressionEngine;
use crate::caffeine::Caffeine;
use crate::has_moe_ffn::router::ExpertRouter;
...
pub struct SACAIntegration {
    ...
    has_moe_router: Option<Arc<ExpertRouter>>,
}
...
impl SACAIntegration {
    ...
    pub fn with_has_moe_routing(mut self, router: Arc<ExpertRouter>) -> Self {
    ...
    async fn apply_has_moe_routing(
        ...
        let mut router_clone = match Arc::try_unwrap(Arc::clone(router)) {
            ...
            Err(_) => {
                ...
                ExpertRouter::new(crate::has_moe_ffn::config::HasMoeFfnConfig::medium_model().router_config.clone())
                    .map_err(|e| SACAError::RerankError(format!("Router creation failed: {}", e)))?
            }
        };
    ...
    fn solution_to_tensor(&self, solution: &SACASolution) -> SACAResult<ndarray::ArrayD<f32>> {
        ...
        Ok(ndarray::ArrayD::from_shape_vec(shape, features)
            .map_err(|e| SACAError::ContextError(format!("Tensor creation failed: {}", e)))?)
    }
...
impl SACAFactory {
    ...
    pub async fn create_full_saca(
        ...
        has_moe_config: Option<crate::has_moe_ffn::config::HasMoeFfnConfig>,
    ...
    pub async fn create_saca_with_has_moe(
        saca_config: SACAConfig,
        has_moe_config: crate::has_moe_ffn::config::HasMoeFfnConfig,
    ) -> SACAResult<SACAIntegration> {
        let router = Arc::new(ExpertRouter::new(has_moe_config.router_config.clone())
            .map_err(|e| SACAError::RerankError(format!("Router creation failed: {}", e)))?);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_saca_integration_creation() {
    ...
    async fn test_integration_stats() {
        let config = SACAConfig::default();
        let integration = SACAIntegration::new(config).await.unwrap();
    ...
    #[tokio::test]
    async fn test_solution_to_tensor() {
        let integration = SACAIntegration::new(SACAConfig::default()).await.unwrap();
        ...
        let tensor = integration.solution_to_tensor(&solution).unwrap();
        assert_eq!(tensor.shape(), &[1, 768]);
    }
}
