        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if entry_path.is_dir() {
                ...
                if self.should_scan_directory(&entry_path) {
                    let sub_files = Box::pin(self.scan_repository_files(entry_path.to_str().unwrap())).await?;
        ...
        for line in content.lines() {
            ...
            if trimmed.starts_with("fn ") {
                if let Some(fn_name) = trimmed.split('(').next() {
                    let fn_name = fn_name.replace("fn ", "").trim().to_string();
                    if !fn_name.is_empty() {
                        analysis.functions.push(fn_name.clone());
                        analysis.naming_samples.function_names.push(fn_name);
                    }
                }
            }
            ...
            if trimmed.contains("impl ") {
                *analysis.patterns.entry("impl".to_string()).or_insert(0) += 1;
            }
            
            if trimmed.contains("struct ") {
                *analysis.patterns.entry("struct".to_string()).or_insert(0) += 1;
                if let Some(struct_name) = trimmed.split('{').next() {
                    let struct_name = struct_name.replace("struct ", "").trim().to_string();
                    if !struct_name.is_empty() {
                        analysis.naming_samples.class_names.push(struct_name);
                    }
                }
            }
            
            if trimmed.contains("const ") {
                *analysis.patterns.entry("const".to_string()).or_insert(0) += 1;
                if let Some(const_name) = trimmed.split('=').next() {
                    let const_name = const_name.replace("const ", "").trim().to_string();
                    if !const_name.is_empty() {
                        analysis.naming_samples.constant_names.push(const_name);
                    }
                }
            }
        }
    ...
    fn analyze(&self, content: &str, _file_path: &str) -> SACAResult<FileAnalysis> {
        ...
        for line in content.lines() {
            ...
            if trimmed.starts_with("def ") {
                if let Some(fn_name) = trimmed.split('(').next() {
                    let fn_name = fn_name.replace("def ", "").trim().to_string();
                    if !fn_name.is_empty() {
                        analysis.functions.push(fn_name.clone());
                        analysis.naming_samples.function_names.push(fn_name);
                    }
            ...
            if trimmed.starts_with("import ") || trimmed.starts_with("from ") {
                let import = trimmed.split_whitespace().next().unwrap_or("").to_string();
                if !import.is_empty() {
                    analysis.dependencies.push(import);
                }
            }
            
            // Detect patterns
            if trimmed.starts_with("class ") {
                *analysis.patterns.entry("class".to_string()).or_insert(0) += 1;
                if let Some(class_name) = trimmed.split('(').next() {
                    let class_name = class_name.replace("class ", "").trim().to_string();
                    if !class_name.is_empty() {
                        analysis.naming_samples.class_names.push(class_name);
                    }
                }
            }
        }
    ...
    fn analyze(&self, content: &str, _file_path: &str) -> SACAResult<FileAnalysis> {
        ...
        for line in content.lines() {
            ...
            if trimmed.starts_with("import ") || trimmed.starts_with("const ") && trimmed.contains("require") {
                analysis.dependencies.push("module".to_string());
            }
        }
...
/// Generic file analyzer for other languages
...
mod tests {
    ...
    async fn test_context_analysis() {
        ...
        let context = engine.analyze(&modules, &task).await.unwrap();
    ...
    async fn test_rust_analyzer() {
        ...
        let analysis = analyzer.analyze(content, "test.rs").unwrap();
