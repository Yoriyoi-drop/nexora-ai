...
use crate::saca::config::FeedbackConfig;
...
mod tests {
    ...
    async fn test_feedback_system() {
        ...
        let system = FeedbackSystem::new(config).unwrap();
        ...
        let feedback = system.generate_feedback(&solution, &context).await.unwrap();
    ...
    async fn test_quality_analysis() {
        let system = FeedbackSystem::new(FeedbackConfig::default()).unwrap();
        ...
        let analysis = system.analyze_solution_quality(&solution).await.unwrap();
