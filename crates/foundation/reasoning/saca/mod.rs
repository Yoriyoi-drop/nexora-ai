...
pub struct SACA {
    ...
    // 6 core phases
    cot_engine: Arc<crate::saca::cot::CoTEngine>,
    decompose_engine: Arc<crate::saca::decompose::DecomposeEngine>,
    context_engine: Arc<crate::saca::context::ContextEngine>,
    sampling_engine: Arc<crate::saca::sampling::SamplingEngine>,
    execute_engine: Arc<crate::saca::execute::ExecuteEngine>,
    rerank_engine: Arc<crate::saca::rerank::RerankEngine>,
    
    // Feedback loop system
    feedback_system: Arc<crate::saca::feedback::FeedbackSystem>,
...
impl SACA {
    ...
    pub async fn new(config: SACAConfig) -> Result<Self, SACAError> {
        ...
        let cot_engine = Arc::new(crate::saca::cot::CoTEngine::new(config.cot_config.clone())?);
        let decompose_engine = Arc::new(crate::saca::decompose::DecomposeEngine::new(config.decompose_config.clone())?);
        let context_engine = Arc::new(crate::saca::context::ContextEngine::new(config.context_config.clone())?);
        let sampling_engine = Arc::new(crate::saca::sampling::SamplingEngine::new(config.sampling_config.clone())?);
        let execute_engine = Arc::new(crate::saca::execute::ExecuteEngine::new(config.execute_config.clone())?);
        let rerank_engine = Arc::new(crate::saca::rerank::RerankEngine::new(config.rerank_config.clone())?);
        
        // Initialize feedback system
        let feedback_system = Arc::new(crate::saca::feedback::FeedbackSystem::new(config.feedback_config.clone())?);
