...
impl MetricsCollector {
    ...
    pub async fn record_request_completed(
        ...
        metrics.requests.avg_request_size =
            (metrics.requests.avg_request_size * (total_requests - 1) as f64 + input_tokens as f64)
                / total_requests as f64;
        metrics.requests.avg_response_size = (metrics.requests.avg_response_size
        ...
        metrics.performance.avg_latency_ms =
            (metrics.performance.avg_latency_ms * (total_requests - 1) as f64 + latency)
                / total_requests as f64;
        ...
        if output_tokens > 0 {
            ...
            metrics.performance.throughput_tokens_per_second =
                (metrics.performance.throughput_tokens_per_second * (total_requests - 1) as f64
                    + tokens_per_second)
                    / total_requests as f64;
        }
    ...
    pub async fn get_aggregated_metrics(&self, window_seconds: u64) -> Result<InferenceMetrics> {
        ...
        let relevant_metrics: Vec<&InferenceMetrics> = history
            .iter()
        ...
        aggregated.requests.total_requests = relevant_metrics
            .iter()
            .map(|m| m.requests.total_requests)
            .max()
            .unwrap_or(0);
        aggregated.requests.successful_requests = relevant_metrics
            .iter()
            .map(|m| m.requests.successful_requests)
            .max()
            .unwrap_or(0);
        aggregated.requests.failed_requests = relevant_metrics
            .iter()
            .map(|m| m.requests.failed_requests)
            .max()
            .unwrap_or(0);
        aggregated.requests.avg_request_size = relevant_metrics
            .iter()
            .map(|m| m.requests.avg_request_size)
            .sum::<f64>()
            / count as f64;
        aggregated.requests.avg_response_size = relevant_metrics
            .iter()
        ...
        aggregated.performance.avg_latency_ms = relevant_metrics
            .iter()
            .map(|m| m.performance.avg_latency_ms)
            .sum::<f64>()
            / count as f64;
        aggregated.performance.throughput_tokens_per_second = relevant_metrics
            .iter()
        ...
        aggregated.resources.cpu_usage_percent = relevant_metrics
            .iter()
            .map(|m| m.resources.cpu_usage_percent)
            .sum::<f64>()
            / count as f64;
        aggregated.resources.memory_usage_percent = relevant_metrics
            .iter()
    ...
    async fn cleanup_old_data(&self) -> Result<()> {
        ...
        // Clean up resolved alerts
        {
            ...
            alerts.retain(|a| {
                a.resolved_at.is_none() || a.resolved_at.unwrap_or(Utc::now()) > cutoff_time
            });
...
pub mod analysis {
    ...
    pub fn detect_anomalies(metrics: &[InferenceMetrics], threshold: f64) -> Vec<usize> {
        if metrics.len() < 3 {
            return Vec::new();
        }
        ...
        for i in 1..metrics.len() - 1 {
            ...
            let avg_latency = (prev.performance.avg_latency_ms
                + curr.performance.avg_latency_ms
                + next.performance.avg_latency_ms)
                / 3.0;
            let std_dev = (((prev.performance.avg_latency_ms - avg_latency).powi(2)
                + (curr.performance.avg_latency_ms - avg_latency).powi(2)
                + (next.performance.avg_latency_ms - avg_latency).powi(2))
                / 3.0)
                .sqrt();
