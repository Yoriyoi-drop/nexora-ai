...
impl InferenceSession {
    ...
    pub async fn add_entry(&self, entry: SessionEntry) -> Result<()> {
        debug!(
            "Adding entry to session {}: {}",
            self.session_id, entry.entry_id
        );
    ...
    pub async fn validate(&self) -> Result<()> {
        ...
        if self.is_token_limit_exceeded().await {
            return Err(InferenceError::InternalError(
                "Session token limit exceeded".to_string(),
            ));
        }
        ...
        match state {
            ...
            SessionState::Timeout => {
                Err(InferenceError::InternalError("Session timeout".to_string()))
            }
        }
    }
    ...
    async fn check_session_limits(&self, entry: &SessionEntry) -> Result<()> {
        ...
        if current_tokens + entry.tokens_generated as u64 > self.config.max_tokens_per_session {
            return Err(InferenceError::InternalError(
                "Session token limit would be exceeded".to_string(),
            ));
        }
        ...
        let current_context_length = {
            ...
            history
                .iter()
                .map(|e| e.prompt.len() + e.response.len())
                .sum::<usize>()
        };
        ...
        if new_context_length > self.config.max_context_length {
            return Err(InferenceError::InternalError(
                "Session context length limit would be exceeded".to_string(),
            ));
        }
