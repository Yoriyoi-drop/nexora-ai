...
impl PersistentResonanceMemory {
    ...
    pub fn retrieve(
        ...
        let mut resonances =
            Vec::with_capacity(self.memory_entries.iter().filter(|e| e.is_some()).count());

        let entries_to_process: Vec<(usize, ArrayD<f32>)> = self
            .memory_entries
            .iter()
            .enumerate()
            .filter_map(|(slot_idx, entry)| {
                entry
                    .as_ref()
                    .map(|memory_entry| (slot_idx, memory_entry.data.clone()))
            })
            .collect();
        ...
        for (slot_idx, entry) in self.memory_entries.iter().enumerate() {
            if let Some(memory_entry) = entry {
                let decayed_importance = memory_entry.importance
                    * self
                        .importance_decay
                        .powf((self.current_timestamp - memory_entry.timestamp) as f32);
    ...
    fn calculate_phase_resonance(
        ...
        let phase_diff: f32 = query_phase
            .iter()
            .zip(memory_phase.iter())
            .map(|(&qp, &mp)| (qp - mp).abs())
            .sum();
    ...
    fn update_statistics(&mut self) -> DLResult<()> {
        ...
        if !self.novelty_history.is_empty() {
            self.average_novelty =
                self.novelty_history.iter().sum::<f32>() / self.novelty_history.len() as f32;
        }

        // Saturation level
        let high_importance_count = self
            .memory_entries
            .iter()
