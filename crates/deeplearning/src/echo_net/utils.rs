...
impl HolographicFFT {
    ...
    pub fn ifft_1d(input: &Array1<Complex>) -> DLResult<Array1<f32>> {
        ...
        Ok(Array1::from(
            result.iter().map(|c| c.real).collect::<Vec<_>>(),
        ))
    }
    ...
    fn fft_recursive(x: &mut [Complex]) {
        ...
        // Combine
        for k in 0..n / 2 {
            let t = Complex::from_polar(1.0, -2.0 * PI * k as f32 / n as f32) * odd[k];
            odd[k] = even[k] - t;
            even[k] = even[k] + t;
        }
    }
...
impl ResonanceCalculator {
    ...
    pub fn interference_pattern(
        wave1: &HolographicWave,
        wave2: &HolographicWave,
    ) -> DLResult<HolographicWave> {
        ...
        for ((idx, &amp1), &amp2) in wave1.amplitude.indexed_iter().zip(wave2.amplitude.iter()) {
            ...
            result.amplitude[idx.clone()] =
                (amp1 * amp1 + amp2 * amp2 + 2.0 * amp1 * amp2 * (phase1 - phase2).cos()).sqrt();
...
impl MemoryCompressor {
    ...
    pub fn generate_summary(input: &Array2<f32>) -> Array1<f32> {
        ...
        // Sparsity
        let sparsity =
            values.iter().filter(|&&x| x.abs() < 1e-6).count() as f32 / values.len() as f32;
