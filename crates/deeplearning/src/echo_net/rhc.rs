...
impl RecursiveHolographicCompression {
    ...
    /// Combine FFT pooled data with summary
    fn combine_compression_results(
        &self,
        fft_pooled: &Array2<f32>,
        summary: &Array1<f32>,
        _level: &CompressionLevel,
    ) -> DLResult<Array2<f32>> {
    ...
    fn update_level_memory(&mut self, level_idx: usize, data: &Array2<f32>) -> DLResult<()> {
        ...
        let start_pos =
            (self.level_statistics[level_idx].total_writes % memory_rows as u64) as usize;
    ...
    fn update_compression_statistics(
        &mut self,
        input: &Array2<f32>,
        compressed_levels: &[Array2<f32>],
        timestamp: usize,
    ) -> DLResult<()> {
        // Calculate quality scores first to avoid borrow conflicts
        let quality_scores: Vec<DLResult<f32>> = compressed_levels
            .iter()
    ...
    fn calculate_compression_quality(
        &self,
        original: &Array2<f32>,
    ...
    fn record_compression_event(
        ...
        let event = CompressionEvent {
            ...
            compression_ratio: output_shape.iter().product::<usize>() as f32
...
mod tests {
    ...
    fn test_importance_calculation() {
        ...
        let input = Array2::from_shape_vec(
            (4, 4),
            vec![
                1.0, 2.0, 3.0, 4.0, 0.0, 0.0, 0.0, 0.0, 5.0, 6.0, 7.0, 8.0, 1.0, 1.0, 1.0, 1.0,
            ],
        )
        .unwrap();
