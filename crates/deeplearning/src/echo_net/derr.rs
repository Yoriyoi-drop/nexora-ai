...
impl DualEntropicResonanceRetrieval {
    ...
    pub fn forward(
        ...
        let final_candidates = if filtered_candidates.is_empty() {
            let mut sorted_candidates: Vec<&RetrievalCandidate> =
                processed_candidates.iter().collect();
    ...
    fn calculate_gate_score(&self, energy: f32, entropy: f32, coherence: f32) -> DLResult<f32> {
        ...
        let gate_input = self.gate_alpha * normalized_energy - self.gate_beta * normalized_entropy
    ...
    fn update_adaptive_weights(&mut self) -> DLResult<()> {
        ...
        let recent_events: Vec<&RetrievalEvent> =
            self.retrieval_history.iter().rev().take(10).collect();

        let avg_efficiency: f32 = recent_events
            .iter()
            .map(|e| e.retrieval_efficiency)
            .sum::<f32>()
            / 10.0;
        let avg_gate_score: f32 = recent_events
            .iter()
            .map(|e| e.average_gate_score)
            .sum::<f32>()
            / 10.0;
    ...
    fn update_memory(&mut self, candidates: Vec<RetrievalCandidate>) -> DLResult<()> {
        ...
        if self.retrieval_memory.len() > self.max_memory_size {
            ...
            self.retrieval_memory.sort_by(|a, b| {
                a.resonance_strength
                    .partial_cmp(&b.resonance_strength)
                    .unwrap_or(std::cmp::Ordering::Equal)
            });
            self.retrieval_memory
                .drain(0..self.retrieval_memory.len() - self.max_memory_size);
        }
    ...
    pub fn get_parameters(&self) -> Vec<ArrayD<f32>> {
        ...
        let gate_alpha = ArrayD::from_shape_vec(vec![1], vec![self.gate_alpha])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
        let gate_beta = ArrayD::from_shape_vec(vec![1], vec![self.gate_beta])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
        let gate_delta = ArrayD::from_shape_vec(vec![1], vec![self.gate_delta])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));

        // Weight parameters: energy_weight, entropy_weight, coherence_weight
        let energy_weight = ArrayD::from_shape_vec(vec![1], vec![self.energy_weight])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
        let entropy_weight = ArrayD::from_shape_vec(vec![1], vec![self.entropy_weight])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
        let coherence_weight = ArrayD::from_shape_vec(vec![1], vec![self.coherence_weight])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));

        // Learning rate parameter
        let weight_update_rate = ArrayD::from_shape_vec(vec![1], vec![self.weight_update_rate])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
