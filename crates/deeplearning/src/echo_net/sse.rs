...
impl SemanticSpectralEmbedding {
    ...
    pub fn forward(&self, token_ids: &[usize], positions: &[usize]) -> DLResult<HolographicWave> {
        ...
        for (i, (&token_id, &pos)) in token_ids.iter().zip(positions.iter()).enumerate() {
            ...
            let embedding_view: ArrayView1<f32> = embedding.view().into_dimensionality()?;
    ...
    pub fn apply_frequency_filter(
        ...
        wave.frequency
            .mapv_inplace(|f| if f < min_freq || f > max_freq { 0.0 } else { f });
    ...
    pub fn update_weights(
        &mut self,
        amplitude_grad: &Array2<f32>,
        phase_grad: &Array2<f32>,
        frequency_grad: &Array2<f32>,
        learning_rate: f32,
    ) -> DLResult<()> {
