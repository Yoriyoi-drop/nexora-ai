...
impl AdaptivePhaseSeparationStabilizer {
    ...
    fn compute_similarity_matrix(&self, embeddings: &ArrayD<f32>) -> DLResult<Array2<f32>> {
        #[cfg(feature = "gpu")]
        if self.use_gpu {
            if let Ok(result) = crate::echo_net::gpu_ops::gpu_cosine_similarity_matrix(embeddings) {
                return Ok(result);
            }
        }
            ...
            for j in 0..num_tokens {
                if i != j {
                    ...
                    if similarity < self.similarity_threshold {
                        ...
                        for k in 0..phase_dim {
                            phase_adjustment[k] +=
                                self.phase_separation_strength * dissimilarity * phase_diff[k];
                        }
                    }
                }
            }
    ...
    fn normalize_phases(&self, phases: &mut ArrayD<f32>) -> DLResult<()> {
        ...
        phases.mapv_inplace(|phase| {
            ...
            if normalized_phase < min_range {
                normalized_phase =
                    min_range + (normalized_phase - min_range) * self.normalization_strength;
            } else if normalized_phase > max_range {
                normalized_phase =
                    max_range + (normalized_phase - max_range) * self.normalization_strength;
            }
    ...
    pub fn get_parameters(&self) -> Vec<ArrayD<f32>> {
        ...
        // Phase adjustment parameters
        let phase_separation_strength =
            ArrayD::from_shape_vec(vec![1], vec![self.phase_separation_strength])
                .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
        let similarity_threshold = ArrayD::from_shape_vec(vec![1], vec![self.similarity_threshold])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
        let max_phase_adjustment = ArrayD::from_shape_vec(vec![1], vec![self.max_phase_adjustment])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));

        // Phase stabilization parameters
        let momentum_factor = ArrayD::from_shape_vec(vec![1], vec![self.momentum_factor])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));

        // Conflict detection parameters
        let conflict_threshold = ArrayD::from_shape_vec(vec![1], vec![self.conflict_threshold])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
        let conflict_penalty = ArrayD::from_shape_vec(vec![1], vec![self.conflict_penalty])
            .unwrap_or_else(|_| ArrayD::zeros(vec![1]));

        // Phase normalization parameters
        let normalization_strength =
            ArrayD::from_shape_vec(vec![1], vec![self.normalization_strength])
                .unwrap_or_else(|_| ArrayD::zeros(vec![1]));
    ...
    pub fn set_parameters_from_tensors(&mut self, tensors: &[crate::autograd::Tensor]) {
