...
impl MultiBandHolographicWriter {
    ...
    pub fn forward(
        &mut self,
        wave: &HolographicWave,
        timestamp: usize,
    ) -> DLResult<Vec<Array2<Complex>>> {
    ...
    fn write_to_band(
        ...
        let new_memory =
            self.apply_interference_handling(&decayed_memory, &scaled_convolution, write_pos)?;
    ...
    fn convolve_with_kernel(
        ...
        #[cfg(feature = "gpu")]
        if self.use_gpu {
            if let Ok(result) = crate::echo_net::gpu_ops::gpu_conv_2d(input, kernel) {
                return Ok(result);
            }
        }
                ...
                for ki in 0..kernel_rows {
                    for kj in 0..kernel_cols {
                        ...
                        output[[output_i, output_j]] =
                            output[[output_i, output_j]] + input[[i, j]] * kernel[[ki, kj]];
                    }
                }
            }
        }
    ...
    fn apply_interference_handling(
        &self,
        current_memory: &Array2<Complex>,
        new_data: &Array2<Complex>,
        write_pos: usize,
    ) -> DLResult<Array2<Complex>> {
    ...
    fn update_interference_matrix(&mut self, memories: &[Array2<Complex>]) -> DLResult<()> {
        ...
        for i in 0..self.num_bands {
            for j in 0..self.num_bands {
                if i != j {
                    let interference =
                        self.calculate_band_interference(&memories[i], &memories[j])?;
    ...
    fn calculate_band_interference(
        &self,
        band1: &Array2<Complex>,
        band2: &Array2<Complex>,
    ) -> DLResult<f32> {
    ...
    fn create_frequency_filter(
        ...
        for i in 0..filter_size {
            ...
            let filter_value =
                (-(freq - center_freq).powi(2) / (2.0 * bandwidth * bandwidth)).exp();
