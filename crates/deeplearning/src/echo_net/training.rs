...
impl PolarGradientNormalization {
    ...
    pub fn apply_pgn(
        ...
        let normalized_magnitude =
            self.normalize_magnitude_gradients(&magnitude_grad, learning_rate)?;

        // Recombine into complex gradients
        let normalized_complex =
            self.recombine_gradients(&normalized_magnitude, &normalized_phase)?;
    ...
    fn apply_momentum(
        &mut self,
        magnitude_grad: &ArrayD<f32>,
        phase_grad: &ArrayD<f32>,
    ) -> DLResult<()> {
        // Update momentum
    ...
    fn normalize_magnitude_gradients(
        &self,
        magnitude_grad: &ArrayD<f32>,
        learning_rate: f32,
    ) -> DLResult<ArrayD<f32>> {
        ...
        if self.magnitude_clipping {
            normalized
                .mapv_inplace(|g| g.clamp(-self.max_magnitude_value, self.max_magnitude_value));
        }
    ...
    fn recombine_gradients(
        &self,
        magnitude: &ArrayD<f32>,
        phase: &ArrayD<f32>,
    ) -> DLResult<ComplexTensor> {
    ...
    fn update_gradient_statistics(
        ...
        let gradient_norm = (magnitude_grad.iter().map(|&g| g * g).sum::<f32>()
            + phase_grad.iter().map(|&g| g * g).sum::<f32>())
        .sqrt();
        ...
        self.average_magnitude_update =
            self.average_magnitude_update * 0.9 + magnitude_mean.abs() * 0.1;
    ...
    fn update_adaptive_lr(&mut self) -> DLResult<()> {
        ...
        let recent_stats: Vec<&GradientStatistics> =
            self.gradient_history.iter().rev().take(10).collect();

        let avg_magnitude = recent_stats
            .iter()
            .map(|s| s.magnitude_mean.abs())
            .sum::<f32>()
            / 10.0;
...
impl ResonanceEnergyClipping {
    ...
    fn calculate_energy(&self, data: &ArrayD<f32>) -> f32 {
        match self.normalization_method {
            NormalizationMethod::L2 => data.iter().map(|&x| x * x).sum::<f32>().sqrt(),
            NormalizationMethod::L1 => data.iter().map(|&x| x.abs()).sum(),
            NormalizationMethod::Softmax => {
                let max_val = data.iter().fold(f32::NEG_INFINITY, |acc, &x| acc.max(x));
                let exp_sum: f32 = data.iter().map(|&x| (x - max_val).exp()).sum();
                exp_sum.ln()
            }
            NormalizationMethod::Tanh => data.iter().map(|&x| x.tanh().abs()).sum(),
        }
    }
    ...
    fn calculate_adaptive_threshold(&self) -> DLResult<f32> {
        ...
        let recent_energies: Vec<f32> =
            self.energy_history.iter().rev().take(50).cloned().collect();

        let mean_energy = recent_energies.iter().sum::<f32>() / recent_energies.len() as f32;
        let std_energy = (recent_energies
            .iter()
    ...
    /// Update energy statistics
    fn update_energy_statistics(
        &mut self,
        _original_energy: f32,
        clipped_energy: f32,
        clipping_ratio: f32,
    ) -> DLResult<()> {
        ...
        if self.energy_stats.average_energy == 0.0 {
            ...
            self.energy_stats.average_energy =
                self.energy_stats.average_energy * 0.99 + clipped_energy * 0.01;
        }
        ...
        if clipping_ratio > 0.0 {
            self.energy_stats.clipping_ratio =
                self.energy_stats.clipping_ratio * 0.99 + clipping_ratio * 0.01;
        }
    ...
    fn adjust_adaptive_threshold(&mut self) -> DLResult<()> {
        ...
        let recent_events: Vec<&ClippingEvent> =
            self.clipping_events.iter().rev().take(100).collect();

        let clipping_frequency = recent_events
            .iter()
        ...
        // Clamp threshold
        self.energy_clip_threshold = self
            .energy_clip_threshold
...
impl TrainingStabilizer {
    ...
    pub fn new(
        ...
        let rec = ResonanceEnergyClipping::new(rec_config.0, rec_config.1, rec_config.2)?;
    ...
    pub fn stabilize(
        ...
        learning_rate: f32,
    ) -> DLResult<(ComplexTensor, ClippingEvent)> {
        ...
        let gradient_stats = self.pgn.get_gradient_statistics().ok_or_else(|| {
            crate::echo_net::DeepLearningError::Computation {
    ...
    fn update_performance_metrics(&mut self) -> DLResult<()> {
        if let Some(gradient_stats) = self.pgn.get_gradient_statistics() {
            self.average_gradient_norm =
                self.average_gradient_norm * 0.99 + gradient_stats.gradient_norm * 0.01;
        }
        ...
        if !self.stabilization_events.is_empty() {
            let recent_events: Vec<&StabilizationEvent> =
                self.stabilization_events.iter().rev().take(100).collect();

            let pgn_applied_ratio = recent_events.iter().filter(|e| e.pgn_applied).count() as f32
                / recent_events.len() as f32;

            let rec_applied_ratio = recent_events.iter().filter(|e| e.rec_applied).count() as f32
                / recent_events.len() as f32;
...
mod tests {
    ...
    fn test_training_stabilizer_creation() {
        let stabilizer =
            TrainingStabilizer::new(vec![10, 10], (0.01, 0.001, 0.1), (10.0, 0.99, true)).unwrap();
