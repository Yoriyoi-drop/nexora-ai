...
pub fn try_matmul_int2(
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        // Dequantize Q2 → FP32 on CPU (fast, once per call)
        let w_fp32 = dequant_q2_to_fp32(b_packed, scales, n, k, group_size);
    ...
    for col in 0..n {
        for g in 0..groups {
            ...
            for kk in (g_start..g_end).step_by(4) {
                let byte_idx = (kk / 4) * n + col;
                let (v0, v1, v2, v3) = if byte_idx < packed.len() {
                    crate::quantization::gemm::unpack_q2(packed[byte_idx])
                } else {
                    (0, 0, 0, 0)
                };
                out[kk * n + col] = (v0 as f32 - 1.5) * scale;
                if kk + 1 < k {
                    out[(kk + 1) * n + col] = (v1 as f32 - 1.5) * scale;
                }
                if kk + 2 < k {
                    out[(kk + 2) * n + col] = (v2 as f32 - 1.5) * scale;
                }
                if kk + 3 < k {
                    out[(kk + 3) * n + col] = (v3 as f32 - 1.5) * scale;
                }
            }
        }
    }
        ...
        for g in 0..groups {
            ...
            for kk in (g_start..g_end).step_by(2) {
                ...
                let (b0, b1) = if byte_idx < packed.len() {
                    crate::quantization::gemm::unpack_q4(packed[byte_idx])
                } else {
                    (0, 0)
                };
