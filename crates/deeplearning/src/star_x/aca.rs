...
impl AdaptiveComputeAllocation {
    ...
    fn softmax_array(&self, input: &ArrayD<f32>) -> DLResult<ArrayD<f32>> {
        ...
        // Compute exp and sum
        let exp_vals: Vec<f32> = input_flat.iter().map(|&x| (x - max_val).exp()).collect();
        let sum_exp: f32 = exp_vals.iter().sum();

        // Normalize
        let softmax_vals: Vec<f32> = exp_vals.iter().map(|&e| e / sum_exp).collect();
...
impl AdaptiveCompute for AdaptiveComputeAllocation {
    fn determine_compute_level(
        ...
        let level = match self.routing_strategy {
            RoutingStrategy::Threshold => self.threshold_routing(input)?,
            RoutingStrategy::Adaptive => self.adaptive_routing(input, hidden_state)?,
            RoutingStrategy::Complexity => self.complexity_routing(input)?,
            RoutingStrategy::Importance => self.importance_routing(input, hidden_state)?,
            RoutingStrategy::Hybrid => self.hybrid_routing(input, hidden_state)?,
        };
    ...
    fn get_compute_stats(&self) -> (f32, f32, f32) {
        (
            self.compute_efficiency,
            self.get_compute_cost(ComputeLevel::Full),
            1.0 - self.compute_efficiency,
        )
    }
...
impl AdaptiveComputeAllocation {
    ...
    pub fn memory_constrained_allocation(
        &self,
        inputs: &[ArrayD<f32>],
        hidden_states: &[ArrayD<f32>],
    ...
    pub fn update_routing_with_feedback(
        &mut self,
    ...
    pub fn get_allocation_recommendations(&self) -> Vec<String> {
        ...
        if self.compute_efficiency < 0.5 {
            recommendations
                .push("Consider lowering compute thresholds to improve efficiency".to_string());
        }

        if self.compute_efficiency > 0.9 {
            recommendations
                .push("Consider raising compute thresholds to utilize more capacity".to_string());
        }
        ...
        if micro_ratio > 0.8 {
            recommendations.push(
                "Most inputs use micro compute - consider increasing model capacity".to_string(),
            );
        }
        ...
        if full_ratio > 0.5 {
            recommendations
                .push("High full compute usage - consider model optimization".to_string());
        }
