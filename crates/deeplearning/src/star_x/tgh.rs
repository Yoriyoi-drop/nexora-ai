...
impl TemporalGatingHierarchy {
    ...
    fn update_fusion_weights(&mut self, alpha: f32, beta: f32, gamma: f32) {
        ...
        let softmax_weights: Vec<f32> = weights.iter().map(|&w| w.exp()).collect();

        let sum: f32 = softmax_weights.iter().sum();
        let normalized: Vec<f32> = softmax_weights.iter().map(|&w| w / sum).collect();
...
impl HierarchicalGating for TemporalGatingHierarchy {
    ...
    fn fuse_hierarchical(
        ...
        for i in 0..fused_flat.len() {
            fused_flat[i] =
                alpha_norm * micro_flat[i] + beta_norm * meso_flat[i] + gamma_norm * macro_flat[i];
        }
...
impl crate::star_x::traits::Forward for TemporalGatingHierarchy {
