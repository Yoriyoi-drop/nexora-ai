...
use crate::star_x::core::{core_utils, SparseAttention};
use crate::star_x::fused_ops::{ElementWiseOp, FusedAttentionSoftmax, FusedElementWise};
...
impl SparseCausalAttention {
    ...
    fn matmul_gpu_keep_gpu(
        &self,
        weights_gpu: &crate::autograd::gpu::GpuTensor,
    ...
    fn dynamic_sparse_routing(&self, scores: &[f32], k: usize) -> DLResult<Vec<usize>> {
        ...
        let selected_indices: Vec<usize> =
            indexed_scores.iter().take(k).map(|(idx, _)| *idx).collect();

        Ok(selected_indices)
    }

    /// Softmax dengan entropy regularization
    fn softmax_with_entropy(
        &self,
        scores: &[f32],
        selected_indices: &[usize],
    ) -> DLResult<(Vec<f32>, f32)> {
        // Extract selected scores
        let selected_scores: Vec<f32> = selected_indices.iter().map(|&i| scores[i]).collect();

        // Compute softmax
        let max_score = selected_scores
            .iter()
        ...
        let softmax_probs: Vec<f32> = exp_scores.iter().map(|&e| e / sum_exp).collect();

        // Compute entropy
        let entropy = core_utils::compute_entropy(&softmax_probs);
    ...
    fn adaptive_k_selection(&self, sequence_length: usize, base_entropy: f32) -> usize {
        ...
        let adaptive_k = if adaptive_k_f.is_nan() {
            1
        } else {
            adaptive_k_f.max(0.0) as usize
        };
...
impl SparseAttention for SparseCausalAttention {
    fn compute_sparse_attention(
            ...
            .enumerate()
        {
            ...
            let (attention_weights, entropy) =
                self.softmax_with_entropy(&scores, &selected_indices)?;
            all_entropies.push(entropy);

            // Apply attention
            let head_output =
                self.apply_attention(&attention_weights, &selected_indices, &[v_head.clone()])?;
        ...
        self.update_sparsity_stats(
            total_connections,
            self.num_heads * self.max_sparse_connections,
        );
    ...
    fn set_sparsity_level(&mut self, ratio: f32) -> DLResult<()> {
        ...
        let target_connections = if target_connections_f.is_nan() {
            1
        } else {
            target_connections_f.max(0.0) as usize
        };
...
impl SparseCausalAttention {
    ...
    pub fn forward_cached(&mut self, input: &ArrayD<f32>) -> DLResult<ArrayD<f32>> {
        ...
        let attention_output = if let Some(cache) = &mut self.kv_cache {
            // Add to cache
            cache.append(
                require_contiguous(key.as_slice())?.to_vec().into(),
                require_contiguous(value.as_slice())?.to_vec().into(),
            )?;

            // Compute attention with cache
            cache
                .compute_attention(&require_contiguous(query.as_slice())?.to_vec().into())?
                .into_dyn()
            ...
            self.compute_sparse_attention(
                input,
                &ArrayD::zeros(vec![0]),
                &ArrayD::zeros(vec![0]),
                &Array1::zeros(0).into_dyn(),
            )?
            .0
        };
...
impl crate::star_x::traits::Forward for SparseCausalAttention {
