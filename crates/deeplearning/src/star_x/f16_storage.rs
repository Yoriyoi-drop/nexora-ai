...
use crate::star_x::{F16Storage, StoragePrecision};
use ndarray::ArrayD;
...
impl MixedPrecisionTensor {
    ...
    pub fn to_f32(&self) -> ArrayD<f32> {
        match (&self.f32_data, &self.f16_data) {
            ...
            (None, Some(f16)) => crate::star_x::f16_storage_to_f32_tensor(f16, self.shape.clone()),
    ...
    pub fn convert_to_f16(&mut self) {
        if let Some(f32) = self.f32_data.take() {
            let storage = crate::star_x::f32_tensor_to_f16_storage(&f32);
    ...
    pub fn convert_to_f32(&mut self) {
        if let Some(f16) = self.f16_data.take() {
            let f32 = crate::star_x::f16_storage_to_f32_tensor(&f16, self.shape.clone());
