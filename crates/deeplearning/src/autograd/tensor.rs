...
impl Tensor {
    pub fn new(data: ArrayD<f32>) -> Self {
        ...
        if is_gpu_auto_create() {
            if let Ok(_ctx) = crate::autograd::gpu::GpuContext::global() {
                if let Ok(gpu_t) = crate::autograd::gpu::GpuTensor::from_cpu(&data) {
        ...
        if is_gpu_auto_create() {
            if let Ok(rt) = crate::autograd::gpu::CudaRuntime::init(0) {
                let flat: Vec<f32> = data.iter().copied().collect();
                match rt.stream.clone_htod(&flat) {
                    Ok(buf) => {
                        let cuda_t = crate::autograd::gpu::cuda::CudaTensor {
    ...
    #[cfg(feature = "device-cuda")]
    pub fn from_cuda(
        cuda_tensor: crate::autograd::gpu::cuda::CudaTensor,
        id: usize,
        requires_grad: bool,
    ) -> Self {
    ...
    pub(crate) fn from_cuda_with_grad_fn(
        ...
        let device_id = cuda_tensor.device_id;
        let shape = cuda_tensor.shape.clone();
        let grad_fn_idx = crate::autograd::tape::register_grad_fn(inputs, saved, backward);
        let id = TENSOR_COUNTER.fetch_add(1, Ordering::SeqCst);
        Self(Arc::new(RwLock::new(TensorInner {
            id,
            #[cfg(feature = "device-cuda")]
    ...
    pub(crate) fn from_gpu_with_grad_fn(
        ...
        inputs: Vec<Tensor>,
        saved: Vec<ArrayD<f32>>,
        saved_gpu: Vec<crate::autograd::gpu::GpuTensor>,
        backward: Box<dyn FnOnce(&ArrayD<f32>, &[ArrayD<f32>]) -> Vec<ArrayD<f32>>>,
        ...
        match &mut inner.grad {
            ...
            Some(Storage::Gpu(ref mut gpu, _)) => {
                if let Ok(ctx) = crate::autograd::gpu::GpuContext::global() {
                    match crate::autograd::gpu::GpuTensor::from_cpu(grad) {
                        Ok(g_gpu) => {
    ...
    #[cfg(feature = "device-gpu")]
    pub fn zero_grad_gpu(&self) -> Result<(), crate::autograd::gpu::GpuError> {
