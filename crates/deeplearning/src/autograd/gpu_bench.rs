...
pub fn benchmark_gradient_clip(
    ...
    let ctx = match crate::autograd::gpu::GpuContext::global() {
        Ok(c) => c,
    ...
    for _ in 0..grad_tensor_count {
        ...
        grad_tensors.push(
            crate::autograd::gpu::GpuTensor::from_cpu(&arr)
                .map_err(|e| format!("benchmark gpu transfer error: {e}"))?,
        );
    }
    let grad_refs: Vec<&crate::autograd::gpu::GpuTensor> = grad_tensors.iter().collect();
    ...
    for _ in 0..ITERS {
        let _ = crate::autograd::gpu_grad_clip::clip_gradients_batched(ctx, &grad_refs, 10.0);
    }
