                            ...
                            Ok(gpu_result_2d) => {
                                ...
                                return Tensor::from_gpu_with_grad_fn(
                                    ...
                                    Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                        let da = crate::autograd::gpu_backward::softmax_backward_last_dim(
                                            ctx,
                                            &saved_gpu[0],
                                            grad_gpu,
                                        )
    ...
    #[cfg(feature = "device-gpu")]
    {
        ...
        if let Storage::Gpu(gpu_input, _) = &storage {
            if let Ok(ctx) = crate::autograd::gpu::GpuContext::global() {
                ...
                let seed = rand::random::<u32>();
                if let Ok(mask) = crate::autograd::gpu::GpuTensor::zeros(&shape) {
                            ...
                            match ctx.layer_norm(gpu_in, gpu_w, gpu_b, eps) {
                                Ok(gpu_result) => {
                                    ...
                                    return Tensor::from_gpu_with_grad_fn(
                                        ...
                                        Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                            use crate::autograd::gpu_backward::layer_norm_backward_gpu;
                    ...
                    Ok(gpu_result) => {
                        ...
                        return Tensor::from_gpu_with_grad_fn(
                            ...
                            Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                ...
                                let t = &saved_gpu[1];
                                let shape = x.shape();
                                let eps = crate::autograd::gpu::GpuTensor::from_cpu(
                                    &ndarray::ArrayD::from_elem(shape.to_vec(), 1e-7),
                                )
                                .map_err(|e| format!("BCE backward: eps from_cpu failed: {e}"))?;
                                let one_minus_eps = crate::autograd::gpu::GpuTensor::from_cpu(
                                ...
                                let ones = crate::autograd::gpu::GpuTensor::ones(&shape)
                                ...
                                let eps2 = crate::autograd::gpu::GpuTensor::from_cpu(
                ...
                match ctx.cross_entropy(gpu_in, gpu_t) {
                    Ok(gpu_result) => {
                        ...
                        return Tensor::from_gpu_with_grad_fn(
                            ...
                            Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                use crate::autograd::gpu_backward::cross_entropy_backward_gpu;
                ...
                match ctx.embedding(gpu_ids, gpu_w) {
                    Ok(gpu_result) => {
                        ...
                        return Tensor::from_gpu_with_grad_fn(
                            ...
                            Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                use crate::autograd::gpu_backward::embedding_backward_gpu;
                ...
                match ctx.rms_norm(gpu_in, gpu_w, eps) {
                    Ok(gpu_result) => {
                        ...
                        return Tensor::from_gpu_with_grad_fn(
                            ...
                            Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                use crate::autograd::gpu_backward::rms_norm_backward_gpu;
                                let input_gpu = &saved_gpu[0];
                                let weight_gpu = &saved_gpu[1];
                    ...
                    match ctx.fused_attention(gq, gk, gv, scale, true) {
                        Ok(gpu_result) => {
                            ...
                            return Tensor::from_gpu_with_grad_fn(
                                ...
                                Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                    use crate::autograd::gpu_backward::attention_backward_gpu;
                                    let gq = &saved_gpu[0];
                                    let gk = &saved_gpu[1];
                                    let gv = &saved_gpu[2];
                    ...
                    Ok(gpu_out) => {
                        ...
                        if !requires_grad {
                            return Tensor::from_gpu(
                                gpu_out,
                                crate::autograd::tensor::next_tensor_id(),
                                false,
                            );
                        }
