...
macro_rules! gen_unary_op {
    (
        ...
        pub fn $op_name(input: &$crate::autograd::tensor::Tensor) -> $crate::autograd::tensor::Tensor {
                        ...
                        match ctx.$cuda_fn(cu_input) {
                            Ok(cuda_result) => {
                                ...
                                if !requires_grad {
                                    ...
                                    return $crate::autograd::tensor::Tensor::from_cuda(cuda_result, id, false);
                                }
                                let input_data = input.data();
                                let result_data = input_data.mapv($cpu_forward);
                                let saved = $cpu_saved(&input_data, &result_data);
                                return $crate::autograd::tensor::Tensor::from_cuda_with_grad_fn(
                                    cuda_result,
                                    vec![input.clone()],
                                    saved,
                                    Box::new($backward),
                            ...
                            Err(e) => {
                                $crate::autograd::ops::GPU_MATH_FALLBACKS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            ...
            {
                use $crate::autograd::gpu::{ElemOp, GpuContext, GpuError};
                        ...
                        if let Ok(ctx) = GpuContext::global() {
                            match ctx.elementwise_unary(gpu_input, ElemOp::$elem_op) {
                                Ok(gpu_result) => {
                                    ...
                                    if !requires_grad {
                                        let id = $crate::autograd::tensor::next_tensor_id();
                                        return $crate::autograd::tensor::Tensor::from_gpu(gpu_result, id, false);
                                    }
                                    let input_data = input.data();
                                    let result_data = input_data.mapv($cpu_forward);
                                    let saved = $cpu_saved(&input_data, &result_data);
                                    let gpu_bwd = $gpu_backward;
                                    return $crate::autograd::tensor::Tensor::from_gpu_with_grad_fn(
                                        gpu_result,
                                        vec![input.clone()],
                                        saved,
                                        vec![gpu_input.clone()],
                                ...
                                Err(e) => {
                                    ...
                                    tracing::warn!(
                                        ...
                                        $crate::autograd::ops::GPU_MATH_FALLBACKS.load(std::sync::atomic::Ordering::Relaxed)
                                    );
                                }
                            }
                        }
            ...
            if !input.requires_grad() {
                return $crate::autograd::tensor::Tensor::new(result);
            }
            let saved = $cpu_saved(&data, &result);
            $crate::autograd::tensor::Tensor::with_grad_fn(
                result,
                vec![input.clone()],
                saved,
                Box::new($backward),
...
macro_rules! gen_binary_op {
    (
        ...
        pub fn $op_name(a: &$crate::autograd::tensor::Tensor, b: &$crate::autograd::tensor::Tensor) -> $crate::autograd::tensor::Tensor {
            ...
            #[cfg(feature = "device-cuda")]
            {
                ...
                if let ($crate::autograd::device::Storage::Cuda(ca, _), $crate::autograd::device::Storage::Cuda(cb, _)) = (&a_storage, &b_storage) {
                    if let Ok(ctx) = $crate::autograd::gpu::CudaRuntime::global() {
                        match ctx.$cuda_fn(ca, cb) {
                            Ok(cuda_result) => {
                                ...
                                let a_data = a.data();
                                let b_data = b.data();
                                let (a_bc, b_bc, _) = $crate::autograd::broadcast::broadcast_arrays(&a_data, &b_data);
            ...
            #[cfg(feature = "device-gpu")]
            {
                use $crate::autograd::gpu::{GpuContext, GpuError};
                use $crate::autograd::gpu::gpu_recovery::RECOVERY_MANAGER;
                let a_storage = a.storage();
                let b_storage = b.storage();
                let on_gpu = matches!(&a_storage, $crate::autograd::device::Storage::Gpu(..))
                    && matches!(&b_storage, $crate::autograd::device::Storage::Gpu(..));
                if on_gpu && !RECOVERY_MANAGER.is_circuit_open() {
                    if let ($crate::autograd::device::Storage::Gpu(ga, _), $crate::autograd::device::Storage::Gpu(gb, _)) = (&a_storage, &b_storage) {
                        if let Ok(ctx) = GpuContext::global() {
...
macro_rules! gen_activation_op {
    (
        ...
        pub fn $op_name(input: &$crate::autograd::tensor::Tensor) -> $crate::autograd::tensor::Tensor {
            #[cfg(feature = "device-gpu")]
            {
                use $crate::autograd::gpu::{ElemOp, GpuContext};
