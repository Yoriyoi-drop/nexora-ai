...
use crate::gnac::execution::backend::CpuBackend;
use crate::gnac::execution::optimizer::GraphOptimizer;
use crate::gnac::execution::{ExecutionBackend, GraphIR, IROpType};
    ...
    fn execute_gpu(
        ...
        for (op_idx, op) in ir.operations.iter().enumerate() {
            let get_gpu = |name: &str| -> DLResult<&GpuTensor> {
                gpu_tensors
                    .get(name)
                    .ok_or_else(|| crate::gnac::DeepLearningError::Computation {
...
mod tests {
    ...
    use crate::gnac::execution::{IROpType, IRValue};
