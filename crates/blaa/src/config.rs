...
use super::{defaults, BlaaError, BlaaResult};
...
impl BlaaConfig {
    ...
    pub fn from_env() -> BlaaResult<Self> {
        let api_key = std::env::var("BLAA_API_KEY").map_err(|_| {
            BlaaError::InvalidConfiguration("BLAA_API_KEY environment variable not set".to_string())
        })?;
        ...
        if let Ok(timeout) = std::env::var("BLAA_TIMEOUT_SECS") {
            config.timeout_secs = timeout.parse().map_err(|_| {
                BlaaError::InvalidConfiguration("Invalid BLAA_TIMEOUT_SECS value".to_string())
            })?;
        }

        if let Ok(max_retries) = std::env::var("BLAA_MAX_RETRIES") {
            config.max_retries = max_retries.parse().map_err(|_| {
                BlaaError::InvalidConfiguration("Invalid BLAA_MAX_RETRIES value".to_string())
            })?;
        }

        if let Ok(rate_limit) = std::env::var("BLAA_RATE_LIMIT_RPS") {
            config.rate_limit_rps = rate_limit.parse().map_err(|_| {
                BlaaError::InvalidConfiguration("Invalid BLAA_RATE_LIMIT_RPS value".to_string())
            })?;
        }
