...
impl BlaaClient {
    ...
    ) -> BlaaResult<ChatCompletionResponse> {
    ...
    pub async fn create_chat_completion_stream(
        &mut self,
        request: ChatCompletionRequest,
    ) -> BlaaResult<impl Stream<Item = BlaaResult<ChatCompletionChunk>>> {
        let mut streaming_request = request.clone();
        streaming_request.stream = Some(true);

        let endpoint = self.config.endpoint_url("chat/completions");
        let response = self.post(&endpoint, &streaming_request).await?;
    ...
    async fn execute_request(&self, request: RequestBuilder) -> BlaaResult<Response> {
            ...
            match timeout(self.config.timeout_duration(), request_clone.send()).await {
                Ok(Ok(response)) => {
                    ...
                    if status.is_success() {
                        ...
                        let error = BlaaError::ApiRequest(format!("Unexpected status: {}", status));
                        return Err(error);
                    }
                }
                Ok(Err(e)) => {
        ...
        Err(last_error
            .unwrap_or_else(|| BlaaError::ApiRequest("All retry attempts failed".to_string())))
    }

    /// Parse API error response
    fn parse_api_error(&self, error_text: &str) -> BlaaError {
        if error_text.len() > MAX_ERROR_RESPONSE_SIZE {
            return BlaaError::ApiRequest(format!(
                "API error response too large: {} bytes",
                error_text.len()
            ));
        }
        match serde_json::from_str::<BlaaErrorResponse>(error_text) {
            Ok(error_response) => {
                ...
                match error_info.error_type.as_str() {
                    "authentication_error" => BlaaError::Authentication(error_info.message),
                    "rate_limit_error" => BlaaError::RateLimit(error_info.message),
                    "invalid_request_error" => BlaaError::InvalidConfiguration(error_info.message),
                    _ => BlaaError::ApiRequest(format!("API error: {}", error_info.message)),
                }
            }
            Err(_) => {
            ...
            match self.stream.as_mut().poll_next(cx) {
                Poll::Ready(Some(Ok(chunk))) => {
                    ...
                    if let Some(line_end) = self.buffer.iter().position(|&b| b == b'\n') {
                        ...
                        let line = String::from_utf8(line_bytes).map_err(|e| {
                            BlaaError::InvalidResponse(format!(
                ...
                Poll::Ready(None) => {
                    if !self.buffer.is_empty() {
                        ...
                        let line = String::from_utf8(remaining).map_err(|e| {
                            BlaaError::InvalidResponse(format!(
                                "Invalid UTF-8 in SSE stream: {}",
                                e
                            ))
                        ...
                        if let Some(data) = line.strip_prefix("data: ") {
                            ...
                            match serde_json::from_str::<ChatCompletionChunk>(data.trim()) {
                                Ok(chunk) => return Poll::Ready(Some(Ok(chunk))),
                                Err(_) => {
                                    debug!("Skipping non-chunk SSE line in buffer: {}", data.trim())
                                }
                            }
                        }
                    }
