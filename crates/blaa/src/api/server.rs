...
impl ApiServer {
    ...
    pub async fn new(config: ApiConfig) -> Result<Self> {
        ...
        let _ = crate::api::api_validate_tensor(&[1]);
        let _db = crate::api::api_db();
        let _monitoring = crate::api::api_monitoring();

        let middleware = if config.enable_logging || config.enable_metrics || config.enable_cors {
            let mw = crate::api::create_default_middleware_stack().await;
            Arc::new(mw)
        } else {
