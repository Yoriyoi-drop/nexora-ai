...
use crate::{GeneratedToken, InferenceError, InferenceRequest, Result};
...
impl StreamingEngine {
        ...
        // Check engine state
        {
            ...
            if *state != EngineState::Ready {
                return Err(InferenceError::InternalError(
                    "Streaming engine not ready".to_string(),
                )
                .into());
            }
        }
    ...
    pub async fn submit_request(
        ...
        if !is_streaming {
            return Err(
                InferenceError::InternalError("Request is not for streaming".to_string()).into(),
            );
        }
    ...
    pub async fn send_token(
        ...
        if let Some(stream_info) = streams.get_mut(&stream_id) {
            if stream_info.finished {
                return Err(InferenceError::InternalError(
                    "Stream is already finished".to_string(),
                )
                .into());
            }
            ...
            if let Some(max_rate) = stream_info.config.max_tokens_per_second {
                if !self.check_rate_limit(stream_info, max_rate).await? {
                    return Err(
                        InferenceError::InternalError("Rate limit exceeded".to_string()).into(),
                    );
                }
            }
            ...
            if let Err(_) = stream_info.token_tx.send(stream_token).await {
                ...
                return Err(
                    InferenceError::InternalError("Stream receiver dropped".to_string()).into(),
                );
            }
    ...
    async fn process_stream(
        ...
        simple_tx: mpsc::Sender<GeneratedToken>,
    ) {
    ...
    async fn cleanup_expired_streams(&self) -> Result<()> {
        ...
        let expired_streams = {
            ...
            for (stream_id, stream_info) in streams.iter() {
                ...
                {
                    // 5 minutes idle for finished streams
