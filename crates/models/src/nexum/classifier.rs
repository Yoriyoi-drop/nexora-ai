use crate::classifier_util;
use ndarray::{Array1, Array2};
use std::sync::OnceLock;

pub const COMPLEXITY_LEVELS: [&str; 4] = [
    "simple",
    "moderate",
    "complex",
    "multi_domain",
];

const HIDDEN: usize = 32;

static CLASSIFIER: OnceLock<ComplexityClassifier> = OnceLock::new();

pub struct ComplexityClassifier {
    embed_table: Array2<f32>,
    w1: Array2<f32>,
    b1: Array1<f32>,
    w2: Array2<f32>,
    b2: Array1<f32>,
}

impl ComplexityClassifier {
    pub fn global() -> &'static Self {
        CLASSIFIER.get_or_init(|| Self {
            embed_table: Array2::zeros((1, 1)),
            w1: Array2::zeros((1, 1)),
            b1: Array1::zeros(1),
            w2: Array2::zeros((1, 1)),
            b2: Array1::zeros(1),
        })
    }

    pub fn init(embed_table: Array2<f32>) {
        let hidden_size = embed_table.shape()[1];
        let _ = CLASSIFIER.set(Self {
            w1: classifier_util::xavier_init(hidden_size, HIDDEN),
            b1: Array1::zeros(HIDDEN),
            w2: classifier_util::xavier_init(HIDDEN, COMPLEXITY_LEVELS.len()),
            b2: Array1::zeros(COMPLEXITY_LEVELS.len()),
            embed_table,
        });
    }

    pub fn is_initialized(&self) -> bool {
        self.embed_table.shape()[0] > 1
    }

    pub fn predict(&self, token_ids: &[u32]) -> Vec<(String, f32)> {
        if token_ids.is_empty() || !self.is_initialized() {
            return vec![("simple".to_string(), 1.0)];
        }

        let avg = classifier_util::embed_average(&self.embed_table, token_ids);
        let h = classifier_util::gelu(&(avg.dot(&self.w1) + &self.b1));
        let logits = h.dot(&self.w2) + &self.b2;
        let probs = classifier_util::softmax(&logits);

        let mut results: Vec<_> = COMPLEXITY_LEVELS
            .iter()
            .zip(probs.iter())
            .map(|(c, p)| ((*c).to_string(), *p))
            .collect();
        results.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        results
    }
}

pub fn decomposition_strategy(level: &str) -> &'static str {
    match level {
        "simple" => "Single direct response — no decomposition needed.",
        "moderate" => "Split into 2-3 logical subtasks, solve sequentially.",
        "complex" => "Decompose into 3-5 subtasks with dependencies. Solve each with reasoning chain.",
        "multi_domain" => "Decompose by domain expertise. Each subtask assigned to domain expert. Synthesize results.",
        _ => "Split into manageable subtasks and solve independently.",
    }
}

pub fn detect_complexity(text: &str, token_ids: &[u32]) -> Vec<(String, f32)> {
    let classifier = CLASSIFIER.get().unwrap_or_else(ComplexityClassifier::global);
    if token_ids.is_empty() || !classifier.is_initialized() {
        return vec![("simple".to_string(), 1.0)];
    }
    classifier.predict(token_ids)
}

#[cfg(test)]
mod tests {
    use super::*;

    fn uninit() -> ComplexityClassifier {
        ComplexityClassifier {
            embed_table: Array2::zeros((1, 1)),
            w1: Array2::zeros((1, 1)),
            b1: Array1::zeros(1),
            w2: Array2::zeros((1, 1)),
            b2: Array1::zeros(1),
        }
    }

    fn init_cls(hidden: usize) -> ComplexityClassifier {
        ComplexityClassifier {
            embed_table: Array2::zeros((10, hidden)),
            w1: classifier_util::xavier_init(hidden, HIDDEN),
            b1: Array1::zeros(HIDDEN),
            w2: classifier_util::xavier_init(HIDDEN, COMPLEXITY_LEVELS.len()),
            b2: Array1::zeros(COMPLEXITY_LEVELS.len()),
        }
    }

    #[test]
    fn test_global_not_initialized() {
        assert!(!ComplexityClassifier::global().is_initialized());
    }

    #[test]
    fn test_uninit_not_initialized() {
        assert!(!uninit().is_initialized());
    }

    #[test]
    fn test_init_initialized() {
        assert!(init_cls(512).is_initialized());
    }

    #[test]
    fn test_predict_default_on_uninit() {
        assert_eq!(uninit().predict(&[1])[0].0, "simple");
    }

    #[test]
    fn test_predict_empty_ids() {
        assert_eq!(init_cls(512).predict(&[])[0].0, "simple");
    }

    #[test]
    fn test_predict_returns_all_levels() {
        let r = init_cls(512).predict(&[0, 1]);
        assert_eq!(r.len(), COMPLEXITY_LEVELS.len());
        let sum: f32 = r.iter().map(|(_, p)| p).sum();
        assert!((sum - 1.0).abs() < 1e-5);
    }

    #[test]
    fn test_detect_default_on_uninit() {
        assert_eq!(detect_complexity("x", &[])[0].0, "simple");
    }
}
