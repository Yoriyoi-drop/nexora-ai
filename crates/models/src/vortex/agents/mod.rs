...
fn detect_language(code: &str) -> String {
    let extensions = vec![
        ("rust", vec!["fn ", "let ", "impl ", "struct ", "enum ", "pub ", "use ", "mod ", "mut ", "crate"]),
        ("python", vec!["def", "import", "class", "async def", "yield", "if __name__", "lambda", "raise ", "except"]),
        ("typescript", vec!["interface", "type", "const", "let", "function", "=>", "export", "import", "as", "<T>", ": string", ": number"]),
        ("javascript", vec!["function", "const", "let", "var", "=>", "export", "import", "require(", "console.log", "prototype", "this."]),
        ("go", vec!["func", "package", "import (", "defer", "go", "han", "goroutine", "interface{}"]),
        ("java", vec!["public class", "private", "protected", "static", "void main", "extends", "implements", "@Override"]),
        ("cpp", vec!["#include", "int main", "std::", "::", "template<", "class", "virtual", "override"]),
        ("ruby", vec!["def ", "end", "require ", "class ", "module ", "attr_", "gem ", "do |"]),
    ];
