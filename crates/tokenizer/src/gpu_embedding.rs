...
pub struct GpuEmbedding {
    weight: Option<nexora_autograd::gpu::GpuTensor>,
    vocab_size: usize,
    embed_dim: usize,
}
...
impl GpuEmbedding {
    ...
    pub fn lookup(&self, token_ids: &[u32]) -> Option<Vec<f32>> {
        let ctx = nexora_autograd::gpu::GpuContext::global().ok()?;
        let weight = self.weight.as_ref()?;

        let ids_f32: Vec<f32> = token_ids.iter().map(|&id| id as f32).collect();
        let ids_tensor = match nexora_autograd::gpu::GpuTensor::from_slice(
            vec![token_ids.len()],
            &ids_f32,
        ) {
            Ok(t) => t,
    ...
    pub fn update_weights(&mut self, cpu_weights: &ndarray::Array2<f32>) {
        ...
        match nexora_autograd::gpu::GpuTensor::from_cpu(&cpu_weights.clone().into_dyn()) {
            Ok(t) => {
                self.weight = Some(t);
    ...
    pub fn reset_weights(&mut self) {
        ...
        if let Ok(t) = nexora_autograd::gpu::GpuTensor::from_cpu(&arr.into_dyn()) {
            self.weight = Some(t);
        }
    }
}

fn try_init_weight(vocab_size: usize, embed_dim: usize) -> Option<nexora_autograd::gpu::GpuTensor> {
    let _ = nexora_autograd::gpu::GpuContext::init().ok()?;
    let ctx = nexora_autograd::gpu::GpuContext::global().ok()?;
    let arr = ndarray::Array2::<f32>::zeros((vocab_size, embed_dim));
    match nexora_autograd::gpu::GpuTensor::from_cpu(&arr.into_dyn()) {
        Ok(t) => {
            ctx.flush();
            Some(t)
        }
