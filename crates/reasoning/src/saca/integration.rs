...
use nexora_multimodal::caffeine::Caffeine;
use std::sync::Arc;
...
impl SACAIntegration {
    ...
    pub async fn solve_with_models(&self, task: CodingTask) -> SACAResult<EnhancedSACASolution> {
        ...
        let enhanced_solution = self
            .enhance_solution_with_models(base_solution, &task)
            .await?;

        info!(
            "Enhanced SACA solution completed. Quality: {:.3}",
            enhanced_solution.base_solution.quality_score
        );
    ...
    async fn enhance_solution_with_models(
        ...
        if let Some(ref compression) = self.atqs_compression {
            enhanced_solution = self
                .apply_atqs_compression(enhanced_solution, compression)
                .await?;
        }
        ...
        if let Some(ref caffeine) = self.caffeine_model {
            enhanced_solution = self
                .apply_caffeine_enhancement(enhanced_solution, caffeine, task)
                .await?;
        }
        ...
        if let Some(ref router) = self.has_moe_router {
            enhanced_solution = self
                .apply_has_moe_routing(enhanced_solution, router)
                .await?;
        }
    ...
    async fn apply_atqs_compression(
        ...
        let compressed_code = compression
            .compress_string(&solution.base_solution.final_code)
            .map_err(|e| SACAError::ContextError(format!("Compression failed: {}", e)))?;
        let compression_ratio =
            solution.base_solution.final_code.len() as f64 / compressed_code.len() as f64;
        ...
        debug!(
            "ATQS compression applied with ratio: {:.2}",
            compression_ratio
        );
        Ok(solution)
    }
    ...
    async fn apply_caffeine_enhancement(
        ...
        let multimodal_inputs = nexora_multimodal::caffeine::types::MultiModalInputs {
            text: Some(nexora_multimodal::caffeine::types::TextInput {
            ...
            audio: None,
            video: None,
            context: Some(nexora_multimodal::caffeine::types::ContextInfo {
                task_type: nexora_multimodal::caffeine::types::TaskType::Generation,
        ...
        let multimodal_outputs = caffeine
            .forward(&multimodal_inputs)
            .await
        ...
        if let Some(text_output) = &multimodal_outputs.text {
            ...
            solution.multimodal_features = vec![
                ...
                text_output
                    .token_probs
                    .as_ref()
                    .map_or(0.5, |probs| probs.iter().sum::<f32>() / probs.len() as f32),
            ];
        } else {
            solution.multimodal_features = vec![];
        }
        ...
        debug!(
            "Caffeine enhancement applied with {} features",
            solution.multimodal_features.len()
        );
        Ok(solution)
    }
    ...
    async fn apply_has_moe_routing(
        ...
        debug!(
            "HAS MoE routing applied with efficiency: {:.3}",
...
impl SACAFactory {
    ...
    pub async fn create_full_saca(
        ...
        atqs_config: Option<nexora_atqs::config::ATQSConfig>,
        caffeine_config: Option<nexora_multimodal::caffeine::config::CaffeineConfig>,
        has_moe_config: Option<nexora_has_moe_ffn::HasMoeFFNConfig>,
        ...
        if let Some(atqs_config) = atqs_config {
            let compression = Arc::new(CompressionEngine::new(atqs_config).map_err(|e| {
                SACAError::ContextError(format!("Compression engine creation failed: {}", e))
            })?);
        ...
        if let Some(caffeine_config) = caffeine_config {
            let caffeine = Arc::new(Mutex::new(Caffeine::new(caffeine_config).map_err(|e| {
                SACAError::ContextError(format!("Caffeine creation failed: {}", e))
            })?));
        ...
        if let Some(has_moe_config) = has_moe_config {
            let router = Arc::new(Router::new(
                has_moe_config.hidden_size,
                has_moe_config.num_experts,
                has_moe_config.top_k,
            ));
    ...
    pub async fn create_saca_with_atqs(
        saca_config: SACAConfig,
        atqs_config: nexora_atqs::config::ATQSConfig,
    ) -> SACAResult<SACAIntegration> {
        let compression = Arc::new(CompressionEngine::new(atqs_config).map_err(|e| {
            SACAError::ContextError(format!("Compression engine creation failed: {}", e))
        })?);
        Ok(SACAIntegration::new(saca_config)
            .await?
            .with_atqs_compression(compression))
    }

    /// Create SACA with Caffeine only
    pub async fn create_saca_with_caffeine(
        saca_config: SACAConfig,
        caffeine_config: nexora_multimodal::caffeine::config::CaffeineConfig,
    ) -> SACAResult<SACAIntegration> {
        let caffeine = Arc::new(Mutex::new(Caffeine::new(caffeine_config).map_err(|e| {
            SACAError::ContextError(format!("Caffeine creation failed: {}", e))
        })?));
        Ok(SACAIntegration::new(saca_config)
            .await?
            .with_caffeine(caffeine))
    }

    /// Create SACA with HAS MoE FFN only
    pub async fn create_saca_with_has_moe(
        saca_config: SACAConfig,
        has_moe_config: nexora_has_moe_ffn::HasMoeFFNConfig,
    ) -> SACAResult<SACAIntegration> {
        let router = Arc::new(Router::new(
            has_moe_config.hidden_size,
            has_moe_config.num_experts,
...
mod tests {
    ...
    async fn test_integration_stats() -> anyhow::Result<()> {
        let config = SACAConfig::default();
        let integration = SACAIntegration::new(config)
            .await
    ...
    async fn test_solution_to_tensor() -> anyhow::Result<()> {
        let integration = SACAIntegration::new(SACAConfig::default())
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create integration: {}", e))?;
