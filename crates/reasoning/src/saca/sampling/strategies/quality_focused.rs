...
use super::super::generators::{
    AlgorithmGenerator, AlgorithmType, OptimizedAlgorithmGenerator, StandardAlgorithmGenerator,
};
...
impl QualityFocusedSamplingStrategy {
    ...
    pub async fn sample(
        ...
        while candidates.len() < num_candidates as usize {
            let algorithm_type = quality_algorithm_types
                [rand::random::<usize>() % quality_algorithm_types.len()]
            .clone();
            let generator = if matches!(algorithm_type, AlgorithmType::Optimized) {
                &self.generators[1]
            } else {
