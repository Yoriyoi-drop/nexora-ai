...
use super::super::generators::{
    AlgorithmGenerator, AlgorithmType, HybridAlgorithmGenerator, OptimizedAlgorithmGenerator,
    StandardAlgorithmGenerator,
};
...
impl PerformanceFocusedSamplingStrategy {
    ...
    pub async fn sample(
        ...
        for (i, algorithm_type) in performance_algorithm_types.iter().enumerate() {
            ...
            let mut candidate =
                generator.generate(module, context, cot_result, algorithm_type.clone())?;
        ...
        while candidates.len() < num_candidates as usize {
            let algorithm_type = performance_algorithm_types
                [rand::random::<usize>() % performance_algorithm_types.len()]
            .clone();
