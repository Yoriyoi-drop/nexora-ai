...
impl RerankEngine {
    ...
    pub fn new(config: RerankConfig) -> SACAResult<Self> {
        ...
        info!(
            "Rerank Engine initialized with {:?} normalization",
    ...
    pub async fn rerank(
        ...
        debug!(
            "Starting mathematical reranking for {} candidates",
            executed_candidates.len()
        );
        ...
        if executed_candidates.len() > 1000 {
            warn!(
                "Large number of candidates ({}). Consider filtering first.",
                executed_candidates.len()
            );
        }
        ...
        let scored_candidates = self
            .calculate_candidate_scores_with_fallback(&executed_candidates, context)
            .await?;
        ...
        let best_candidate = sorted_candidates.into_iter().next().ok_or_else(|| {
            SACAError::RerankError(
        ...
        if best_candidate.final_score < 0.0 || best_candidate.final_score > 1.0 {
            warn!(
                "Best candidate has invalid score: {}",
        ...
        let solution = self
            .create_solution_safe(best_candidate, &executed_candidates, context)
            .await?;

        info!(
            "Reranking completed successfully. Best solution score: {:.3}",
    ...
    async fn calculate_candidate_scores_with_fallback(
        ...
        let results: Vec<_> = join_all(executed_candidates.iter().enumerate().map(
            |(idx, execution_result)| async move {
                (
                    idx,
                    self.calculate_candidate_score(execution_result, context)
                        .await,
                )
            },
        ))
        ...
        if failed_count > 0 {
            warn!(
                "Failed to score {} out of {} candidates",
                failed_count,
                executed_candidates.len()
            );
        }
        ...
        if successful_candidates.is_empty() {
            return Err(SACAError::RerankError(format!(
    ...
    async fn normalize_scores_safe(
        &self,
        ...
        for candidate in &candidates {
            for (metric_name, score) in &candidate.metrics {
                if !score.is_finite() {
                    warn!(
                        "Non-finite score detected for metric {}: {}",
                        metric_name, score
                    );
                    continue;
                }
                        ...
                        Ok(normalized_score) => {
                            if normalized_score.is_finite() {
                                *score = normalized_score;
                                ...
                                warn!(
                                    "Non-finite normalized score for metric {}: {}",
    ...
    async fn apply_weighting_robust(
        &self,
        candidates: Vec<ScoredCandidate>,
    ) -> SACAResult<Vec<ScoredCandidate>> {
        ...
        for candidate in &mut weighted_candidates {
            ...
            final_score += candidate.metrics.get("correctness").unwrap_or(&0.0)
                * self.config.criteria.correctness_weight;
            final_score += candidate.metrics.get("performance").unwrap_or(&0.0)
                * self.config.criteria.performance_weight;
            final_score += candidate.metrics.get("readability").unwrap_or(&0.0)
                * self.config.criteria.readability_weight;
            final_score += candidate.metrics.get("maintainability").unwrap_or(&0.0)
                * self.config.criteria.maintainability_weight;
            final_score += candidate.metrics.get("test_coverage").unwrap_or(&0.0)
                * self.config.criteria.test_coverage_weight;
            final_score += candidate.metrics.get("documentation").unwrap_or(&0.0)
                * self.config.criteria.documentation_weight;
    ...
    async fn create_solution_safe(
        ...
        let execution_result = all_executions
            ...
            .ok_or_else(|| {
                SACAError::RerankError(format!(
                    "Execution result not found for candidate {}",
        ...
        let solved_modules = vec![SolvedModule {
            ...
            quality_metrics: ModuleQualityMetrics {
                correctness: best_candidate
                    .metrics
                    .get("correctness")
                    .copied()
                    .unwrap_or(0.0),
                efficiency: best_candidate
                    .metrics
                    .get("performance")
                    .copied()
                    .unwrap_or(0.0),
                readability: best_candidate
                    .metrics
                    .get("readability")
                    .copied()
                    .unwrap_or(0.0),
                maintainability: best_candidate
                    .metrics
                    .get("maintainability")
                    .copied()
                    .unwrap_or(0.0),
                test_coverage,
                documentation_score: best_candidate
                    .metrics
                    .get("documentation")
                    .copied()
                    .unwrap_or(0.0),
        ...
        Ok(SACASolution {
            ...
            execution_time: chrono::Duration::milliseconds(
                execution_result.execution_time_ms as i64,
            ),
    ...
    fn calculate_test_coverage_safe(&self, _execution_result: &SACAExecutionResult) -> f32 {
        ...
        let passed_tests = _execution_result
            .test_results
            .iter()
            .filter(|t| t.passed)
            .count();
...
impl MetricCalculator for CorrectnessCalculator {
    ...
    fn calculate(
        ...
        Ok(BaseMetricCalculator::calculate_pass_rate(
            &_execution_result.test_results,
        ))
    }
}
...
impl MetricCalculator for PerformanceCalculator {
    ...
    fn calculate(
        ...
        let time_score = self
            .thresholds
            .get_time_score(_execution_result.execution_time_ms);

        // Memory usage score
        let memory_score = self
            .thresholds
            .get_memory_score(_execution_result.memory_usage_mb as f32);
...
impl MetricCalculator for DocumentationCalculator {
    ...
    fn calculate(
        ...
        let has_meaningful_errors = _execution_result
            .error_logs
            .iter()
...
impl ScoreNormalizer for ZScoreNormalizer {
    fn normalize(&self, score: f32, all_scores: &[f32]) -> SACAResult<f32> {
        ...
        let variance =
            all_scores.iter().map(|s| (s - mean).powi(2)).sum::<f32>() / all_scores.len() as f32;
...
mod tests {
    ...
    async fn test_rerank_engine() -> Result<(), anyhow::Error> {
        ...
        let solution = engine
            .rerank(vec![execution_result], &context)
            .await
    ...
    async fn test_correctness_calculator() -> Result<(), anyhow::Error> {
        ...
        let score = calculator
            .calculate(&execution_result, &context)
