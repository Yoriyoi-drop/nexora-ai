...
impl CoTEngine {
    ...
    fn score_dimension(text: &str, keywords: &[&str]) -> f32 {
        ...
        let total: usize = keywords.iter().map(|kw| {
            lower.matches(kw).count()
        }).sum();
    ...
    async fn analyze_task(&self, task: &CodingTask) -> SACAResult<String> {
        ...
        let data_score = Self::score_dimension(&desc_lower, &["array", "list", "collection", "data", "stream", "vector", "matrix", "record"]);
        if data_score > 0.3 { dimension_scores.push(("data_processing".to_string(), data_score)); }

        let io_score = Self::score_dimension(&desc_lower, &["read", "write", "parse", "serialize", "format", "deserialize", "encode", "decode"]);
        if io_score > 0.3 { dimension_scores.push(("io".to_string(), io_score)); }

        let net_score = Self::score_dimension(&desc_lower, &["request", "response", "http", "api", "client", "server", "endpoint", "rest"]);
        if net_score > 0.3 { dimension_scores.push(("network".to_string(), net_score)); }

        let state_score = Self::score_dimension(&desc_lower, &["state", "cache", "store", "persist", "memo", "session", "context"]);
        if state_score > 0.3 { dimension_scores.push(("state_management".to_string(), state_score)); }

        let conc_score = Self::score_dimension(&desc_lower, &["thread", "async", "parallel", "concurrent", "sync", "lock", "atomic", "race"]);
        if conc_score > 0.3 { dimension_scores.push(("concurrency".to_string(), conc_score)); }

        let comp_score = Self::score_dimension(&desc_lower, &["compute", "calculate", "transform", "process", "map", "reduce", "aggregate", "evaluate"]);
        if comp_score > 0.3 { dimension_scores.push(("computation".to_string(), comp_score)); }

        let ml_score = Self::score_dimension(&desc_lower, &["train", "model", "predict", "classify", "regression", "neural", "embedding", "feature"]);
        if ml_score > 0.3 { dimension_scores.push(("machine_learning".to_string(), ml_score)); }
        ...
        let dimension_str = if dimension_scores.is_empty() {
            ...
            dimension_scores.iter().map(|(d, s)| format!("{}@{:.2}", d, s)).collect::<Vec<_>>().join(", ")
        };
        ...
        for constraint in &task.constraints {
            ...
            if cl.contains("memory") || cl.contains("resource") {
                analysis.push_str(&format!("\n  - [HIGH] Constraint insight: '{}' requires space-efficient design", constraint));
            }
            if cl.contains("safe") || cl.contains("error") || cl.contains("robust") {
                analysis.push_str(&format!("\n  - [MEDIUM] Constraint insight: '{}' requires defensive programming", constraint));
            }
            if cl.contains("real") || cl.contains("deadline") || cl.contains("time") {
                analysis.push_str(&format!("\n  - [CRITICAL] Constraint insight: '{}' imposes real-time requirements", constraint));
            }
        }
    ...
    async fn identify_components(&self, task: &CodingTask) -> SACAResult<String> {
        ...
        let all_text = {
            ...
            for req in &task.requirements { t.push_str(" "); t.push_str(req); }
            for con in &task.constraints { t.push_str(" "); t.push_str(con); }
            t.to_lowercase()
        };
        ...
        if Self::score_dimension(&desc_lower, &["sort", "order", "rank", "compare", "sorted"]) > 0.3 {
            ...
            components.push(("ComparisonStrategy".to_string(), vec!["InputCollection".to_string()]));
            components.push(("SortingAlgorithm".to_string(), vec!["ComparisonStrategy".to_string(), "InputCollection".to_string()]));
            components.push(("OrderedOutput".to_string(), vec!["SortingAlgorithm".to_string()]));
        }
        // Search operations
        if Self::score_dimension(&desc_lower, &["search", "find", "locate", "query", "lookup"]) > 0.3 {
            components.push(("SearchableCollection".to_string(), vec![]));
            components.push(("SearchPredicate".to_string(), vec![]));
            components.push(("SearchAlgorithm".to_string(), vec!["SearchableCollection".to_string(), "SearchPredicate".to_string()]));
            components.push(("ResultHandler".to_string(), vec!["SearchAlgorithm".to_string()]));
        }
        ...
        if Self::score_dimension(&desc_lower, &["parse", "tokenize", "lex", "grammar"]) > 0.3 {
            ...
            components.push(("ParserStateMachine".to_string(), vec!["Tokenizer".to_string(), "GrammarDefinition".to_string()]));
            components.push(("AbstractSyntaxTree".to_string(), vec!["ParserStateMachine".to_string()]));
        }
        // Transform/convert operations
        if Self::score_dimension(&desc_lower, &["transform", "convert", "encode", "decode", "map"]) > 0.3 {
            components.push(("InputValidator".to_string(), vec![]));
            components.push(("TransformFunction".to_string(), vec!["InputValidator".to_string()]));
            components.push(("OutputEncoder".to_string(), vec!["TransformFunction".to_string()]));
        }
        // Aggregate operations
        if Self::score_dimension(&desc_lower, &["aggregate", "summarize", "group", "reduce"]) > 0.3 {
            components.push(("DataPartitioner".to_string(), vec![]));
            components.push(("AggregationFunction".to_string(), vec!["DataPartitioner".to_string()]));
            components.push(("ResultCombiner".to_string(), vec!["AggregationFunction".to_string()]));
        }
        ...
        if Self::score_dimension(&desc_lower, &["graph", "tree", "node", "edge", "vertex"]) > 0.3 {
            ...
            components.push(("TraversalStrategy".to_string(), vec!["GraphRepresentation".to_string()]));
            components.push(("VisitedTracker".to_string(), vec![]));
            components.push(("PathCollector".to_string(), vec!["TraversalStrategy".to_string()]));
        }
        // ML/Model operations
        if Self::score_dimension(&desc_lower, &["train", "model", "predict", "neural", "classify"]) > 0.3 {
            components.push(("DatasetLoader".to_string(), vec![]));
            components.push(("FeatureExtractor".to_string(), vec!["DatasetLoader".to_string()]));
            components.push(("ModelDefinition".to_string(), vec![]));
            components.push(("TrainingLoop".to_string(), vec!["ModelDefinition".to_string(), "FeatureExtractor".to_string()]));
            components.push(("InferenceEngine".to_string(), vec!["ModelDefinition".to_string()]));
        }

        // If no specific categories matched, derive from requirements with dependency tracking
        if components.is_empty() {
            let deps: Vec<String> = task.requirements.iter().enumerate().map(|(i, r)| {
                format!("ReqComponent_{}(depends on: {:?})", i, 
                    task.requirements.iter().take(i).map(|pr| format!("ReqComponent_{}", task.requirements.iter().position(|x| x == pr).unwrap_or(0))).collect::<Vec<_>>())
            }).collect();
            for d in deps { components.push((d, vec![])); }
            if components.is_empty() {
                ...
                components.push(("OutputFormatting".to_string(), vec!["CoreLogic".to_string()]));
            }
        }
        ...
        graph_repr.push_str(&format!("\nKey components identified: {}", comp_list.join(", ")));
    ...
    async fn design_algorithm(&self, task: &CodingTask) -> SACAResult<String> {
        ...
        let sort_score = Self::score_dimension(&desc_lower, &["sort", "order", "rank", "compare", "sorted"]);
        let search_score = Self::score_dimension(&desc_lower, &["search", "find", "locate", "query", "lookup"]);
        let parse_score = Self::score_dimension(&desc_lower, &["parse", "tokenize", "lex", "grammar"]);
        let graph_score = Self::score_dimension(&desc_lower, &["graph", "tree", "node", "edge", "vertex"]);
        ...
        let mut scores: Vec<(f32, &str, Vec<&str>)> = vec![
            (sort_score, "sort", vec!["QuickSort/MergeSort O(n log n)", "Partition/pivot selection", "In-place merge or auxiliary buffer"]),
            (search_score, "search", vec!["Binary search O(log n) for sorted data", "Hash-based O(1) lookup for unsorted", "Trie for string prefix matching"]),
            (parse_score, "parse", vec!["Tokenize input into lexemes", "Build AST via recursive descent", "Error recovery with panic mode"]),
            (graph_score, "graph", vec!["Adjacency list representation", "BFS/DFS traversal with visited set", "Dijkstra/Bellman-Ford for paths"]),
            (conc_score, "concurrency", vec!["Work-stealing thread pool", "Divide work into independent tasks", "Join/barrier synchronization"]),
            (ml_score, "ml", vec!["Feature extraction and normalization", "Forward pass through model layers", "Backpropagation or inference only"]),
        ];
        ...
        if let Some((score, category, algo_steps)) = scores.first() {
            if *score > 0.3 {
                for (i, step_desc) in algo_steps.iter().enumerate() {
                    add_step(&mut design, step, &format!("Core algorithm ({} phase {})", category, i + 1), step_desc);
                    step += 1;
                }
            }
        }
        ...
        if sort_score <= 0.3 && search_score <= 0.3 && parse_score <= 0.3 && graph_score <= 0.3 && conc_score <= 0.3 && ml_score <= 0.3 {
            if task.constraints.iter().any(|c| c.to_lowercase().contains("recursive") || c.contains("recursion")) {
                add_step(&mut design, step, "Core logic",
        ...
        add_step(&mut design, step, "Output formatting",
        ...
        for constraint in &task.constraints {
            ...
            if cl.contains("O(1)") && !constraints_applied {
                add_step(&mut design, step, "Optimization",
        ...
        design.push_str(&format!("\n\nDominant category: {} (score: {:.2})",
            if sort_score > 0.3 { "sort" } else if search_score > 0.3 { "search" }
            else if parse_score > 0.3 { "parse" } else if graph_score > 0.3 { "graph" }
            else if conc_score > 0.3 { "concurrency" } else if ml_score > 0.3 { "ml" }
            else { "generic" },
    ...
    async fn identify_edge_cases(&self, task: &CodingTask) -> SACAResult<Vec<String>> {
        ...
        let mut add_unique = |e: String| { if seen.insert(e.clone()) { edge_cases.push(e); } };
        ...
        add_unique("[NULLABILITY] Null/None values — ensure Option/Result is handled without unwrap".to_string());
        add_unique("[BOUNDARY] Maximum size inputs — test memory and performance limits".to_string());
        ...
        if Self::score_dimension(&desc_lower, &["array", "list", "vec", "slice", "collection"]) > 0.3 {
            ...
            add_unique("[PROPERTY:boundary] Already sorted / already in expected order".to_string());
        ...
        if Self::score_dimension(&desc_lower, &["map", "hash", "dict", "hashmap"]) > 0.3 {
            ...
            add_unique("[PROPERTY:idempotency] Overwrite of existing key produces correct state".to_string());
            add_unique("[PROPERTY:boundary] Very large number of entries (hash table resize)".to_string());
        }
        ...
        if Self::score_dimension(&desc_lower, &["sort", "order", "rank", "compare"]) > 0.3 {
            ...
            add_unique("[PROPERTY:invariant] Reverse sorted input (worst case for some algorithms)".to_string());
            add_unique("[PROPERTY:stability] Equal elements — verify stable sort preserves original order".to_string());
        }
        if Self::score_dimension(&desc_lower, &["numeric", "number", "count", "integer", "float"]) > 0.3 {
            add_unique("[BOUNDARY] Zero value".to_string());
            add_unique("[BOUNDARY] Negative values".to_string());
            add_unique("[BOUNDARY] Integer overflow / underflow (use checked arithmetic)".to_string());
            ...
            add_unique("[BOUNDARY] Maximum/minimum integer values (i32::MAX, i64::MIN, etc.)".to_string());
        }
        if Self::score_dimension(&desc_lower, &["string", "text", "char", "utf"]) > 0.3 {
            add_unique("[IDENTITY] Empty string".to_string());
            add_unique("[ENCODING] Unicode / multi-byte characters (emoji, CJK, combining marks)".to_string());
            add_unique("[IDENTITY] Whitespace-only string".to_string());
            add_unique("[BOUNDARY] Very long string (10^6+ characters)".to_string());
            add_unique("[ENCODING] Special characters and escaping (null bytes, control chars)".to_string());
        }
        if Self::score_dimension(&desc_lower, &["file", "io", "read", "write", "persist", "storage"]) > 0.3 {
        ...
        if Self::score_dimension(&desc_lower, &["network", "http", "api", "socket", "tcp", "url"]) > 0.3 {
        ...
        for req in &task.requirements {
            ...
            if rl.contains("deterministic") || rl.contains("idempotent") {
                add_unique("[PROPERTY] Operation is idempotent — repeated calls produce same result".to_string());
            }
        }
    ...
    async fn identify_assumptions(&self, task: &CodingTask) -> SACAResult<Vec<String>> {
        ...
        let mut seen: std::collections::HashSet<String> = std::collections::HashSet::new();
        let mut add_unique = |a: String| { if seen.insert(a.clone()) { assumptions.push(a); } };
        ...
        if Self::score_dimension(&desc_lower, &["sort", "order", "rank", "compare", "sorted"]) > 0.3 {
            ...
            if !desc_lower.contains("stable") {
                add_unique("Sort stability is not required — equal elements may be reordered".to_string());
            }
        }
        if Self::score_dimension(&desc_lower, &["numeric", "count", "math", "number", "integer", "float"]) > 0.3 {
            add_unique("Numeric values fit within standard integer/float ranges (no overflow)".to_string());
            add_unique("Division by zero will not occur".to_string());
            add_unique("Integer operations will not overflow — use checked_add/sub/mul otherwise".to_string());
            if desc_lower.contains("float") || desc_lower.contains("f32") || desc_lower.contains("f64") {
                add_unique("Floating point NaN/Infinity values are handled or excluded from input".to_string());
            }
        }
        if Self::score_dimension(&desc_lower, &["search", "find", "locate", "query", "lookup"]) > 0.3 {
            add_unique("Search predicate is deterministic (same key → same result for same data)".to_string());
            if Self::score_dimension(&desc_lower, &["hash"]) == 0.0 {
                add_unique("Data structure supports the required search operation efficiently".to_string());
            } else {
                add_unique("Hash function distributes keys uniformly to avoid pathological collisions".to_string());
            }
        }
        if Self::score_dimension(&desc_lower, &["string", "text", "char", "utf"]) > 0.3 {
            add_unique("Input strings are valid UTF-8 encoded (not arbitrary byte sequences)".to_string());
            add_unique("String comparison uses Unicode-aware semantics where appropriate".to_string());
        }
        if Self::score_dimension(&desc_lower, &["concurr", "thread", "parallel", "async", "sync"]) > 0.3 {
            add_unique("Operations are safely concurrent or properly synchronized".to_string());
            add_unique("Shared mutable state is protected by synchronization primitives".to_string());
            add_unique("Tasks are sufficiently independent to benefit from parallel execution".to_string());
        }
        ...
        if Self::score_dimension(&desc_lower, &["network", "api", "http", "socket", "rest", "rpc"]) > 0.3 {
            add_unique("Network is available with acceptable latency (< timeout threshold)".to_string());
        ...
        for constraint in &task.constraints {
            ...
            if cl.contains("complexity") || cl.contains("big-o") || cl.contains("O(") {
                add_unique(format!("Input size assumptions match the {} complexity guarantee", constraint));
            }
            if cl.contains("memory") || cl.contains("ram") {
                add_unique(format!("Working set fits within the specified memory budget: {}", constraint));
            }
            if cl.contains("real") || cl.contains("deadline") || cl.contains("latency") {
                add_unique(format!("System meets the real-time performance requirements: {}", constraint));
            }
            if cl.contains("safe") || cl.contains("no panic") || cl.contains("no unsafe") {
                add_unique(format!("Code must not panic on any valid input: {}", constraint));
            }
            if cl.contains("deterministic") || cl.contains("idempotent") {
                add_unique(format!("Operation must produce identical results across calls: {}", constraint));
            }
        }
    ...
    async fn assess_risks(&self, task: &CodingTask) -> SACAResult<Vec<String>> {
        ...
        let mut risks: std::collections::BTreeMap<String, String> = std::collections::BTreeMap::new();
        let risk_insert = |risks: &mut std::collections::BTreeMap<String, String>, id: &str, desc: String| {
            risks.entry(id.to_string()).or_insert(desc);
        };

        // Universal risks with complexity analysis
        risk_insert(&mut risks, "perf_scale", "[COMPLEXITY] Performance degradation with large inputs exceeding O(n) expected scale".to_string());
        risk_insert(&mut risks, "memory_oom", "[COMPLEXITY] Memory overflow or excessive allocation leading to OOM".to_string());
        risk_insert(&mut risks, "error_handling", "[FAILURE-MODE] Incorrect error handling or missing edge case paths".to_string());
        risk_insert(&mut risks, "regression", "[MAINTENANCE] Regression in existing functionality when adding new code".to_string());
        ...
        if Self::score_dimension(&desc_lower, &["sort", "order", "rank", "compare"]) > 0.3 {
            risk_insert(&mut risks, "sort_stability", "[CORRECTNESS] Unstable sort when stability is required by downstream consumers".to_string());
            ...
            risk_insert(&mut risks, "sort_compare_inconsistency", "[CORRECTNESS] Non-transitive comparison function causes undefined behavior".to_string());
        }
        if Self::score_dimension(&desc_lower, &["search", "find", "locate", "query", "lookup"]) > 0.3 {
            risk_insert(&mut risks, "search_false_negative", "[CORRECTNESS] False negatives due to incorrect comparison or hash collision".to_string());
            risk_insert(&mut risks, "search_oob", "[SAFETY] Index out of bounds in edge cases (empty collection, single element)".to_string());
            if Self::score_dimension(&desc_lower, &["hash"]) > 0.3 {
                risk_insert(&mut risks, "search_hash_collision", "[PERFORMANCE] Hash collision attack causing O(n) degradation in HashMap".to_string());
            }
        }
        if Self::score_dimension(&desc_lower, &["parse", "tokenize", "lex", "grammar"]) > 0.3 {
            risk_insert(&mut risks, "parse_infinite_loop", "[CORRECTNESS] Malformed input causing infinite loop or excessive backtracking".to_string());
            risk_insert(&mut risks, "parse_unicode", "[CORRECTNESS] Unicode/encoding issues with multi-byte characters".to_string());
            risk_insert(&mut risks, "parse_oom", "[RESILIENCE] Catastrophic backtracking (ReDoS) on adversarial input".to_string());
        }
        if Self::score_dimension(&desc_lower, &["recursive", "recursion", "recurse"]) > 0.3 {
        ...
        if Self::score_dimension(&desc_lower, &["concurr", "thread", "parallel", "async", "race"]) > 0.3 {
            risk_insert(&mut risks, "concurrency_race", "[SAFETY] Data race due to unsynchronized shared state".to_string());
            risk_insert(&mut risks, "concurrency_deadlock", "[SAFETY] Deadlock from incorrect lock ordering".to_string());
            risk_insert(&mut risks, "concurrency_starvation", "[PERFORMANCE] Thread starvation or excessive context switching".to_string());
            risk_insert(&mut risks, "concurrency_poison", "[RESILIENCE] Mutex poisoning crashes the entire system on one thread failure".to_string());
        }
        if Self::score_dimension(&desc_lower, &["network", "http", "api", "socket", "tcp"]) > 0.3 {
            risk_insert(&mut risks, "network_timeout", "[RESILIENCE] Unhandled network timeout causing indefinite hang".to_string());
            risk_insert(&mut risks, "network_leak", "[RESILIENCE] Leaked connections or file descriptors".to_string());
            risk_insert(&mut risks, "network_retry", "[CORRECTNESS] Missing retry logic causes transient failures".to_string());
        }
        if Self::score_dimension(&desc_lower, &["file", "persist", "disk", "io", "storage"]) > 0.3 {
            risk_insert(&mut risks, "io_partial_write", "[CORRECTNESS] Partial write / corrupted data on crash".to_string());
            risk_insert(&mut risks, "io_race", "[SAFETY] Race condition from concurrent file access".to_string());
        }
        if Self::score_dimension(&desc_lower, &["unsafe", "pointer", "raw", "ffi"]) > 0.3 {
            risk_insert(&mut risks, "unsafe_ub", "[SAFETY] Undefined behavior from incorrect unsafe code".to_string());
            risk_insert(&mut risks, "unsafe_memory", "[SAFETY] Memory safety violation in unsafe blocks".to_string());
        }
        ...
        for req in &task.requirements {
            ...
            if rl.contains("fast") || rl.contains("perform") {
                risk_insert(&mut risks, "over_optimization", "[MAINTENANCE] Over-optimization leading to unreadable or unmaintainable code".to_string());
            }
            ...
            if rl.contains("async") || rl.contains("future") {
                risk_insert(&mut risks, "async_pin", "[CORRECTNESS] Incorrect pinning or self-referential structs in async code".to_string());
            }
        }
