...
impl VOGPPlus {
    ...
    fn compute_adaptive_smoothness_loss(
        &mut self,
        ...
        debug!(
            ...
            current_grad_norm,
            self.ema_stabilizer.get_current_norm(),
            adaptive_threshold,
            penalty
        );
...
impl BatchStatistics {
    ...
    fn update(&mut self, _predictions: &Array2<f32>, gradients: Option<&ArrayD<f32>>) {
        ...
        if let Some(grad) = gradients {
            ...
            self.variance_gradient_norm =
                (1.0 - alpha) * self.variance_gradient_norm + alpha * diff * diff;
        }
    }
...
pub mod vogp_utils {
    ...
    pub fn approximate_gradient_norm<F>(
        gradient_fn: F,
        input_shape: &[usize],
        num_samples: usize,
    ) -> f32
