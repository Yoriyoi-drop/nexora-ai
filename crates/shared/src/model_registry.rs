...
pub async fn initialize_global_registry() -> Result<(), RegistryError> {
    ...
    for model_id in crate::model_identity::NxrModelId::all() {
        let config = crate::model_config::NxrModelConfig::for_model(model_id);
        let caps = crate::capability_spec::predefined::get_capabilities(model_id);
        let tier = model_id.tier();
        let meta = crate::model_identity::ModelMeta::new(
            model_id,
            tier,
            "0.1.0".to_string(),
