...
use ndarray::ArrayD;
use nexora_deeplearning::star_x::core::{HierarchicalGating, SelectiveUpdate};
use nexora_deeplearning::star_x::traits::Forward;
use nexora_deeplearning::{
    echo_net::{EchoNetConfig, EchoNetMetrics, EchoNetState},
    star_x::{
        EpisodicMemoryRetention, HarmonicTemporalEncoding, SelectiveStateUpdate,
        SparseCausalAttention, StarXConfig, StarXMetics, StarXState, TemporalGatingHierarchy,
    },
        ...
        match self.architecture {
            ...
            DLArchitecture::Hybrid => {
                if self.star_x_config.is_none() || self.echo_net_config.is_none() {
                    return Err(
                        "Both STAR-X and ECHO-Net config required for Hybrid architecture"
                            .to_string(),
                    );
                }
            }
...
impl DeepLearningEngine {
    pub fn new(config: DeepLearningConfig) -> DLResult<Self> {
        ...
        let state = match config.architecture {
            DLArchitecture::StarX => {
                let star_x_config = config.star_x_config.as_ref().ok_or_else(|| {
                    DeepLearningError::Configuration {
                        reason: "star_x_config required for StarX architecture".into(),
                    }
                })?;
                DeepLearningState::StarX(StarXState::new(star_x_config))
            }
            DLArchitecture::EchoNet => {
                let echo_net_config = config.echo_net_config.as_ref().ok_or_else(|| {
                    DeepLearningError::Configuration {
                        reason: "echo_net_config required for EchoNet architecture".into(),
                    }
                })?;
            ...
            DLArchitecture::Hybrid => {
                let star_x_config = config.star_x_config.as_ref().ok_or_else(|| {
                    DeepLearningError::Configuration {
                        reason: "star_x_config required for Hybrid architecture".into(),
                    }
                })?;
                let echo_net_config = config.echo_net_config.as_ref().ok_or_else(|| {
                    DeepLearningError::Configuration {
                        reason: "echo_net_config required for Hybrid architecture".into(),
                    }
                })?;
            ...
            DLArchitecture::StarX | DLArchitecture::Hybrid => {
                ...
                Some(StarXPipeline {
                    ...
                    sca: SparseCausalAttention::new(
                        sc.hidden_size,
                        sc.attention_heads,
                        sc.max_sparse_connections,
                        sc.entropy_regularization,
                    )?,
                    hte: HarmonicTemporalEncoding::new(
                        sc.temporal_embedding_dim,
                        sc.harmonic_frequencies,
                        1024,
                    )?,
    ...
    pub async fn forward(&self, input: &ArrayD<f32>) -> DLResult<ArrayD<f32>> {
        ...
        match &*state {
            DeepLearningState::StarX(_) => {
                let pipeline = self.starx_pipeline.as_ref().ok_or_else(|| {
                    DeepLearningError::Configuration {
            ...
            DeepLearningState::EchoNet(echo_net_state) => {
                ...
                let result_data: Vec<f32> = flat_input
                    .iter()
                    .enumerate()
                    .map(|(i, x)| {
                        let amp = echo_net_state
                            .amplitude_spectrum
                            .as_slice()
                            .and_then(|s| s.get(i % amp_dim))
                            .copied()
                            .unwrap_or(0.0);
                        let phase = echo_net_state
                            .semantic_phase
                            .as_slice()
                ...
                ArrayD::from_shape_vec(input.shape(), result_data).map_err(DeepLearningError::from)
            }
            DeepLearningState::Hybrid { star_x, echo_net } => {
                let starx_output = {
                    let pipeline = self.starx_pipeline.as_ref().ok_or_else(|| {
    ...
    pub async fn backward(&self, gradient: &ArrayD<f32>) -> DLResult<ArrayD<f32>> {
        ...
        match &*state {
            DeepLearningState::StarX(_) => {
                let _pipeline = self.starx_pipeline.as_ref().ok_or_else(|| {
                    DeepLearningError::Configuration {
                        reason: "StarX pipeline not initialized".into(),
                    }
                })?;
    ...
    fn text_to_tensor(&self, text: &str) -> DLResult<ArrayD<f32>> {
        ...
        ArrayD::from_shape_vec(vec![1, target_dim], features).map_err(DeepLearningError::from)
    }
