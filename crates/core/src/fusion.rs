...
use crate::types::{FusionResult, ModelId, TaskExecution};
...
impl ResponseFusion {
    ...
    pub async fn fuse_responses(&self, tasks: &[TaskExecution]) -> CoreResult<FusionResult> {
        ...
        if model_responses.is_empty() {
            return Err(crate::error::CoreError::Fusion(
                "No responses to fuse".to_string(),
            ));
        }
        ...
        info!(
            "Response fusion completed: confidence={:.2}, conflicts={}",
    ...
    fn detect_conflicts(&self, responses: &[String]) -> bool {
        ...
        for response in responses {
            ...
            if response_lower.contains("yes")
                || response_lower.contains("true")
                || response_lower.contains("success")
            {
                has_positive = true;
            }
            if response_lower.contains("no")
                || response_lower.contains("false")
