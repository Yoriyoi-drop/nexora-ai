...
use crate::types::{FusionResult, IntentType, ModelId};
...
impl MultiModelCoordinator {
    ...
    pub async fn coordinate_execution(
        ...
        info!(
            "Coordinating multi-model execution for intent: {:?}",
            primary_intent
        );
        ...
        let fusion_confidence = if results.is_empty() {
            0.0
        } else {
        ...
        info!(
            "Multi-model coordination completed with {} models",
            selected_models.len()
        );
        Ok(fusion_result)
    }
    ...
    fn find_model_for_intent(&self, intent: IntentType) -> CoreResult<ModelId> {
        ...
        if self.model_instances.contains_key(&model_id) {
            ...
            Err(CoreError::ModelNotAvailable {
                model_id: model_id as u8,
            })
        }
    }
        ...
        while completed.len() < total {
            let ready: Vec<usize> = plan
                ...
                .filter(|(_, step)| {
                    step.dependencies
                        .iter()
                        .all(|dep| completed.iter().any(|&j| plan[j].model == *dep))
                })
                .map(|(i, _)| i)
                .collect();
            ...
            for &idx in &ready {
                ...
                handles.push(tokio::spawn(async move {
                    ...
                    match model {
                        ...
                        None => Err(CoreError::ModelNotAvailable {
                            model_id: model_id as u8,
                        }),
                    }
                }));
            ...
            for j in (i + 1)..successful_results.len() {
                ...
                if output1.contains("yes") && output2.contains("no") {
                    conflicts.push(format!(
                        ...
                        successful_results[i].model, successful_results[j].model
                    ));
                }
