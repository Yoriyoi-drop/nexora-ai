...
impl AnalysisProcessor {
    ...
    fn detect_programming_language(code: &str) -> String {
        let language_patterns = [
            ...
            (
                "python",
                vec![
                    "def ", "import ", "from ", "class ", "self.", "elif ", "try:",
                ],
            ),
            (
                "javascript",
            ...
            (
                "go",
                vec!["func ", "package ", "import ", "go ", "chan ", "defer "],
            ),
        ];
    ...
    fn categorize_memory_request(input: &str) -> String {
        ...
        } else if input_lower.contains("recall")
            || input_lower.contains("retrieve")
    ...
    fn detect_reasoning_type(input: &str) -> String {
        ...
        if input_lower.contains("if") && input_lower.contains("then") {
            ...
            || input_lower.contains("therefore")
            || input_lower.contains("thus")
        {
            "Causal Reasoning".to_string()
        } else if input_lower.contains("all")
            || input_lower.contains("every")
            || input_lower.contains("always")
        {
            "Deductive Reasoning".to_string()
        } else if input_lower.contains("some")
            || input_lower.contains("might")
            || input_lower.contains("could")
        {
            "Inductive Reasoning".to_string()
        } else if input_lower.contains("similar")
            || input_lower.contains("like")
            || input_lower.contains("as")
        {
            "Analogical Reasoning".to_string()
        } else if input_lower.contains("better")
            || input_lower.contains("worse")
            || input_lower.contains("best")
        {
    ...
    fn analyze_logical_structure(input: &str) -> String {
        ...
        if premise_count > 0 && conclusion_count > 0 {
            format!(
                "Structured Argument ({} premises, {} conclusions)",
                premise_count, conclusion_count
            )
    ...
    fn detect_logical_fallacies(input: &str) -> Vec<String> {
        ...
        let fallacy_patterns = [
            (
                "always",
                "Hasty Generalization - using 'always' without sufficient evidence",
            ),
            (
                "never",
                "Hasty Generalization - using 'never' without sufficient evidence",
            ),
            (
                "obviously",
                "Appeal to Common Sense - assuming something is obvious",
            ),
            (
                "everyone knows",
                "Bandwagon Fallacy - appealing to popular opinion",
            ),
            (
                "straw man",
                "Straw Man Argument - misrepresenting opponent's position",
            ),
            (
                "false dilemma",
    ...
    fn suggest_reasoning_improvements(input: &str) -> Vec<String> {
        ...
        if input.to_lowercase().contains("always") || input.to_lowercase().contains("never") {
            suggestions
                .push("Consider using more nuanced language instead of absolutes".to_string());
        }
