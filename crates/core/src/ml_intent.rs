...
use crate::types::{InputData, IntentResult, IntentType};
...
impl FeatureExtractor {
    pub fn new() -> Self {
        let stop_words = vec![
            ...
            "atau", "tapi", "juga", "bisa", "akan", "telah", "sedang", "masih", "the", "and", "or",
            "but", "in", "on", "at", "to", "for", "of", "with", "by", "is", "are", "was", "were",
    ...
    pub fn extract_features(&self, text: &str) -> HashMap<String, f32> {
        ...
        features.insert(
            ...
            processed_text.split_whitespace().count() as f32,
        );
...
impl NaiveBayesClassifier {
    ...
    pub fn train(&mut self, training_data: &[(String, IntentType)]) -> Result<(), anyhow::Error> {
        info!(
            "Training Naive Bayes classifier with {} samples",
            training_data.len()
        );
        ...
        for (intent, count) in &intent_counts {
            self.priors
                .insert(*intent, *count as f32 / total_samples as f32);
        }
        ...
        let mut feature_counts: HashMap<IntentType, HashMap<String, usize>> =
            HashMap::with_capacity(training_data.len());
        ...
        for intent in intent_counts.keys() {
            ...
            let intent_features = feature_counts
                .get(intent)
            ...
            for feature in &self.vocabulary {
                ...
                let likelihood = (*count as f32 + self.alpha)
                    / (total_intent_features as f32 + self.alpha * vocab_size);
                intent_likelihoods.insert(feature.clone(), likelihood);
            }
        }

        info!(
            "Training completed. Vocabulary size: {}",
            self.vocabulary.len()
        );
        Ok(())
    }
        ...
        for intent in self.priors.keys() {
            ...
            for (feature, _) in features {
                if let Some(likelihood) = self.likelihoods.get(intent).and_then(|l| l.get(feature))
                {
                    ...
                    let total_intent_features: usize =
                        self.feature_totals.get(intent).copied().unwrap_or(0);
                    let smoothed_likelihood = self.alpha
                        / (total_intent_features as f32
                            + self.alpha * self.vocabulary.len() as f32);
...
impl AdvancedIntentDetector {
    ...
    pub fn train(&mut self, training_data: &[(String, IntentType)]) -> CoreResult<()> {
        if training_data.is_empty() {
            return Err(CoreError::IntentDetection(
        ...
        info!(
            "Advanced intent detector trained with {} samples",
            training_data.len()
        );
        Ok(())
    }
    ...
    pub async fn detect_intent(&self, input_data: &InputData) -> CoreResult<IntentResult> {
        if !self.is_trained {
            return Err(CoreError::IntentDetection(
                "Detector not trained".to_string(),
            ));
        }

        debug!(
            "Detecting intent with ML approach: {}",
        ...
        let features = self
            .feature_extractor
            .extract_features(&input_data.raw_input);
        ...
        info!(
            "ML Intent detected: primary={:?}, confidence={:.2}, intents_count={}",
