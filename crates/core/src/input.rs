        ...
        if self.validation_enabled {
            if !input_data.validate() {
                ...
                return Err(CoreError::InputValidation(format!(
                    "Input validation failed for type: {:?}",
                    input_type
                )));
            }
        }
...
mod tests {
    ...
    async fn test_command_input_validation() {
        ...
        // Valid command
        let result = receiver
            .receive_input("buat program", InputType::Command)
            .await;
        assert!(result.is_ok());

        // Invalid command
        let result = receiver
            .receive_input("random text", InputType::Command)
            .await;
        assert!(result.is_err());
    }
    ...
    async fn test_query_input_validation() {
        ...
        let result = receiver
            .receive_input("apa yang terjadi", InputType::Query)
            .await;
        assert!(result.is_ok());
