...
impl IntentDetector {
    ...
    pub async fn detect_intent(&self, input_data: &InputData) -> CoreResult<IntentResult> {
        ...
        info!(
            "Intent detected: primary={:?}, multi_intent={:?}, intents_count={}",
    ...
    fn detect_normal_intents(&self, input: &str, result: &mut IntentResult) {
        let input_lower = input.to_lowercase();
        let mut intent_scores: HashMap<IntentType, f32> =
            HashMap::with_capacity(self.keyword_weights.len());
...
mod tests {
    ...
    async fn test_coding_intent_detection() {
        ...
        let input_data = InputData::new(
            "buat fungsi rekursif".to_string(),
    ...
    async fn test_episodic_memory_priority() {
        let detector = IntentDetector::new();
        let input_data = InputData::new(
            "ingat pengalaman episodic".to_string(),
            crate::types::InputType::Text,
        );
