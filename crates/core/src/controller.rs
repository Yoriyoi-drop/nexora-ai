...
impl CoreController {
    ...
    pub async fn process_request(&self, input: &str, input_type: InputType) -> CoreResult<String> {
        ...
        info!(
            "Processing request: type={:?}, length={}",
            input_type,
            input.len()
        );
        ...
        // 8. Update statistics dan metrics
        {
            ...
            if let Ok(duration) = start_time.elapsed() {
                ...
                if total_requests > 0 {
                    state.stats.avg_processing_time_ms = (state.stats.avg_processing_time_ms
                        * (total_requests - 1) as f32
    ...
    async fn route_request(
        ...
        debug!(
            "Routing request: primary_intent={:?}",
        ...
        if !self.is_model_available(target_model) {
            ...
            if let Some(alt_model) = alternative {
                warn!(
                    "Primary model {:?} not available, using alternative: {:?}",
                    target_model, alt_model
                );
                return self
                    .route_to_alternative_model(alt_model, intent, context)
                    .await;
            } else {
                return Err(CoreError::ModelNotAvailable {
                    model_id: target_model as u8,
                });
            }
        }

        let routing_confidence =
            ControllerCore::calculate_routing_confidence(intent.primary_intent, context);
        ...
        debug!(
            "Routing decision: model={:?}, confidence={:.2}",
            decision.target_model, decision.routing_confidence
        );
        Ok(decision)
    }
    ...
    async fn route_to_alternative_model(
        &self,
        model: ModelId,
        intent: &IntentResult,
        context: &ContextInfo,
...
mod tests {
    ...
    async fn test_intent_mapping() {
        // Test coding intent
        assert_eq!(
            ControllerCore::map_intent_to_model(IntentType::Coding),
            ModelId::Coding
        );

        // Test memory intent
        assert_eq!(
            ControllerCore::map_intent_to_model(IntentType::Memory),
            ModelId::Memory
        );

        // Test unknown intent
        assert_eq!(
            ControllerCore::map_intent_to_model(IntentType::Unknown),
            ModelId::Controller
        );
    }
