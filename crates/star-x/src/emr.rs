...
impl EpisodicMemoryRetention {
    pub fn new(max_size: usize, hidden_size: usize, _write_threshold: f32) -> DLResult<Self> {
        if hidden_size == 0 {
    ...
    fn find_available_slot(&self) -> Option<usize> {
        ...
        for entry in self.memory_entries.values() {
            let decayed_priority =
                entry.priority * entry.compute_decay_factor(self.current_time, self.decay_rate);
    ...
    fn retrieve_similar_memories(
        &self,
        query: &ArrayD<f32>,
    ) -> DLResult<Vec<(usize, f32, ArrayD<f32>)>> {
        ...
        for (id, entry) in &self.memory_entries {
            ...
            if similarity >= self.similarity_threshold {
                ...
                let decayed_similarity =
                    similarity * entry.compute_decay_factor(self.current_time, self.decay_rate);
        ...
        Ok(similarities
            .into_iter()
            .take(self.retrieval_top_k)
            .map(|(&id, sim, content)| (id, sim, content))
            .collect())
    }
...
impl EpisodicMemoryRetention {
    ...
    pub fn consolidate_memory(&mut self) -> DLResult<usize> {
        ...
        let mut groups: HashMap<Vec<usize>, Vec<(usize, f32, ArrayD<f32>)>> =
            HashMap::with_capacity(self.memory_entries.len());
        ...
        for (_group_key, memories) in groups {
            if memories.len() > 1 {
                ...
                let avg_priority =
                    memories.iter().map(|(_, p, _)| *p).sum::<f32>() / memories.len() as f32;
