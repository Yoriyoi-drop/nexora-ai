...
use crate::core::GradientResonance;
...
impl AdaptiveGradientResonance {
    pub fn new(initial_resonance: f32, momentum_factor: f32, decay_rate: f32) -> DLResult<Self> {
    ...
    fn compute_cosine_similarity(&self, a: &ArrayD<f32>, b: &ArrayD<f32>) -> DLResult<f32> {
        let a_flat = a
            .as_slice()
            .ok_or_else(|| DeepLearningError::ShapeMismatch {
                expected: vec![a.len()],
                actual: vec![a.len()],
            })?;
        let b_flat = b
    ...
    fn adjust_resonance(&mut self, current_norm: f32, previous_norm: f32) {
        ...
        self.resonance_factor = self
            .resonance_factor
            .clamp(self.min_resonance, self.max_resonance);
    }
    ...
    fn update_history(&mut self, resonance: f32) {
        ...
        if !self.resonance_history.is_empty() {
            self.avg_resonance_factor =
                self.resonance_history.iter().sum::<f32>() / self.resonance_history.len() as f32;
        }
    }
...
impl AdaptiveGradientResonance {
    ...
    pub fn frequency_aware_resonance(
        &mut self,
        current_state: &ArrayD<f32>,
        previous_state: &ArrayD<f32>,
    ) -> DLResult<ArrayD<f32>> {
        ...
        let clamped_resonance =
            frequency_adjusted_resonance.clamp(self.min_resonance, self.max_resonance);

        self.apply_resonance(current_state, previous_state, clamped_resonance)
    }

    /// Multi-scale resonance untuk different time scales
    pub fn multi_scale_resonance(
        &self,
        current_state: &ArrayD<f32>,
        ...
        let current_flat = multi_scale_resonated.as_slice_mut().ok_or_else(|| {
    ...
    pub fn gradient_feedback_resonance(
        &mut self,
        candidate_state: &ArrayD<f32>,
        previous_state: &ArrayD<f32>,
        gradient_norm: f32,
        ...
        let clamped_resonance =
            gradient_adjusted_resonance.clamp(self.min_resonance, self.max_resonance);
