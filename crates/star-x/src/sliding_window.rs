...
use crate::tensor_pool::PooledTensor2D;
...
impl SlidingWindowAttention {
    ...
    fn compute_qkv(
        &self,
        input: &ArrayView<f32, ndarray::Ix2>,
    ) -> DLResult<(Array2<f32>, Array2<f32>, Array2<f32>)> {
    ...
    fn sliding_window_attention(
        ...
        for head in 0..self.num_heads {
            ...
            let head_output =
                self.sliding_window_attention_head(&q_head, &k_head, &v_head, seq_len)?;
    ...
    fn update_statistics(&mut self, seq_len: usize) {
        ...
        let total_windows = if total_windows_f.is_nan() {
            0
        } else {
            total_windows_f as usize
        };
...
impl HierarchicalSlidingWindow {
    pub fn new(
        ...
        let base_attention =
            SlidingWindowAttention::new(hidden_dim, num_heads, window_size, stride, num_levels)?;
