...
impl TensorPool {
    ...
    fn find_2d_category(&self, rows: usize, cols: usize) -> DLResult<usize> {
        self.size_categories_2d
            ...
            .ok_or_else(|| DeepLearningError::Configuration {
                reason: format!(
                    "No size category found for 2D tensor of shape ({}, {})",
                    rows, cols
                ),
            })
    }
    ...
    fn find_dyn_category(&self, shape: &[usize]) -> DLResult<usize> {
        self.size_categories_dyn
            ...
            .ok_or_else(|| DeepLearningError::Configuration {
                reason: format!(
                    "No size category found for dynamic tensor of shape {:?}",
                    shape
                ),
            })
    }
    ...
    pub fn get_stats(&self) -> PoolStats {
...

        {
            ...
            for (i, category) in pool.iter().enumerate() {
                stats
                    .pool_1d_sizes
                    .push((self.size_categories_1d[i], category.len()));
            }
        }

        {
            ...
            for (i, category) in pool.iter().enumerate() {
                stats
                    .pool_2d_sizes
                    .push((self.size_categories_2d[i], category.len()));
            }
        }

        {
            ...
            for (i, category) in pool.iter().enumerate() {
                stats
                    .pool_dyn_sizes
                    .push((self.size_categories_dyn[i].clone(), category.len()));
            }
        }
...
// TensorPool::new only fails on impossible allocation (OOM) or invalid config.
// Uses expect() since this must succeed for the system to function — failure here
// indicates fatal OOM or programming error, which should crash early.
static GLOBAL_TENSOR_POOL: std::sync::LazyLock<TensorPool> = std::sync::LazyLock::new(|| {
    TensorPool::new().expect(
        "Failed to create global tensor pool (OOM or invalid size category config)"
    )
});
