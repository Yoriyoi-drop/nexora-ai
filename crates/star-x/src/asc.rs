...
impl ParallelAssociativeScan {
    ...
    pub fn state_transition(
        ...
        let state_part = self
            .associative_op
            .associative_operation(a_matrix, previous_state)?;
    ...
    fn compute_prefix_sums(&mut self) -> DLResult<()> {
        ...
        for i in 1..self.block_results.len() {
            running_sum = self
                .associative_op
                .associative_operation(&running_sum, &self.block_results[i])?;
            self.prefix_sums.push(running_sum.clone());
        }
    ...
    pub fn optimized_long_sequence_scan(
        &mut self,
        sequence: &[ArrayD<f32>],
        initial_state: &ArrayD<f32>,
    ) -> DLResult<Vec<ArrayD<f32>>> {
    ...
    fn sequential_scan(
        ...
        for element in sequence {
            current_state = self
                .associative_op
                .associative_operation(&current_state, element)?;
...
impl ParallelAssociativeScan {
    ...
    pub fn verify_scan_correctness(
        &self,
        sequence: &[ArrayD<f32>],
        initial_state: &ArrayD<f32>,
    ) -> DLResult<bool> {
