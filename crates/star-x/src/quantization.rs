...
impl QuantizationEngine {
    ...
    pub fn quantize_weights_awq(
        &mut self,
        weights: &Array2<f32>,
        activations: &Array2<f32>,
    ) -> DLResult<QuantizedTensor> {
    ...
    fn dequantize_int8(&self, quantized: &QuantizedTensor) -> DLResult<Array2<f32>> {
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                dequantized[[i, j]] =
                    (q_val as f32 - quantized.zero_point() as f32) * quantized.scale();
            }
        }
...
impl MixedPrecisionEngine {
    ...
    pub fn set_layer_precision(&mut self, layer_idx: usize, precision: QuantPrecision) {
        if layer_idx >= self.precision_map.len() {
            self.precision_map
                .resize(layer_idx + 1, QuantPrecision::FP16);
        }
...
pub fn create_quantization_engine(
    precision: QuantPrecision,
    method: QuantMethod,
) -> DLResult<QuantizationEngine> {
    QuantizationEngine::new(precision, method)
}

/// Global mixed precision engine instance
pub fn create_mixed_precision_engine(
    base_precision: QuantPrecision,
