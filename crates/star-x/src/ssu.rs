...
impl SelectiveStateUpdate {
    ...
    fn concatenate(
        ...
        let tgh_flat = require_contiguous(tgh_output.as_slice())?;
        let sca_flat = require_contiguous(sca_output.as_slice())?;
        ...
        let concat_flat = require_contiguous_mut(concatenated.as_slice_mut())?;
...
impl SelectiveUpdate for SelectiveStateUpdate {
    ...
    fn selective_update(
        ...
        let rel_flat = require_contiguous(relevance.as_slice())?;
        ...
        let updated_flat = require_contiguous_mut(updated_state.as_slice_mut())?;
...
impl SelectiveStateUpdate {
    ...
    pub fn hierarchical_selective_update(
        &self,
        previous_state: &ArrayD<f32>,
        candidate_state: &ArrayD<f32>,
        relevance: &ArrayD<f32>,
    ...
    pub fn temporal_coherence_update(
        &self,
        previous_state: &ArrayD<f32>,
        candidate_state: &ArrayD<f32>,
        relevance: &ArrayD<f32>,
...
mod tests {
    ...
    fn test_hierarchical_selective_update() -> DLResult<()> {
        ...
        let result =
            ssu.hierarchical_selective_update(&prev, &cand, &relevance, &[0.2, 0.6, 0.9])?;
        assert_eq!(result.shape(), &[128]);
        Ok(())
    }
