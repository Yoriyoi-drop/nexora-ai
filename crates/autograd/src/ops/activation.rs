...
gen_activation_op!(relu,
    ...
    move |sg: &[GpuTensor], gg: &GpuTensor, ctx: &GpuContext| {
        let x = &sg[0];
        let relu_out = ctx
            .elementwise_unary(x, crate::gpu::ElemOp::Relu)
...
gen_activation_op!(gelu,
    ...
    |gi: &GpuTensor, _gr: &GpuTensor| vec![gi.clone()],
    move |sg: &[GpuTensor], gg: &GpuTensor, ctx: &GpuContext| {
        let da = crate::gpu_backward::gelu_backward(ctx, &sg[0], gg)
...
gen_activation_op!(silu,
    ...
    move |sg: &[GpuTensor], gg: &GpuTensor, ctx: &GpuContext| {
        let x = &sg[0];
        let sig = ctx
            .elementwise_unary(x, crate::gpu::ElemOp::Sigmoid)
                    ...
                    Ok(silu_gate) => match ctx.mul(&silu_gate, gpu_x) {
                        Ok(gpu_result) => {
                            ...
                            return Tensor::from_gpu_with_grad_fn(
                                ...
                                Some(Box::new(move |saved_gpu, grad_gpu, ctx| {
                                    let (d_gate, d_x) =
                                        crate::gpu_backward::swiglu_backward_gpu(
                                            ctx,
                                            &saved_gpu[0],
                                            &saved_gpu[1],
                                            grad_gpu,
