...
impl MemoryAgent {
    ...
    pub fn new(memory_store: Arc<RwLock<MemoryLayers>>, config: MemoryAgentConfig) -> Self {
        Self {
            id: Uuid::new_v4(),
    ...
    pub async fn store_memory(
        ...
        self.memory_store
            ...
            .await
            .store(
                nexora_memory::MemoryLayer::Short,
                &memory_key,
                &memory_value,
            )
            .await?;
        ...
        let mut memory_result =
            self.get_memory(memory_id)
                ...
                .ok_or_else(|| AgentError::ProcessingError {
                    ...
                    reason: format!("Memory {} not found", memory_id),
                })?;
    ...
    pub async fn get_user_memories(
        ...
        let query = MemoryQuery {
            ...
            limit: limit
                .map(|l| l as usize)
                .or(Some(self.config.default_query_limit as usize)),
            offset: None,
            filters: HashMap::new(),
        };
    ...
    pub async fn search_memories(
        ...
        let truncated = if search_term.len() > 100 {
            format!(
                "{} [truncated {} chars]",
                &search_term[..100],
                search_term.len()
        ...
        for memory in results {
            if memory.timestamp < cutoff_time.timestamp().max(0) as u64 {
                self.memory_store
                    .write()
                    .await
                    .delete(nexora_memory::MemoryLayer::Short, &memory.key)
                    .await?;
                cleaned_count += 1;
            }
        }
...
impl Agent for MemoryAgent {
    ...
    async fn process(&mut self, context: AgentContext) -> Result<AgentResponse> {
        ...
        debug!(
            "MemoryAgent processing request for session: {}",
            context.session_id
        );
        ...
        let result = match action {
                ...
                let memory_type = match memory_type_str {
                    ...
                    _ => {
                        return Err(AgentError::ProcessingError {
                            operation: "validate_memory_type".to_string(),
                            reason: format!("Invalid memory_type: {}", memory_type_str),
                        })
                    }
                };
            ...
            "get" => {
                let memory_id_str = context
                    .parameters
                    .get("memory_id")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| AgentError::ProcessingError {
                        operation: "validate".to_string(),
                        reason: "memory_id required".to_string(),
                    })?;

                let memory_id =
                    Uuid::parse_str(memory_id_str).map_err(|_| AgentError::ProcessingError {
            ...
            "search" => {
                let search_term = context
                    ...
                    .ok_or_else(|| AgentError::ProcessingError {
                        operation: "validate".to_string(),
                        reason: "search_term required".to_string(),
                    })?;
                ...
                let memory_type = match memory_type_str {
                    Some(t) => Some(match t {
                        ...
                        _ => {
                            return Err(AgentError::ProcessingError {
                                operation: "validate_memory_type".to_string(),
                                reason: format!("Invalid memory_type: {}", t),
                            })
                        }
                    }),
                    None => None,
                ...
                let results = self
                    .search_memories(search_term.to_string(), context.user_id, memory_type, limit)
                    .await?;
