...
pub fn create_ipeps_from_weights(
    ...
    for i in 0..unit_cell_size {
        ...
        let sub_tensor = weights
            .slice_axis(ndarray::Axis(0), ndarray::Slice::from(start_idx..end_idx))
            .to_owned()
            .into_dyn();
...
pub fn compute_entanglement_entropy(
    ...
    match network {
        QuantumNetwork::MatrixProductOperator(mpo) => compute_mpo_entanglement(mpo, partition),
        QuantumNetwork::InfinitePEPS(ipeps) => compute_ipeps_entanglement(ipeps, partition),
    }
}
...
fn compute_mutual_information(
    ...
    for i in 0..min_size {
        inner_product += tensor_a.as_slice().ok_or_else(|| {
            crate::ATQSError::InvalidInput("Failed to access tensor_a slice".to_string())
        })?[i]
            * tensor_b.as_slice().ok_or_else(|| {
                crate::ATQSError::InvalidInput("Failed to access tensor_b slice".to_string())
            })?[i];
    }
...
fn contract_neighbors(tensor: &ArrayD<f32>) -> Result<ArrayD<f32>, crate::ATQSError> {
    ...
    let mut result = Array::zeros((shape[0] * shape[1], shape[2] * shape[3]));
...
fn compute_eigenvalues(matrix: &ArrayD<f32>) -> Result<Vec<f32>, crate::ATQSError> {
    ...
    for k in 0..n.min(5) {
        ...
        for _iteration in 0..max_iterations {
            ...
            for i in 0..n {
                for j in 0..n {
                    let matrix_elem = matrix.as_slice().ok_or_else(|| {
                        crate::ATQSError::InvalidInput("Failed to access matrix slice".to_string())
                    })?[i * n + j];
            ...
            for i in 0..n {
                ...
                for j in 0..n {
                    matrix_vector += matrix.as_slice().ok_or_else(|| {
                        crate::ATQSError::InvalidInput("Failed to access matrix slice".to_string())
                    })?[i * n + j]
                        * new_vector[j];
                }
