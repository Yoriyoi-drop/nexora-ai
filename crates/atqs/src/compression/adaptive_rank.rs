...
fn compute_layer_sensitivity(
    ...
    let criticality_score = compute_criticality_score(layer_idx, &layer_type, model)?;
...
fn compute_rank_assignment(
    ...
    let sensitivity_factor =
        1.0 + (sensitivity.sensitivity_score - 0.5) * config.sensitivity_weight;
    ...
    let compression_ratio =
        estimate_compression_ratio(final_rank, &tensor_format, sensitivity.tensor.shape())?;
...
fn compute_output_variance(layer: &dyn crate::ModelLayer) -> Result<f32, crate::ATQSError> {
    ...
    let variance = outputs.iter().map(|&x| (x - mean).powi(2)).sum::<f32>() / outputs.len() as f32;
...
fn compute_overall_compression_ratio(
    ...
    let total_compression: f32 = assignments.iter().map(|a| a.compression_ratio).sum();
...
fn apply_layer_weights(
    weights: &ArrayD<f32>,
    input: &ArrayD<f32>,
) -> Result<f32, crate::ATQSError> {
    // Realistic matrix-vector multiplication with bias simulation
    if weights.len() != input.len() {
        return Err(crate::ATQSError::InvalidInput(format!(
            "Weight and input dimensions mismatch: {} vs {}",
            weights.len(),
            input.len()
        )));
    }
    ...
    fn tensor_to_string(&self, tensor: &ndarray::ArrayD<f32>) -> crate::ATQSResult<String> {
        ...
        if tensor.ndim() >= 2 {
            ...
            for i in 0..rows {
                // Extract character features
                let features = [
                    *tensor.get([i, 0]).unwrap_or(&0.0),
                    *tensor.get([i, 1]).unwrap_or(&0.0),
                    *tensor.get([i, 2]).unwrap_or(&0.0),
                    *tensor.get([i, 3]).unwrap_or(&0.0),
                    *tensor.get([i, 4]).unwrap_or(&0.0),
                    *tensor.get([i, 5]).unwrap_or(&0.0),
                    *tensor.get([i, 6]).unwrap_or(&0.0),
                    *tensor.get([i, 7]).unwrap_or(&0.0),
                ];
