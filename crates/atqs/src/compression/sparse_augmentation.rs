...
pub fn apply_sparse_augmentation(
    ...
    let compression_ratio =
        calculate_compression_ratio_sparse(original_tensor, low_rank_tensor, &sparse_component)?;
...
fn compute_statistical_threshold(
    tensor: &ArrayD<f32>,
    std_devs: f32,
...
fn refine_low_rank_component(
    ...
    let (u, s, vt) = compute_svd_truncated(
        &residual_2d.view(),
        residual_2d.ncols().min(residual_2d.nrows()) / 2,
    )?;
    ...
    tensor
        ...
        .and_then(|arr| {
            arr.into_dimensionality().map_err(|_| {
                crate::ATQSError::InvalidInput("Failed to convert dimensionality".to_string())
            })
        })
}
