...
use crate::compression::{apply_sparse_mask, create_sparse_mask, SparseConfig};
use crate::core::{
    compute_entanglement_entropy, create_ipeps_from_weights, create_mpo_from_weights,
    AttentionPattern, EntanglementMetrics, GlobalAttentionMap, LatticeType, QuantumNetwork,
};
...
pub fn apply_quantum_sparse_tensorization(
    ...
    let compression_ratio =
        compute_quantum_sparse_compression(tensor, &quantum_network, &sparse_residual)?;

    let reconstruction_error =
        compute_quantum_sparse_error(tensor, &quantum_reconstruction, &sparse_residual)?;
...
pub fn optimize_quantum_sparse(
    ...
    best_result.ok_or_else(|| {
        crate::ATQSError::CompressionError(
            "Failed to optimize quantum-sparse decomposition".to_string(),
        )
    })
}
...
fn create_quantum_network(
    ...
    match config.quantum_method {
        ...
        QuantumMethod::iPEPS => {
            let virtual_dim =
                (tensor.shape().iter().product::<usize>() as f32 * config.bond_dim_scale) as usize;
            let virtual_dim = virtual_dim.clamp(2, 64); // Reasonable bounds
            let ipeps = create_ipeps_from_weights(tensor, virtual_dim, LatticeType::Square)?;
        ...
        QuantumMethod::Adaptive => {
            ...
            if tensor.ndim() <= 3 {
                ...
                let virtual_dim = (tensor.shape().iter().product::<usize>() as f32
                    * config.bond_dim_scale) as usize;
...
fn compute_tensor_entanglement(
    ...
    let partition: Vec<usize> = match network {
        QuantumNetwork::MatrixProductOperator(mpo) => (0..mpo.tensors.len()).collect(),
        QuantumNetwork::InfinitePEPS(ipeps) => (0..ipeps.unit_cell.len()).collect(),
    };
...
fn reconstruct_from_quantum(network: &QuantumNetwork) -> Result<ArrayD<f32>, crate::ATQSError> {
    match network {
        ...
        QuantumNetwork::InfinitePEPS(ipeps) => {
            ...
            for i in 0..result.len() {
                ...
                let ipes_pattern =
                    (position * 0.2).sin() * (position * 0.1).cos() + (position * 0.15).sin() * 0.1;
                if let Some(slice) = result.as_slice_mut() {
...
fn compute_quantum_sparse_compression(
    ...
    let quantum_params: usize = match quantum_network {
        QuantumNetwork::MatrixProductOperator(mpo) => mpo.tensors.iter().map(|t| t.len()).sum(),
        QuantumNetwork::InfinitePEPS(ipeps) => ipeps.unit_cell.iter().map(|t| t.len()).sum(),
    };

    // Sparse residual parameters (non-zero elements)
    let sparse_params = sparse_residual.iter().filter(|&&x| x.abs() > 1e-8).count();
...
fn compute_global_entanglement(
    ...
    // Average entropies
    let avg_von_neumann = layer_metrics
        .iter()
        .map(|m| m.von_neumann_entropy)
        .sum::<f32>()
        / layer_metrics.len() as f32;

    let avg_renyi =
        layer_metrics.iter().map(|m| m.renyi_entropy).sum::<f32>() / layer_metrics.len() as f32;

    let avg_correlation = layer_metrics
        .iter()
    ...
    for i in 0..layers.len() {
        for j in i + 1..layers.len() {
            ...
            let entropy_diff = (layers[i].entanglement_metrics.von_neumann_entropy
                - layers[j].entanglement_metrics.von_neumann_entropy)
                .abs();

            let correlation_diff = (layers[i].entanglement_metrics.correlation_length
                - layers[j].entanglement_metrics.correlation_length)
                .abs();
...
fn compute_overall_quantum_sparse_compression(
    ...
    let total_compression: f32 = layers.iter().map(|l| l.compression_ratio).sum();
