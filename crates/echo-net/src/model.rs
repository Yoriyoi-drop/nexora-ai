...
impl EchoNetModel {
    pub fn new(config: EchoNetConfig) -> DLResult<Self> {
        ...
        let bands: Vec<crate::FrequencyBand> = config.band_frequencies.iter().enumerate().map(|(i, &freq)| {
            crate::FrequencyBand {
                id: i,
        ...
        let levels: Vec<crate::CompressionLevel> = (0..config.compression_levels).map(|i| {
            crate::CompressionLevel {
                level: i,
