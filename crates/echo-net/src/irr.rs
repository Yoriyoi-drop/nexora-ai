...
impl IterativeResonanceReasoner {
    ...
    pub fn forward(
        ...
        for step in 0..self.max_iterations {
            ...
            let mut reasoning_step =
                ReasoningStep::new(step, current_query.clone(), resonance.clone());
    ...
    fn calculate_novelty(&self, query: &ArrayD<f32>, resonance: &ArrayD<f32>) -> DLResult<f32> {
        ...
        let similarity =
            self.cosine_similarity(&self.to_1d_array(query)?, &self.to_1d_array(resonance)?);
    ...
    fn calculate_variance(&self, scores: &[f32]) -> f32 {
        ...
        let variance: f32 = scores.iter().map(|&x| (x - mean).powi(2)).sum();
    ...
    fn update_performance_metrics(&mut self) -> DLResult<()> {
        if !self.convergence_scores.is_empty() {
            self.average_confidence =
                self.convergence_scores.iter().sum::<f32>() / self.convergence_scores.len() as f32;
        }
