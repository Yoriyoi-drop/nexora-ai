...
use crate::utils::ResonanceCalculator;
...
impl ResonanceCandidate {
    pub fn new(
        index: usize,
        resonance_score: f32,
        resonance_data: ArrayD<f32>,
        holographic_component: ArrayD<f32>,
    ) -> Self {
        Self {
            index,
            resonance_score,
...
impl Ord for ResonanceCandidate {
    fn cmp(&self, other: &Self) -> Ordering {
        other
            .resonance_score
            .partial_cmp(&self.resonance_score)
...
impl TopKResonanceRouting {
    ...
    pub fn forward(
        &mut self,
        query: &HolographicWave,
        resonance_data: &[ArrayD<f32>],
        holographic_memory: &[ArrayD<f32>],
    ) -> DLResult<ArrayD<f32>> {
        ...
        if let Some(cached_indices) = self.routing_cache.get(&cache_key) {
            ...
            return self.route_from_cached_indices(
                &cached_indices,
                resonance_data,
                holographic_memory,
            );
        ...
        let context = self.generate_sparse_routing_context(
            &diverse_candidates,
            resonance_data,
            holographic_memory,
        )?;
    ...
    fn calculate_resonance_score(
        ...
        let holographic_alignment =
            self.calculate_holographic_alignment(&query_tensor, holographic_component)?;
    ...
    /// Calculate holographic alignment
    fn calculate_holographic_alignment(
        &self,
        query: &ArrayD<f32>,
        holographic: &ArrayD<f32>,
    ) -> DLResult<f32> {
    ...
    fn select_top_k_candidates(
        &self,
        candidates: &mut Vec<ResonanceCandidate>,
    ) -> DLResult<Vec<ResonanceCandidate>> {
    ...
    fn apply_diversity_filtering(
        ...
        for candidate in candidates {
            let is_diverse = if selected_data.is_empty() {
                ...
                let min_similarity = selected_data
                    .iter()
    ...
    /// Generate sparse routing context
    fn generate_sparse_routing_context(
        &self,
        candidates: &[ResonanceCandidate],
        _resonance_data: &[ArrayD<f32>],
        _holographic_memory: &[ArrayD<f32>],
    ) -> DLResult<ArrayD<f32>> {
    ...
    fn record_routing_event(
        &mut self,
        all_candidates: &[ResonanceCandidate],
        selected_candidates: &[ResonanceCandidate],
        routing_time: f32,
    ) -> DLResult<()> {
        let event = RoutingEvent {
            ...
            routing_time,
            average_resonance: selected_candidates
                .iter()
                .map(|c| c.resonance_score)
                .sum::<f32>()
                / selected_candidates.len() as f32,
        };
    ...
    fn update_performance_metrics(&mut self, routing_time: f32) -> DLResult<()> {
        ...
        if !self.routing_history.is_empty() {
            let recent_events: Vec<&RoutingEvent> =
                self.routing_history.iter().rev().take(10).collect();

            let avg_selected: f32 = recent_events
                .iter()
                .map(|e| e.selected_candidates as f32)
                .sum::<f32>()
                / recent_events.len() as f32;
            let avg_total: f32 = recent_events
                .iter()
                .map(|e| e.total_candidates as f32)
                .sum::<f32>()
                / recent_events.len() as f32;
...
mod tests {
    ...
    fn test_candidate_ordering() {
        let candidate1 = ResonanceCandidate::new(
            0,
            0.5,
            ArrayD::from_shape_vec(vec![1], vec![1.0]).unwrap(),
            ArrayD::from_shape_vec(vec![1], vec![1.0]).unwrap(),
        );
        let candidate2 = ResonanceCandidate::new(
            1,
            0.8,
            ArrayD::from_shape_vec(vec![1], vec![2.0]).unwrap(),
            ArrayD::from_shape_vec(vec![1], vec![2.0]).unwrap(),
        );
        let candidate3 = ResonanceCandidate::new(
            2,
            0.3,
