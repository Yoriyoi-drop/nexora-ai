...
use crate::{
    Database, DatabaseConfig, DatabaseInfo,
    QueryResult, Value, Statement, ExecuteResult, Transaction,
...
impl PostgreSQLDatabase {
    ...
    pub async fn new(config: DatabaseConfig) -> Result<Self> {
        ...
        let credentials = credential_manager.load_database_credentials()
            .map_err(|e| anyhow!("Failed to load database credentials: {}", e))?;
        
        let connection_pool = Arc::new(PostgreSQLConnectionPool::new(
    ...
    pub async fn with_credentials(credentials: DatabaseCredentials, config: DatabaseConfig) -> Result<Self> {
        let connection_pool = Arc::new(PostgreSQLConnectionPool::new(
            ConnectionPoolConfig {
                max_connections: config.max_connections,
                min_connections: std::cmp::max(1, config.max_connections / 4),
    ...
    async fn create_client(&self) -> Result<Client> {
        ...
        tracing::info!("Connecting to PostgreSQL: {}", credentials.build_connection_string_safe());
...
impl Database for PostgreSQLDatabase {
    ...
    async fn disconnect(&self) -> Result<()> {
        ...
        tracing::info!("Disconnected from PostgreSQL database: {}", self.config.database);
        Ok(())
    }
...
impl PostgreSQLConnectionPool {
    ...
    pub async fn new(config: ConnectionPoolConfig, credentials: DatabaseCredentials) -> Result<Self> {
        ...
        *connections.write().await = conn_vec;
        ...
        tracing::info!("Created PostgreSQL connection pool with {} initial connections", config.min_connections);
            ...
            if let Some(pos) = connections.iter().position(|c| !c.is_active) {
                ...
                if connection.ping().await {
                    ...
                    if stats.total_connections > 0 {
                        stats.average_wait_time_ms = stats.total_wait_time_ms as f64 / stats.total_connections as f64;
                    }
...
impl ConnectionPool for PostgreSQLConnectionPool {
    ...
    async fn return_connection(&self, connection: Box<dyn crate::DatabaseConnection>) -> Result<()> {
...
impl PostgreSQLConnection {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        // Log safe connection info (without password)
        tracing::info!("Establishing PostgreSQL connection: {}", credentials.build_connection_string_safe());
        ...
        tracing::debug!("PostgreSQL connection established successfully for connection {}", self.id);
        Ok(())
    }
    ...
    pub async fn execute_query(&self, query: &str, _params: Vec<crate::Value>) -> Result<crate::QueryResult> {
        if !self.is_active {
