...
mod serde_timestamp {
        ...
        D: Deserializer<'de>,
    {
        ...
        let elapsed = system_time.duration_since(SystemTime::now()).unwrap_or_default();
...
impl DatabasePool {
    ...
    pub async fn new(config: DatabaseConfig) -> Result<Self> {
        info!("Creating database pool with max {} connections", config.max_connections);
    ...
    pub async fn execute_query(
        ...
        match result {
            ...
            Err(e) => {
                ...
                // Log failed query
                self.log_slow_query(query, duration, Some(e.to_string()), None).await;
    ...
    pub async fn execute_transaction(&self) -> Result<()> {
        ...
        let duration = start_time.elapsed();
        if duration > self.config.slow_query_threshold {
            self.log_slow_query("TRANSACTION", duration, None, None).await;
        }
    ...
    async fn cache_query(&self, query: &str, _result: Vec<sqlx::postgres::PgRow>) -> Result<()> {
        ...
        cache.insert(query.to_string(), CachedQuery {
            query_hash,
    ...
    async fn update_query_stats(&self, duration: Duration, failed: bool) {
        ...
        let total_time_ms = stats.avg_query_time.as_millis() as u64 * (stats.total_queries - 1) + duration.as_millis() as u64;
    ...
    async fn log_slow_query(&self, query: &str, duration: Duration, error: Option<String>, parameters: Option<serde_json::Value>) {
    ...
    async fn find_unused_indexes(&self) -> Result<Vec<String>> {
        ...
        Ok(rows.iter()
            .map(|row| {
                let schema: String = row.get::<Option<String>, _>("schemaname").unwrap_or_else(|| "public".to_string());
                let table: String = row.get::<Option<String>, _>("tablename").unwrap_or_else(|| "unknown".to_string());
                let index: String = row.get::<Option<String>, _>("indexname").unwrap_or_else(|| "unknown".to_string());
    ...
    pub fn set_unused_indexes(&mut self, indexes: Vec<String>) {
        ...
        if !self.unused_indexes.is_empty() {
            self.recommendations.push(
                format!("Consider dropping {} unused indexes", self.unused_indexes.len())
            );
        }
        ...
        for (table, stats) in &self.table_stats {
            if stats.dead_tuples > stats.live_tuples * 2 {
                self.recommendations.push(
                    format!("Table {} has many dead tuples, consider VACUUM", table)
                );
            }
        }
    }
