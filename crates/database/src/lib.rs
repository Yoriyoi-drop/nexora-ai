...
impl Database for MySQLDatabase {
    ...
    async fn execute_query(&self, query: &str, params: Vec<Value>) -> Result<QueryResult, Box<dyn std::error::Error>> {
        let mut conn = self.pool.get_conn()?;
        
        // Convert params to MySQL format
        let mysql_params: Vec<mysql::Value> = params.into_iter()
            .map(|v| match v {
    ...
    async fn execute_statement(&self, statement: &Statement) -> Result<ExecuteResult, Box<dyn std::error::Error>> {
        let mut conn = self.pool.get_conn()?;
        
        // Convert params to MySQL format
        let mysql_params: Vec<mysql::Value> = statement.parameter_types.iter()
            .map(|v| match v {
    ...
    async fn get_info(&self) -> Result<DatabaseInfo, Box<dyn std::error::Error>> {
        ...
        let version = version_result.first().cloned().unwrap_or_else(|| "Unknown".to_string());
        ...
        let database_size_mb = size_result.first()
...
impl Transaction {
    /// Create a new transaction without raw connection (for PostgreSQL/SQLite)
...
impl Transaction {
    ...
    pub fn create_savepoint(&mut self, name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn rollback_to_savepoint(&mut self, name: &str) -> Result<(), Box<dyn std::error::Error>> {
        if let Some(ref mut conn) = self.conn {
    ...
    pub fn execute(&mut self, query: &str) -> Result<ExecuteResult, Box<dyn std::error::Error>> {
        if let Some(ref mut conn) = self.conn {
...
impl DatabaseFactory {
    ...
    pub async fn create(config: DatabaseConfig) -> Result<Arc<dyn Database>> {
        match config.database_type {
            ...
            #[cfg(feature = "mysql")]
            DatabaseType::MySQL => {
                let config = config.as_any().downcast_ref::<MySQLConfig>()
            ...
            DatabaseType::MySQL => {
                Err(anyhow::anyhow!("MySQL support not enabled. Enable with 'mysql' feature"))
            }
        }
    }
}
...
impl DatabaseManager {
    ...
    pub async fn get_all_status(&self) -> HashMap<String, DatabaseStatus> {
        ...
        for (name, database) in &self.databases {
            ...
            status.insert(name.clone(), DatabaseStatus {
                name: name.clone(),
                is_connected,
                info,
                pool_status: database.get_pool_status().await.unwrap_or_else(|_| PoolStatus {
                    total_connections: 0,
...
impl Value {
    ...
    fn parse_timestamp(&self, timestamp_str: &str) -> Self {
        ...
        if let Ok(unix_timestamp) = timestamp_str.parse::<f64>() {
            ...
            if let Some(system_time) = std::time::SystemTime::UNIX_EPOCH.checked_add(std::time::Duration::new(seconds.try_into().unwrap(), nanos)) {
        ...
        for format in &formats {
            if let Ok(naive_dt) = NaiveDateTime::parse_from_str(timestamp_str, format) {
                ...
                if let Some(nanos) = datetime.timestamp_nanos_opt() {
                    if let Some(system_time) = std::time::SystemTime::UNIX_EPOCH.checked_add(std::time::Duration::new(
