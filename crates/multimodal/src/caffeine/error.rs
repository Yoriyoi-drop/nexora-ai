...
impl From<nexora_atqs::error::ATQSError> for CaffeineError {
    fn from(err: nexora_atqs::error::ATQSError) -> Self {
        Self::AtqsCompression(format!("ATQS error: {}", err))
    }
}

/// Convert HAS-MoE-FFN errors to CAFFEINE errors
impl From<nexora_has_moe_ffn::error::HasMoeFfnError> for CaffeineError {
    fn from(err: nexora_has_moe_ffn::error::HasMoeFfnError) -> Self {
