...
impl TokenSequenceProcessor {
    ...
    pub fn create_modality_mask(
        &self,
        tokens: &[UnifiedToken],
    ) -> Result<HashMap<ModalityType, Vec<f32>>> {
    ...
    pub fn create_position_embeddings(
        &self,
        tokens: &[UnifiedToken],
        embed_dim: usize,
    ) -> Result<Vec<Vec<f32>>> {
        ...
        for token in tokens {
            ...
            for d in 0..embed_dim {
                if d % 2 == 0 {
                    embedding[d] = (token.position as f32
                        / 10000.0_f32.powf(d as f32 / embed_dim as f32))
                    .sin();
                } else {
                    embedding[d] = (token.position as f32
                        / 10000.0_f32.powf(d as f32 / embed_dim as f32))
    ...
    pub fn create_modality_embeddings(
        &self,
        tokens: &[UnifiedToken],
        embed_dim: usize,
    ) -> Result<Vec<Vec<f32>>> {
    ...
    pub fn validate_sequence(&self, tokens: &[UnifiedToken]) -> Result<()> {
        if tokens.len() > self.max_sequence_length {
            return Err(crate::caffeine::error::CaffeineError::tokenizer(&format!(
