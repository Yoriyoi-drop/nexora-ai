...
impl ActionPlanningModule {
    ...
    fn extract_constraints_and_goals(
        ...
        if let Some(ref instruction) = context.instruction {
            ...
            if instruction.to_lowercase().contains("quick")
                || instruction.to_lowercase().contains("fast")
            {
                context.constraints.push("time_limited".to_string());
            }

            if instruction.to_lowercase().contains("careful")
                || instruction.to_lowercase().contains("precise")
            {
    ...
    fn create_click_parameters(
        ...
        if let Some(spatial_token) = tokens.iter().find(|t| t.modality == ModalityType::Image) {
            if let Some((x, y, _w, _h)) = spatial_token.spatial_coords {
                params.insert(
                    "x".to_string(),
                    serde_json::Value::Number(serde_json::Number::from_f64(x as f64).ok_or_else(
                        || {
                            crate::caffeine::error::CaffeineError::output_generation(
                                "Failed to convert x to number",
                            )
                        },
                    )?),
                );
                params.insert(
                    "y".to_string(),
                    serde_json::Value::Number(serde_json::Number::from_f64(y as f64).ok_or_else(
                        || {
                            crate::caffeine::error::CaffeineError::output_generation(
                                "Failed to convert y to number",
                            )
                        },
                    )?),
                );
    ...
    fn create_extract_parameters(
        ...
        if let Some(ref instruction) = context.instruction {
            if let Some(extract_target) = self.extract_target_from_instruction(instruction) {
                params.insert(
                    "target".to_string(),
                    serde_json::Value::String(extract_target),
                );
            }
        }
