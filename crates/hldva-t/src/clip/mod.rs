    ...
    pub fn new(config: &ClipConfig) -> HLDVAResult<Self> {
        ...
        for i in 0..26 {
            let letter = char::from_u32('a' as u32 + i)
                .ok_or_else(|| {
                    crate::types::HLDVAError::InvalidInput(
                        "Failed to convert to character".to_string(),
                    )
                })?
                .to_string();
...
use super::dit::{FeedForward, LayerNorm, Linear, MultiHeadAttention, PositionEmbedding};
...
impl TokenEmbedding {
    ...
    fn embed(&self, tokens: &[usize]) -> HLDVAResult<Tensor> {
        ...
        Ok(Tensor::new(
            embedded,
            vec![tokens.len(), self.embedding_dim],
        ))
    }
}
...
impl PatchEmbedding {
    ...
    fn embed(&self, image: &Tensor) -> HLDVAResult<Tensor> {
        ...
        let mut patches =
            Vec::with_capacity(num_patches * self.patch_size * self.patch_size * channels);
        ...
        let patch_tensor = Tensor::new(
            patches,
            vec![num_patches, self.patch_size * self.patch_size * channels],
        );
