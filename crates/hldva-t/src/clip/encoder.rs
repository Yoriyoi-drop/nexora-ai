        ...
        for i in 0..self.config.embedding_dim {
            ...
            for (pos, &token) in tokens.iter().enumerate() {
                ...
                let pos_encoding = if i % 2 == 0 {
                    ((pos as f32)
                        / 10000.0_f32.powf((i as f32) / (self.config.embedding_dim as f32)))
                    .sin()
                } else {
                    ((pos as f32)
                        / 10000.0_f32.powf((i as f32) / (self.config.embedding_dim as f32)))
                ...
                let attention_weight =
                    1.0 / (1.0 + ((pos as f32 - tokens.len() as f32 / 2.0).abs() / 10.0));
                    ...
                    if j + receptive_field <= image_data.len() {
                        ...
                        for k in 0..receptive_field {
                            if j + k < image_data.len() {
                                // Gabor-like kernel
                                let kernel_weight = ((k as f32 - receptive_field as f32 / 2.0)
                                    / receptive_field as f32)
                                    .cos()
                                    * (-((k as f32 - receptive_field as f32 / 2.0).powi(2)
                                        / (2.0 * (receptive_field as f32 / 4.0).powi(2))))
                                    .exp();
