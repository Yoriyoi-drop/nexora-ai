...
use crate::{config::VAEConfig, types::*};
...
impl Conv2D {
    ...
    pub fn forward(&self, input: &Tensor) -> HLDVAResult<Tensor> {
        ...
        for out_c in 0..self.out_channels {
            for out_y in 0..output_height {
                for out_x in 0..output_width {
                    ...
                    // Add bias
                    let bias_val = if out_c < bias_data.len() {
                        bias_data[out_c]
                    } else {
                        0.0
                    };
        ...
        Ok(Tensor::new(
            output,
            vec![output_height, output_width, self.out_channels],
        ))
    }
}
            ...
            for w in 0..width {
                ...
                for c in 0..channels {
                    ...
                    if idx < input_data.len() {
                        ...
                        let weight_val = if c < weight_data.len() {
                            weight_data[c]
                        } else {
                            1.0
                        };
                        let bias_val = if c < bias_data.len() {
                            bias_data[c]
                        } else {
                            0.0
                        };
