...
impl HLDVAPipeline {
    ...
    pub fn generate(&self, input: HLDVAInput) -> HLDVAResult<HLDVAOutput> {
        ...
        let (final_latent, intermediate_latents) =
            self.cascaded_upsampling(base_latent, &clip_embedding, &input.negative_prompt)?;
    ...
    fn dit_denoising_loop(
        ...
        for (step_idx, &timestep) in timesteps.iter().enumerate() {
            ...
            latent.data = self
                .ddpm
                .step(latent.data, noise_pred.predicted_noise, timestep)?;
    ...
    fn calculate_metrics(
        ...
        let clip_score =
            self.clip_embedding_similarity(&clip_embedding.text_features, &image_embedding);
