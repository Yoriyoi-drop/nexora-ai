...
impl CascadedModel {
    ...
    pub fn upsample(
        ...
        // Upsample dengan DiT
        let upscaled_latent =
            upsampler.upscale(conditioned_latent, clip_embedding, negative_embedding)?;
...
impl UpsamplerStage {
    pub fn new(config: &UpsamplerConfig, stage_idx: usize) -> HLDVAResult<Self> {
        ...
        let dit_config = crate::config::DiTConfig {
            model_size: crate::config::DiTModelSize::Small, // Use smaller model for upsampling
    ...
    pub fn upscale(
        ...
        let denoised_latent =
            self.dit_denoising(upsampled_latent, clip_embedding, negative_embedding)?;
    ...
    fn spatial_upsample(&self, latent: &LatentSpace) -> HLDVAResult<LatentSpace> {
        ...
        // Update resolution
        let new_resolution =
            Resolution::new(latent.resolution.width * 2, latent.resolution.height * 2);

        Ok(LatentSpace::new(upscaled, new_resolution, latent.channels))
    }
