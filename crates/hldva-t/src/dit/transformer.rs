...
impl TransformerLayer {
    ...
    pub fn forward(&self, hidden: &Tensor, conditioning: &Tensor) -> HLDVAResult<Tensor> {
        ...
        let attn_output = self
            .self_attention
            .forward(&norm_hidden, &norm_hidden, &norm_hidden)?;
        ...
        let cross_attn_output =
            self.cross_attention
                .forward(&norm_hidden2, conditioning, conditioning)?;
        let hidden_with_cross = self
            .cross_attention_dropout
            .forward(&hidden_with_attn, &cross_attn_output)?;
...
impl MixtureOfExperts {
    ...
    fn select_top_experts(&self, gate_scores: &[f32]) -> HLDVAResult<(Vec<usize>, Vec<f32>)> {
        let mut indexed_scores: Vec<(usize, f32)> = gate_scores
            .iter()
            .enumerate()
            .map(|(i, &score)| (i, score))
            .collect();
        ...
        // Select top-k
        let selected_experts: Vec<usize> = indexed_scores
            .iter()
            .take(self.top_k)
            .map(|(idx, _)| *idx)
            .collect();

        let expert_weights: Vec<f32> = indexed_scores
            .iter()
            .take(self.top_k)
            .map(|(_, score)| *score)
            .collect();
...
use super::attention::{FeedForward, LayerNorm, Linear, MultiHeadAttention};
