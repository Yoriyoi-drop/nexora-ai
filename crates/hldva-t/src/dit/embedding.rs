...
impl PatchEmbedding {
    ...
    pub fn embed(&self, latent: &Tensor) -> HLDVAResult<Tensor> {
        ...
        let mut patches =
            Vec::with_capacity(num_patches * channels * self.patch_size * self.patch_size);
        ...
        let patch_tensor = Tensor::new(
            patches,
            vec![num_patches, channels * self.patch_size * self.patch_size],
        );
...
impl PositionEmbedding {
    ...
    pub fn get_2d_embedding(&self, height: usize, width: usize) -> HLDVAResult<Tensor> {
        ...
        Ok(Tensor::new(
            embedding,
            vec![height * width, self.hidden_dim],
        ))
    }
...
use super::attention::{LayerNorm, Linear};
