    ...
    fn compute_beta(&self, t: usize) -> f32 {
        match self.schedule_type {
            ...
            NoiseScheduleType::Cosine => {
                ...
                let cosine_factor =
                    ((t_f / t_max + s) / (1.0 + s) * std::f32::consts::PI / 2.0).cos();
                let cosine_factor_prev =
                    (((t_f + 1.0) / t_max + s) / (1.0 + s) * std::f32::consts::PI / 2.0).cos();
