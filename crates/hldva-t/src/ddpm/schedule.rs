...
impl NoiseSchedule {
    ...
    pub fn new(config: NoiseScheduleConfig) -> HLDVAResult<Self> {
        ...
        for t in 0..num_steps {
            let beta = match config.schedule_type {
                ScheduleType::Linear => {
                    config.beta_start
                        + (config.beta_end - config.beta_start) * (t as f32)
                            / (num_steps as f32 - 1.0)
                }
                ScheduleType::Cosine => {
                    let s = 0.008;
                    let f = ((t as f32) / (num_steps as f32) + s) / (1.0 + s)
                        * std::f32::consts::PI
                        / 2.0;
                    (f.cos()).powf(2.0)
                }
    ...
    pub fn add_noise(
        ...
        if original_data.len() != noise_data.len() {
            return Err(HLDVAError::Model(
                "Original and noise must have same dimensions".to_string(),
            ));
        }
    ...
    pub fn remove_noise(
        ...
        for (noisy_val, pred_noise_val) in noisy_data.iter().zip(predicted_noise_data.iter()) {
            let denoised =
                (noisy_val - pred_noise_val * beta / sqrt_one_minus_alpha_bar) / alpha.sqrt();
