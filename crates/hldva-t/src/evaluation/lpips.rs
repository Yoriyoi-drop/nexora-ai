...
impl LPIPSMetric {
    ...
    pub fn calculate_lpips_batch(
        ...
        if images1.len() != images2.len() {
            return Err(HLDVAError::Evaluation(
                "Image batches must have same size".to_string(),
            ));
        }
    ...
    fn compute_lpips_distance(
        ...
        if features1.len() != features2.len() {
            return Err(HLDVAError::Evaluation(
                "Feature layers must match".to_string(),
            ));
        }
        ...
        for (layer_idx, (feat1, feat2)) in features1.iter().zip(features2.iter()).enumerate() {
            ...
            let layer_distance =
                self.calculate_weighted_distance(&normalized1, &normalized2, layer_idx)?;
    ...
    fn calculate_weighted_distance(
        &self,
        feat1: &[f32],
        feat2: &[f32],
        layer_idx: usize,
