...
impl HLDAEvaluator {
    ...
    fn calculate_fid(&self, generated: &[Tensor], reference: &[Tensor]) -> HLDVAResult<f32> {
        let fid_metric = self.metrics.get("fid").ok_or_else(|| crate::HLDVAError::InvalidInput("FID metric not found".to_string()))?;
    ...
    fn calculate_inception_score(&self, images: &[Tensor]) -> HLDVAResult<f32> {
        let is_metric = self.metrics.get("inception_score").ok_or_else(|| crate::HLDVAError::InvalidInput("Inception Score metric not found".to_string()))?;
    ...
    fn calculate_precision_recall(
        ...
        let pr_metric = self.metrics.get("precision_recall").ok_or_else(|| crate::HLDVAError::InvalidInput("Precision-Recall metric not found".to_string()))?;
