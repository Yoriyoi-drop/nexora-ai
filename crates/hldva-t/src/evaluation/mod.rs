...
impl HLDAEvaluator {
    ...
    pub fn new() -> HLDVAResult<Self> {
        ...
        metrics.insert(
            "inception_score".to_string(),
            Box::new(InceptionScoreMetric::new()?),
        );
        metrics.insert("clip_score".to_string(), Box::new(CLIPScoreMetric::new()?));
        metrics.insert(
            "precision_recall".to_string(),
            Box::new(PrecisionRecallMetric::new()?),
        );
    ...
    pub fn evaluate(
        ...
        let (precision, recall) =
            self.calculate_precision_recall(generated_images, reference_images)?;
    ...
    fn calculate_fid(&self, generated: &[Tensor], reference: &[Tensor]) -> HLDVAResult<f32> {
        let fid_metric = self
            .metrics
            .get("fid")
            .ok_or_else(|| crate::HLDVAError::InvalidInput("FID metric not found".to_string()))?;
    ...
    fn calculate_inception_score(&self, images: &[Tensor]) -> HLDVAResult<f32> {
        let is_metric = self.metrics.get("inception_score").ok_or_else(|| {
            crate::HLDVAError::InvalidInput("Inception Score metric not found".to_string())
        })?;
    ...
    fn calculate_clip_score(&self, images: &[Tensor], prompts: &[String]) -> HLDVAResult<f32> {
        let _clip_metric = self.metrics.get("clip_score").ok_or_else(|| {
            crate::HLDVAError::InvalidInput("CLIP Score metric not found".to_string())
        })?;
    ...
    fn calculate_precision_recall(
        ...
        let pr_metric = self.metrics.get("precision_recall").ok_or_else(|| {
            crate::HLDVAError::InvalidInput("Precision-Recall metric not found".to_string())
        })?;
        ...
        if let Some(ref_imgs) = reference {
            ...
            let all_features: Vec<Tensor> = gen_features
                .into_iter()
                .chain(ref_features.into_iter())
                .collect();
    ...
    fn calculate_lpips(
        ...
        let _lpips_metric = self
            .metrics
            .get("lpips")
            .ok_or_else(|| crate::HLDVAError::InvalidInput("LPIPS metric not found".to_string()))?;
...
impl Metric for InceptionScoreMetric {
    fn calculate(&self, inputs: &[&Tensor]) -> HLDVAResult<f32> {
        if inputs.is_empty() {
            return Err(HLDVAError::Evaluation(
                "Need inputs for Inception Score".to_string(),
            ));
        }
...
impl Metric for CLIPScoreMetric {
    fn calculate(&self, inputs: &[&Tensor]) -> HLDVAResult<f32> {
        if inputs.is_empty() {
            return Err(HLDVAError::Evaluation(
                "Need inputs for CLIP Score".to_string(),
            ));
        }
...
impl Metric for PrecisionRecallMetric {
    fn calculate(&self, inputs: &[&Tensor]) -> HLDVAResult<f32> {
        if inputs.len() < 2 {
            return Err(HLDVAError::Evaluation(
                "Need at least 2 inputs for Precision/Recall".to_string(),
            ));
        }
...
impl InceptionNetwork {
    ...
    pub fn forward(&self, image: &Tensor) -> HLDVAResult<Tensor> {
        ...
        if image_shape.len() < 3 {
            return Err(crate::HLDVAError::InvalidInput(
    ...
    fn compute_multi_scale_feature(
        &self,
        image_data: &[f32],
        image_shape: &[usize],
        scale: usize,
    ...
    fn apply_pooling(&self, features: &[f32], _image_shape: &[usize]) -> Vec<f32> {
        match &self.pooling_strategy {
            ...
            PoolingStrategy::Adaptive => {
                ...
                features
                    .iter()
                    .map(|&f| 0.7 * f + 0.2 * avg + 0.1 * max_val)
                    .collect()
            }
        }
    }
