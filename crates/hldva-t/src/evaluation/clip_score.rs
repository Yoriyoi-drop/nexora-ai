...
impl CLIPScoreMetric {
    ...
    pub fn calculate_clip_score(&self, images: &[Tensor], texts: &[String]) -> HLDVAResult<f32> {
        ...
        if images.len() != texts.len() {
            return Err(HLDVAError::Evaluation(
                "Number of images and texts must match".to_string(),
            ));
        }
    ...
    fn get_image_embedding(&self, image: &Tensor) -> HLDVAResult<Vec<f32>> {
        ...
        for i in 0..embedding_dim {
            ...
            let patch_data = &image_data
                [idx..(idx + embedding_dim / image_data.len()).min(image_data.len() - idx)];
    ...
    pub fn calculate_clip_score_with_aggregation(
        ...
        match aggregation {
            AggregationMethod::Mean => Ok(scores.iter().sum::<f32>() / scores.len() as f32),
            ...
            AggregationMethod::Max => Ok(scores.iter().fold(0.0, |a, &b| a.max(b))),
            AggregationMethod::Min => Ok(scores.iter().fold(1.0, |a, &b| a.min(b))),
        }
    }
}
