...
impl MemoryManager {
    ...
    pub async fn store_code_generation(
        &self,
        description: &str,
        generated_code: &str,
    ) -> Result<()> {
...
mod tests {
    ...
    async fn test_memory_layer_operations() {
        ...
        manager
            .store_short_term("temp_key", "temp_value")
            .await
            .unwrap();
        manager
            .store_session("session_key", "session_value")
            .await
            .unwrap();
        manager
            .store_long_term("persistent_key", "persistent_value")
            .await
            .unwrap();
        manager
            .store_knowledge("knowledge_key", "knowledge_value")
            .await
            .unwrap();
    ...
    async fn test_memory_interaction_storage() {
        ...
        manager
            .store_interaction("User: Hello", "Assistant: Hi there!")
            .await
            .unwrap();
    ...
    async fn test_memory_search_functionality() {
        ...
        // Store test data
        manager
            .store_short_term("test_key_1", "test_value_1")
            .await
            .unwrap();
        manager
            .store_session("test_key_2", "test_value_2")
            .await
            .unwrap();
        manager
            .store_long_term("test_key_3", "test_value_3")
            .await
            .unwrap();
        ...
        let layer_results: Vec<_> = all_results
            .iter()
    ...
    fn test_cognitive_dynamics_full_step() {
        ...
        let state = cd.step(
            &field, &curvature, 0.5, 5.0, 1.0, 0.9, 0.8, 0.5, 1.0, 0.5, 0.3,
        );
