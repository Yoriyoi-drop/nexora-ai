...
impl EpisodicMemory {
    ...
    async fn add_episode_internal(&mut self, episode: MemoryEpisode) -> Result<()> {
        ...
        debug!(
            "Added episode: {} (importance: {:.2})",
            episode_id, episode.importance
        );
        Ok(())
    }
    ...
    pub async fn get_episodes_by_time_range(
        &self,
        start_time: u64,
        end_time: u64,
    ) -> Result<Vec<&MemoryEpisode>> {
    ...
    async fn find_similar_episodes(&mut self, new_episode_id: &str) -> Result<()> {
        ...
        let similar_ids: Vec<String> = similar_episodes
            .iter()
            .take(5)
            .map(|(id, _)| id.clone())
            .collect();
...
mod tests {
    ...
    async fn test_episodic_memory() {
        ...
        // Add episode
        let episode_id = episodic
            .add_episode("test_key", "test_content")
            .await
            .unwrap();
    ...
    async fn test_episodic_memory_capacity_limit() {
        ...
        for i in 0..5 {
            episodic
                .add_episode(&format!("Episode {}", i), &format!("Content {}", i))
                .await
                .unwrap();
        }
    ...
    async fn test_episodic_memory_search_with_relevance() {
        ...
        let episode1 = MemoryEpisode::new(
            "Important meeting about project".to_string(),
            "work".to_string(),
        )
        .with_importance(0.9);
        let episode2 = MemoryEpisode::new("Lunch with friends".to_string(), "personal".to_string())
            .with_importance(0.3);
        let episode3 = MemoryEpisode::new(
            "Project deadline discussion".to_string(),
            "work".to_string(),
        )
        .with_importance(0.8);
    ...
    async fn test_episodic_memory_get_by_time_range() {
        ...
        let range_episodes = episodic
            .get_episodes_by_time_range(
    ...
    async fn test_episodic_memory_update_episode() {
        ...
        let episode_id = episodic
            .add_episode("Original content", "test")
            .await
            .unwrap();
    ...
    async fn test_episodic_memory_get_related_episodes() {
        ...
        // Add related episodes
        let episode1 =
            MemoryEpisode::new("Project kickoff meeting".to_string(), "work".to_string())
                .with_tags(vec!["project".to_string(), "meeting".to_string()]);
        let episode2 =
            MemoryEpisode::new("Project planning session".to_string(), "work".to_string())
    ...
    async fn test_episodic_memory_statistics() {
        ...
        let episode1 =
            MemoryEpisode::new("Work meeting".to_string(), "work".to_string()).with_importance(0.8);
