...
impl ContextCompressor {
    ...
    pub async fn compress(&mut self, context: &str) -> Result<CompressedContext> {
        ...
        let (compressed_data, method) = match self.method {
            CompressionMethod::None => (context.as_bytes().to_vec(), CompressionMethod::None),
            CompressionMethod::Simple => {
                (self.simple_compress(context)?, CompressionMethod::Simple)
            }
            CompressionMethod::RunLength => (
                self.run_length_compress(context)?,
                CompressionMethod::RunLength,
            ),
            CompressionMethod::Dictionary => (
                self.dictionary_compress(context)?,
                CompressionMethod::Dictionary,
            ),
            CompressionMethod::Semantic => (
                self.semantic_compress(context)?,
                CompressionMethod::Semantic,
            ),
        };
        ...
        debug!(
            "Compressed context: {} -> {} bytes (ratio: {:.2})",
            compressed.original_length, compressed.compressed_length, compressed.compression_ratio
        );
    ...
    pub async fn decompress(&self, compressed: &CompressedContext) -> Result<String> {
        ...
        let decompressed = match compressed.compression_method {
            CompressionMethod::None => String::from_utf8(compressed.data.clone())?,
            CompressionMethod::Simple => self.simple_decompress(&compressed.data)?,
            CompressionMethod::RunLength => self.run_length_decompress(&compressed.data)?,
            CompressionMethod::Dictionary => self.dictionary_decompress(&compressed.data)?,
            CompressionMethod::Semantic => self.semantic_decompress(&compressed.data)?,
        };
    ...
    fn update_stats(&mut self, compressed: &CompressedContext) {
        ...
        if self.compression_stats.total_original > 0 {
            self.compression_stats.avg_compression_ratio = self.compression_stats.total_compressed
                as f32
