...
impl MemoryLayers {
    ...
    pub async fn retrieve(&mut self, layer: MemoryLayer, key: &str) -> Result<Option<String>> {
        ...
        if let Some(layer_map) = self.layers.get_mut(&layer) {
            if let Some(entry) = layer_map.get_mut(key) {
                ...
                entry.access();
                debug!(
                    "Retrieved entry from {:?}: {} (access count: {})",
    ...
    pub async fn search(
        ...
        layer: MemoryLayer,
        query: &str,
    ) -> Result<Option<Vec<crate::MemorySearchResult>>> {
    ...
    /// Uses `accessed_at` to find the oldest entries via O(n) partial sort.
    async fn evict_entries(&mut self, layer: MemoryLayer, count: usize) -> Result<()> {
        if let Some(layer_map) = self.layers.get_mut(&layer) {
            if count == 0 || layer_map.len() <= count {
                if count >= layer_map.len() {
                    layer_map.clear();
                }
                return Ok(());
            }
...
mod tests {
    ...
    async fn test_memory_layers() {
        ...
        layers
            .store(MemoryLayer::Short, "test_key", "test_value")
            .await
            .unwrap();
        let result = layers
            .retrieve(MemoryLayer::Short, "test_key")
            .await
            .unwrap();
    ...
    async fn test_layer_capacity() {
        ...
        layers
            .store(MemoryLayer::Short, "key1", "value1")
            .await
            .unwrap();
        layers
            .store(MemoryLayer::Short, "key2", "value2")
            .await
            .unwrap();
        layers
            .store(MemoryLayer::Short, "key3", "value3")
            .await
            .unwrap();
    ...
    async fn test_cleanup_expired() {
        ...
        let entry = MemoryEntry::new("expired".to_string(), "value".to_string()).with_ttl(1); // 1 second TTL
    ...
    async fn test_memory_layers_search() {
        ...
        // Store test data
        layers
            .store(MemoryLayer::Session, "test_key_1", "test_value_1")
            .await
            .unwrap();
        layers
            .store(MemoryLayer::Session, "search_key", "searchable_value")
            .await
            .unwrap();
        layers
            .store(MemoryLayer::Long, "another_key", "another_value")
            .await
            .unwrap();
        ...
        let session_results = layers
            .search_in_layer(MemoryLayer::Session, "search")
            .await
            .unwrap();
        assert!(!session_results.is_empty());

        // Test search with no results
        let no_results = layers
            .search(MemoryLayer::Session, "nonexistent")
            .await
            .unwrap();
    ...
    async fn test_memory_layers_clear() {
        ...
        layers
            .store(MemoryLayer::Short, "temp_key", "temp_value")
            .await
            .unwrap();
        layers
            .store(MemoryLayer::Session, "session_key", "session_value")
            .await
            .unwrap();
        layers
            .store(MemoryLayer::Long, "persistent_key", "persistent_value")
            .await
            .unwrap();
    ...
    async fn test_memory_layers_entry_metadata() {
        ...
        layers
            .store_entry(MemoryLayer::Knowledge, entry)
            .await
            .unwrap();
    ...
    async fn test_memory_layers_get_entry() {
        ...
        // Store test entry
        layers
            .store(MemoryLayer::Long, "specific_key", "specific_value")
            .await
            .unwrap();

        // Retrieve specific entry
        let entry = layers
            .get_entry(MemoryLayer::Long, "specific_key")
            .await
            .unwrap();
        ...
        let non_existent = layers
            .get_entry(MemoryLayer::Long, "non_existent")
            .await
            .unwrap();
    ...
    async fn test_memory_layers_remove_entry() {
        ...
        // Store test entry
        layers
            .store(MemoryLayer::Session, "remove_key", "remove_value")
            .await
            .unwrap();

        // Verify entry exists
        let before_removal = layers
            .get_entry(MemoryLayer::Session, "remove_key")
            .await
            .unwrap();
        ...
        // Remove entry
        let removed = layers
            .remove_entry(MemoryLayer::Session, "remove_key")
            .await
            .unwrap();
        assert!(removed);

        // Verify entry is removed
        let after_removal = layers
            .get_entry(MemoryLayer::Session, "remove_key")
            .await
            .unwrap();
        ...
        let not_removed = layers
            .remove_entry(MemoryLayer::Session, "non_existent")
            .await
            .unwrap();
        assert!(!not_removed);
    }
