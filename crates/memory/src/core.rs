...
impl HebbianMemoryModel {
    ...
    pub fn get_relevant_memories(&self, query: &str, limit: usize) -> Vec<&MemoryEntry> {
        let mut relevant_memories: Vec<(&MemoryEntry, f64)> = self
            .memories
            .values()
            .filter_map(|memory| {
    ...
    pub fn consolidate_memories(&mut self) -> Vec<u32> {
        ...
        let consolidated_ids: Vec<u32> = self
            .memories
            .values()
    ...
    fn update_stats(&mut self) {
        ...
        if !self.memories.is_empty() {
            self.stats.average_strength = self.memories.values().map(|m| m.strength).sum::<f64>()
                / self.memories.len() as f64;

            self.stats.total_activations =
                self.memories.values().map(|m| m.activation).sum::<f64>();
