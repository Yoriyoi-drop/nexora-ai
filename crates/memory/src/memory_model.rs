...
impl HebbianMemory {
    ...
    pub fn apply_local_competition(&mut self) {
        ...
        for &memory_id in &memory_ids {
            ...
            let mut competition_effect: f64 = similarities
                .iter()
                .filter_map(|&(neighbor_id, similarity)| {
                    self.entries
                        .get(&neighbor_id)
                        .map(|neighbor| similarity * neighbor.strength)
                })
                .sum();
            ...
            if let Some(entry) = self.entries.get_mut(&memory_id) {
                if !similarities.is_empty() {
                    ...
                    entry.strength =
                        entry.strength * (1.0 - self.config.competition_alpha * competition_effect);
                }
            }
        }
    ...
    pub fn apply_interference(&mut self) {
        ...
        for (i, &id_a) in memory_ids.iter().enumerate() {
            for &id_b in memory_ids.iter().skip(i + 1) {
                ...
                if similarity > 0.5 {
                    // Only interfere with similar memories
        ...
        if let (Some(entry_a), Some(entry_b)) = (self.entries.get(&id_a), self.entries.get(&id_b)) {
            if let (Some(ref content_a), Some(ref content_b)) = (&entry_a.content, &entry_b.content)
            {
                ...
                let words_a: std::collections::HashSet<&str> =
                    content_a.split_whitespace().collect();
                let words_b: std::collections::HashSet<&str> =
                    content_b.split_whitespace().collect();
    ...
    pub fn consolidation_cycle(&mut self) {
        ...
        let strong_memories: Vec<u32> = self
            .entries
            .iter()
    ...
    pub fn apply_adaptive_decay(&mut self, _time_delta: f64) {
        ...
        let memory_ids: Vec<u32> = self.entries.keys().copied().collect();
        let adaptive_decays: Vec<Option<f64>> = memory_ids
            .iter()
            .map(|&id| self.calculate_adaptive_decay(id))
            .collect();

        // Apply decay using collected values
        for ((_memory_id, entry), adaptive_decay) in
            self.entries.iter_mut().zip(adaptive_decays.into_iter())
        {
    ...
    pub fn validate(&self) -> ValidationResult {
        ...
        let zero_relevance_memories: Vec<_> = self
            .entries
            .values()
...
impl NeuralAttentionMemory {
    ...
    pub fn backward(&mut self, query: &[f32], gradient: &[f32]) {
        ...
        for (i, entry) in self.entries.iter_mut().enumerate() {
            ...
            let grad_dot_value: f32 = gradient
                .iter()
            ...
            for j in 0..self.config.key_dim.min(query.len()).min(entry.key.len()) {
                entry.key[j] +=
                    lr * alpha * grad_dot_value * (1.0 - alpha as f64) as f32 * query[j];
            }
