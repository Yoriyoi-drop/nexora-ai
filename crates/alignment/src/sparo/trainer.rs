...
use super::core::{
    JudgeFeedback, ModelState, PolicyModel, ReasoningTrace, SparoConfig, SparoLoss, TrainingMetrics,
};
use super::data::TrainingBatch;
use super::dpo::{DpoConfig, DpoTrainer};
use super::ipo::{IpoConfig, IpoTrainer};
use super::kto::{KtoConfig, KtoTrainer};
use super::rlaif::{RlaifConfig, RlaifManager};
...
impl SparoTrainer {
    ...
    fn generate_single_trace(&self, prompt: &str) -> Result<ReasoningTrace> {
        ...
        for i in 0..step_count {
            ...
            let step = super::core::ReasoningStep {
                id: Uuid::new_v4(),
                content: if score > -10.0 {
                    content
                } else {
                    format!("Step {}: Analyzing {}", i + 1, prompt)
                },
                step_number: i + 1,
        ...
        let final_answers = vec![
            format!(
                "Based on {} steps of analysis, the conclusion about {} is clear.",
                steps.len(),
                prompt
            ),
            format!(
                "After {} reasoning steps, we can conclude the following about {}.",
                steps.len(),
                prompt
            ),
        ];
        let best_answer = final_answers
            .into_iter()
            .max_by(|a, b| {
                model
                    .log_probability(prompt, a)
                    .unwrap_or(0.0)
    ...
    fn verify_steps(&self, traces: &[ReasoningTrace]) -> Result<Vec<JudgeFeedback>> {
        ...
        for trace in traces {
            ...
            let judge_feedbacks = self
                .rlvf_manager
                .feedback_to_judge_feedback(&step_feedbacks);
    ...
    fn combine_feedback(
        &self,
        ai_feedback: &[JudgeFeedback],
        verified_feedback: &[JudgeFeedback],
    ) -> Vec<JudgeFeedback> {
    ...
    fn train_components(&mut self, batch: &TrainingBatch) -> Result<SparoLoss> {
        ...
        let dpo_pairs = self
            .dpo_trainer
            .extract_preference_pairs(&batch.traces, &batch.feedback)?;
        let dpo_loss = self.dpo_trainer.training_step(&dpo_pairs)?;

        // KTO training
        let kto_labels = self
            .kto_trainer
            .extract_independent_labels(&batch.traces, &batch.feedback)?;
        ...
        Ok(SparoLoss {
            total_loss: self.config.alpha * dpo_loss
                + self.config.beta * kto_loss
                + self.config.gamma * ipo_loss,
            dpo_loss,
            kto_loss,
            ipo_loss,
        })
...
pub mod utils {
    ...
    fn calculate_convergence_rate(losses: &[f32]) -> f32 {
        ...
        let recent_var = recent
            ...
            .sum::<f32>()
            / recent.len() as f32;
        let earlier_var = earlier
            .iter()
            .map(|x| (x - earlier.iter().sum::<f32>() / earlier.len() as f32).powi(2))
            .sum::<f32>()
            / earlier.len() as f32;
