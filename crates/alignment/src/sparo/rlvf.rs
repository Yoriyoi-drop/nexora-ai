...
impl Verifier for LogicalVerifier {
    fn verify(&self, step: &ReasoningStep, context: &ReasoningTrace) -> Result<VerificationResult> {
        ...
        Ok(VerificationResult {
            ...
            error_message: if contradictions.is_empty() {
                None
            } else {
...
impl Verifier for SyntacticVerifier {
    fn verify(
        ...
        Ok(VerificationResult {
            ...
            error_message: if syntax_errors.is_empty() {
                None
            } else {
                Some(syntax_errors.join("; "))
            },
            verification_details: details,
        })
    }
...
impl RlvfManager {
    ...
    pub fn verify_trace(&self, trace: &ReasoningTrace) -> Result<Vec<StepFeedback>> {
        ...
        for step in &trace.steps {
            ...
            // Check if verifiable
            let is_verifiable = verification_results
                .iter()
                .any(|r| r.confidence >= self.config.verification_threshold);
    ...
    pub fn feedback_to_judge_feedback(
        &self,
    ...
    pub fn get_verification_stats(&self, step_feedbacks: &[StepFeedback]) -> VerificationStats {
        ...
        let correct_steps = step_feedbacks
            .iter()
            .filter(|f| f.overall_score >= 0.5)
            .count();

        let avg_score =
            step_feedbacks.iter().map(|f| f.overall_score).sum::<f32>() / total_steps.max(1) as f32;
        ...
        for feedback in step_feedbacks {
            for result in &feedback.verification_results {
                *verification_type_counts
                    .entry(result.verification_type.clone())
                    .or_insert(0) += 1;
            }
        }
...
pub mod utils {
    ...
    ) -> HashMap<VerificationType, Vec<&VerificationResult>> {
