...
use super::core::{FeedbackType, JudgeFeedback, ReasoningStep, ReasoningTrace};
...
impl SparoDataset {
    ...
    fn filter_feedback_by_quality(
        &self,
        feedback: Vec<JudgeFeedback>,
    ) -> Result<Vec<JudgeFeedback>> {
    ...
    fn calculate_trace_quality(&self, trace: &ReasoningTrace) -> f32 {
        ...
        // Step consistency
        let step_numbers: std::collections::HashSet<_> =
            trace.steps.iter().map(|s| s.step_number).collect();
    ...
    fn split_validation_data(&mut self) -> Result<()> {
        ...
        let validation_indices: std::collections::HashSet<_> =
            indices.into_iter().take(validation_size).collect();

        // Split traces
        let (validation_traces, training_traces): (Vec<_>, Vec<_>) = self
            .traces
            .drain(..)
            .enumerate()
        ...
        self.validation_traces = validation_traces
            ...
            .map(|(_, trace)| trace)
            .collect();
        self.traces = training_traces
            .into_iter()
            .map(|(_, trace)| trace)
            .collect();

        // Split feedback
        let (validation_feedback, training_feedback): (Vec<_>, Vec<_>) = self
            .feedback
            .drain(..)
            .enumerate()
            .partition(|(idx, _)| validation_indices.contains(idx));
    ...
    fn extract_feedback_for_traces(&self, traces: &[ReasoningTrace]) -> Result<Vec<JudgeFeedback>> {
        ...
        let relevant_feedback: Vec<JudgeFeedback> = self
            .feedback
            .iter()
    ...
    fn generate_augmented_traces(
        &self,
        original_traces: &[ReasoningTrace],
    ...
    fn generate_synthetic_feedback(&self, trace: &ReasoningTrace) -> Result<Vec<JudgeFeedback>> {
        ...
        for step in &trace.steps {
            ...
            let feedback = JudgeFeedback {
                ...
                feedback_type,
                reasoning: format!(
                    "Synthetic feedback for step {} with confidence {}",
...
impl DataProcessor {
    ...
    fn clean_trace(&self, trace: &ReasoningTrace) -> Result<ReasoningTrace> {
        let cleaned_steps: Vec<ReasoningStep> = trace
            .steps
            .iter()
    ...
    pub fn validate_dataset(traces: &[ReasoningTrace], feedback: &[JudgeFeedback]) -> Result<()> {
        ...
        for trace in traces {
            ...
            if step_numbers.len() != trace.steps.len() {
                return Err(anyhow::anyhow!(
                    "Duplicate step numbers in trace {}",
                    trace.id
                ));
            }
        }
        ...
        for fb in feedback {
            if !trace_ids.contains(&fb.trace_id) {
                return Err(anyhow::anyhow!(
                    "Feedback refers to non-existent trace {}",
                    fb.trace_id
                ));
            }
            ...
            match &fb.feedback_type {
                FeedbackType::Independent {
                    ...
                    let step_exists = traces
                        .iter()
                        .any(|trace| trace.steps.iter().any(|step| step.id == *step_id));

                    if !step_exists {
                        return Err(anyhow::anyhow!(
                            "Feedback refers to non-existent step {}",
                            step_id
                        ));
                    }
                }
