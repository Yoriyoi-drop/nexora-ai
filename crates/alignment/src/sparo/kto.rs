...
impl ProspectTheoryCalculator {
    ...
    pub fn probability_weighting(&self, p: f32) -> f32 {
        ...
        p_clamped.powf(self.config.probability_weighting)
            / (p_clamped.powf(self.config.probability_weighting)
                + (1.0 - p_clamped).powf(self.config.probability_weighting))
            .powf(1.0 / self.config.probability_weighting)
    }
    ...
    pub fn calculate_gradient(&self, label: &IndependentLabel) -> Result<f32> {
        ...
        let value_grad = if log_ratio >= self.config.reference_point {
            self.config.probability_weighting
                * (log_ratio - self.config.reference_point)
                    .powf(self.config.probability_weighting - 1.0)
        } else {
            -self.config.loss_aversion
                * self.config.probability_weighting
                * (self.config.reference_point - log_ratio)
                    .powf(self.config.probability_weighting - 1.0)
        };
...
impl KtoTrainer {
    ...
    pub fn extract_independent_labels(
        ...
        for fb in feedback {
            if let FeedbackType::Independent {
                step_id,
                is_good,
                confidence,
            } = &fb.feedback_type
                    ...
                    .find(|t| t.steps.iter().any(|s| s.id == *step_id))
                {
                    ...
                    let label = IndependentLabel {
                        ...
                        log_probability: self
                            .model
                            .log_probability(&trace.prompt, &step_content)?,
                        reference_log_probability: self
                            .model
                            .reference_log_probability(&trace.prompt, &step_content)?,
                    };
                    labels.push(label);
                }
            }
    ...
    fn update_model_parameters(&mut self, gradient: f32, label: &IndependentLabel) -> Result<()> {
        ...
        self.model
            .apply_gradient(&label.prompt, &label.response, gradient, self.learning_rate)?;
        Ok(())
    }
}
...
pub mod utils {
    ...
    pub fn analyze_distribution(labels: &[IndependentLabel]) -> DistributionStats {
        ...
        let avg_confidence_good = labels
            .iter()
            .filter(|l| l.is_good)
            .map(|l| l.confidence)
            .sum::<f32>()
            / good_count.max(1) as f32;

        let avg_confidence_bad = labels
            .iter()
            .filter(|l| !l.is_good)
            .map(|l| l.confidence)
            .sum::<f32>()
            / bad_count.max(1) as f32;
