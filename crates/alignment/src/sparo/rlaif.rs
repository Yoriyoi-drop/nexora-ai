...
pub trait AiJudge {
    ...
    fn compare_steps(&self, step1: &ReasoningStep, step2: &ReasoningStep)
...
impl AiJudge for Gpt4Judge {
    fn evaluate_step(
        ...
        Ok(JudgeEvaluation {
            ...
            confidence,
            reasoning: format!(
                "Step {} shows {} quality with confidence {}",
    ...
    fn compare_steps(
        ...
        Ok(StepComparison {
            ...
            confidence,
            reasoning: format!(
                "Step {} preferred over {} with confidence {}",
                preferred_step,
                if preferred_step == step1.id {
                    step2.id
                } else {
                    step1.id
                },
...
impl Gpt4Judge {
    fn calculate_step_quality(
        &self,
        step: &ReasoningStep,
        _context: &ReasoningTrace,
    ) -> Result<f32> {
        let mut score = 0.5;
...
impl RlaifManager {
    ...
    pub fn get_judge_stats(&self) -> JudgeStats {
        let judge_names: Vec<String> = self
            .judges
            .iter()
    ...
    // Helper methods
    fn convert_evaluation_to_feedback(
        &self,
    ...
    fn compare_traces(
        ...
        let rejected_trace = if preferred_trace == trace1.id {
            trace2.id
        } else {
            trace1.id
        };
        ...
        let confidence = ((eval1.overall_score - eval2.overall_score).abs()
            / (eval1.overall_score + eval2.overall_score).max(0.1))
        .min(1.0);
        ...
        Ok(JudgeFeedback {
            ...
            feedback_type,
            reasoning: format!(
                "Trace {} preferred over trace {} with confidence {}",
        ...
        for feedback in feedbacks {
            let key = match &feedback.feedback_type {
                ...
                FeedbackType::Pairwise {
                    ...
                    rejected,
                    ..
                } => format!("pairwise_{}_{}", preferred, rejected),
            };
    ...
    fn create_consensus_feedback(
        &self,
        feedbacks: &[JudgeFeedback],
    ) -> Result<Option<JudgeFeedback>> {
        ...
        let aggregated_reasoning = feedbacks
            .iter()
    ...
    fn feedback_agrees(&self, feedback1: &JudgeFeedback, feedback2: &JudgeFeedback) -> bool {
        match (&feedback1.feedback_type, &feedback2.feedback_type) {
            (
                ...
                FeedbackType::Independent { is_good: good2, .. },
            ) => good1 == good2,
            (
                FeedbackType::Pairwise {
                    preferred: pref1, ..
