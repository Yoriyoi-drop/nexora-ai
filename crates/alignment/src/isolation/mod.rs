...
use firewall::{AgentBus, InterAgentFirewall, SharedAgentBus, SharedFirewall};
...
use layer3_tool::{SharedToolGateway, ToolIsolationLayer};
...
impl IsolationOrchestrator {
            ...
            tool: Arc::new(RwLock::new(ToolIsolationLayer::new(
                config
                    ...
                    .map(|t| match t.as_str() {
                        "python" => crate::isolation::layer3_tool::ToolKind::Python,
                        "browser" => crate::isolation::layer3_tool::ToolKind::Browser,
                        "terminal" => crate::isolation::layer3_tool::ToolKind::Terminal,
                        "filesystem" => crate::isolation::layer3_tool::ToolKind::FileSystem,
                        "shell" => crate::isolation::layer3_tool::ToolKind::Shell,
                        "compiler" => crate::isolation::layer3_tool::ToolKind::Compiler,
                        _ => crate::isolation::layer3_tool::ToolKind::Custom(t.clone()),
                    })
                    .collect::<Vec<_>>(),
            ))),
    ...
    pub fn pre_inference_check(&self, agent_id: uuid::Uuid) -> Result<(), IsolationCheckError> {
...

        {
            ...
            if !perm.check_capability(
                agent_id,
                &layer6_permission::Capability::ModelInference("default".into()),
            ) {
                return Err(IsolationCheckError::CapabilityDenied(
    ...
    pub fn check_agent_communication(
        ...
        match action {
            ...
            firewall::FirewallAction::Deny | firewall::FirewallAction::AuditDeny => {
                Err(IsolationCheckError::CommunicationDenied {
                    from: source_id,
                    to: dest_id,
                })
            }
    ...
    pub fn verify_tool_access(
        ...
        if !perm.verify_tool_access(agent_id, tool) {
            return Err(IsolationCheckError::CapabilityDenied(format!(
                "tool:{tool}"
            )));
        }
        Ok(())
    ...
    pub fn trigger_kill_switch(
        ...
        match target {
            killswitch::KillTarget::Agent(id) => ks
                .kill_agent(id, reason, trigger)
                .map_err(|e| IsolationCheckError::KillSwitchError(e.to_string())),
            killswitch::KillTarget::Mode(mode_id) => ks
                .kill_mode(mode_id, reason, trigger)
                .map_err(|e| IsolationCheckError::KillSwitchError(e.to_string())),
...
mod tests {
    ...
    fn test_full_isolation_flow() {
        ...
        orch.permission.write().register_agent(
            agent_id,
            "test-agent",
            layer6_permission::AgentRole::Restricted,
        );
        orch.permission
            .write()
            .grant_capability(
...
pub mod prelude {
    pub use crate::isolation::config::IsolationConfig;
    pub use crate::isolation::firewall::{
        AgentBus, AgentBusMessage, AgentBusMsgType, FirewallAction, FirewallAuditEntry,
        FirewallMatch, FirewallRateLimiter, FirewallRule, InterAgentFirewall, SuspiciousPattern,
    };
    pub use crate::isolation::killswitch::*;
    pub use crate::isolation::layer0_global::*;
    pub use crate::isolation::layer1_mode::{
        Mode, ModeId, ModeIsolationLayer, ModeIsolationPolicy, ModeKind, ModeStatus,
    };
    pub use crate::isolation::layer2_agent::*;
    pub use crate::isolation::layer3_tool::{
        SandboxKind as ToolSandboxKind, SandboxSpec, ToolExecutionRequest, ToolExecutionResult,
        ToolGateway, ToolIsolationLayer, ToolKind, ToolPod, ToolStatus,
    };
    pub use crate::isolation::layer4_runtime::*;
    pub use crate::isolation::layer5_cognitive::*;
    pub use crate::isolation::layer6_permission::*;
    pub use crate::isolation::multicluster::*;
    pub use crate::isolation::quarantine::*;
    pub use crate::isolation::{IsolationCheckError, IsolationOrchestrator};
}
