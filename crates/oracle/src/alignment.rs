...
impl CodeDpoTrainer {
    pub fn new(config: CodeDpoConfig, model: CodeModel, reference_model: CodeModel) -> Self {
        Self {
            config,
            model,
            reference_model,
    ...
    fn compute_security_loss(&self, pair: &CodePreferencePair) -> Result<f32> {
        ...
        let rejected_security = self
            .security_analyzer
            .analyze_security(&pair.rejected_code)?;
    ...
    fn compute_efficiency_loss(&self, pair: &CodePreferencePair) -> Result<f32> {
        let chosen_efficiency = self
            .efficiency_analyzer
            .analyze_efficiency(&pair.chosen_code)?;
        let rejected_efficiency = self
            .efficiency_analyzer
            .analyze_efficiency(&pair.rejected_code)?;
    ...
    pub fn generate_preferences(
        ...
        for _ in 0..num_samples {
            ...
            let ref_chosen_logprob = self
                .reference_model
                .compute_log_probability(prompt, chosen_code)?;
            let ref_rejected_logprob = self
                .reference_model
                .compute_log_probability(prompt, rejected_code)?;
    ...
    pub fn get_alignment_stats(&self, preference_pairs: &[CodePreferencePair]) -> AlignmentStats {
        ...
        let avg_security_score = preference_pairs
            .iter()
            .map(|p| p.security_score)
            .sum::<f32>()
            / total_pairs.max(1) as f32;

        let avg_efficiency_score = preference_pairs
            .iter()
            .map(|p| p.efficiency_score)
            .sum::<f32>()
            / total_pairs.max(1) as f32;

        let avg_code_quality_score = preference_pairs
            .iter()
            .map(|p| p.code_quality_score)
            .sum::<f32>()
            / total_pairs.max(1) as f32;

        let security_improvements = preference_pairs
            .iter()
            .filter(|p| p.security_score > 0.5)
            .count();

        let efficiency_improvements = preference_pairs
            .iter()
            .filter(|p| p.efficiency_score > 0.5)
            .count();

        let quality_improvements = preference_pairs
            .iter()
...
impl CodeAnalyzer {
    ...
    pub fn analyze_quality(&self, code: &str) -> Result<f32> {
        ...
        Ok((quality_score as f32).clamp(0.0, 1.0))
    }
    ...
    pub fn get_quality_metrics(&self, code: &str) -> HashMap<String, f32> {
        ...
        metrics.insert(
            "cyclomatic_complexity".to_string(),
            self.calculate_complexity(code),
        );
        metrics.insert(
            "maintainability_index".to_string(),
            self.calculate_maintainability(code),
        );
        metrics.insert(
            "code_duplication".to_string(),
            self.calculate_duplication(code),
        );
        metrics.insert(
            "naming_conventions".to_string(),
...
impl SecurityAnalyzer {
    ...
    pub fn analyze_security(&self, code: &str) -> Result<f32> {
        ...
        Ok((security_score as f32).clamp(0.0, 1.0))
    }
...
impl EfficiencyAnalyzer {
    ...
    pub fn get_efficiency_metrics(&self, code: &str) -> HashMap<String, f32> {
        ...
        metrics.insert(
            "algorithmic_complexity".to_string(),
            self.analyze_algorithmic_complexity(code),
        );
        metrics.insert(
            "memory_efficiency".to_string(),
            self.analyze_memory_usage(code),
        );
        metrics.insert(
            "time_complexity".to_string(),
            self.estimate_time_complexity(code),
        );
    ...
    fn estimate_time_complexity(&self, code: &str) -> f32 {
        ...
        for line in lines {
            ...
            if trimmed == "}" || trimmed == "end" {
                nested_level = (nested_level as i32).saturating_sub(1) as usize;
            }
        }
...
pub mod utils {
    ...
    pub fn analyze_alignment_progress(
        ...
        let security_improvement =
            current_stats.avg_security_score - previous_stats.avg_security_score;
        let efficiency_improvement =
            current_stats.avg_efficiency_score - previous_stats.avg_efficiency_score;
        let quality_improvement =
            current_stats.avg_code_quality_score - previous_stats.avg_code_quality_score;

        AlignmentProgress {
            security_improvement,
            efficiency_improvement,
            quality_improvement,
            overall_improvement: (security_improvement
                + efficiency_improvement
                + quality_improvement)
                / 3.0,
        }
