...
impl OracleTrainer {
    ...
    pub fn new(config: OracleConfig, vocab_size: usize) -> Result<Self> {
        ...
        let reference_model =
            crate::alignment::CodeModel::new(vocab_size, config.training.max_seq_len);
        let current_model =
            crate::alignment::CodeModel::new(vocab_size, config.training.max_seq_len);
    ...
    pub fn final_evaluation(
        ...
        let avg_pretraining_loss =
            pretraining_losses.iter().sum::<f32>() / pretraining_losses.len().max(1) as f32;
        let avg_alignment_loss =
            alignment_losses.iter().sum::<f32>() / alignment_losses.len().max(1) as f32;
        let avg_verification_score =
            verification_scores.iter().sum::<f32>() / verification_scores.len().max(1) as f32;

        Ok(EvaluationMetrics {
            pretraining_loss: avg_pretraining_loss,
            alignment_loss: avg_alignment_loss,
            verification_score: avg_verification_score,
            overall_score: (avg_pretraining_loss + avg_alignment_loss) / 2.0
                * avg_verification_score,
        })
    }

    /// Create pretraining batches
    fn create_pretraining_batches(
        &self,
        training_data: &[TrainingExample],
    ...
    fn create_alignment_batches(
        &self,
        preference_pairs: &[CodePreferencePair],
    ) -> Result<Vec<Vec<CodePreferencePair>>> {
        let mut batches = Vec::new();
...
pub struct AlignmentResult {
    pub final_loss: f32,
    pub total_epochs: usize,
    pub alignment_stats: Vec<crate::alignment::AlignmentStats>,
}
...
pub mod utils {
    ...
    pub fn estimate_training_time(
        ...
        let pretraining_steps =
            (dataset_size / config.training.batch_size) * config.training.pretraining_epochs;
        let alignment_steps =
            (dataset_size / config.training.batch_size) * config.training.alignment_epochs;
