...
impl CodeVerifier for CorrectnessVerifier {
    fn verify(&self, code: &str, language: &str) -> Result<VerificationResult> {
        ...
        let suggestions = self.generate_correctness_suggestions(&issues);

        let mut metrics = HashMap::new();

        metrics.insert("correctness_score".to_string(), score);

        metrics.insert("correctness_issue_count".to_string(), issues.len() as f32);
            ...
            "rust" => {
                ...
                if unwrap_count > 3 {
                    issues.push(CodeIssue {
                        severity: IssueSeverity::Warning,
                        category: "Correctness".to_string(),
                        message: format!("Excessive unwrap() usage ({unwrap_count})"),
                        line_number: None,
                        column_number: None,
                ...
                if code.contains("panic!") {
                    issues.push(CodeIssue {
                        severity: IssueSeverity::Warning,
                        category: "Correctness".to_string(),
                        message: "panic! macro detected".to_string(),
                        line_number: None,
                        column_number: None,
                ...
                if code.contains("unsafe {") {
                    issues.push(CodeIssue {
                        ...
                        category: "Correctness".to_string(),
                        message: "unsafe block detected, review memory safety".to_string(),
                        line_number: None,
                        column_number: None,
            ...
            "python" => {
                if code.contains("global ") {
                    issues.push(CodeIssue {
                        severity: IssueSeverity::Info,
                        category: "Correctness".to_string(),
                        message: "Global variable usage detected".to_string(),
                        line_number: None,
                        column_number: None,
    ...
    fn generate_correctness_suggestions(&self, issues: &[CodeIssue]) -> Vec<String> {
        ...
        if issues.iter().any(|i| i.rule_id == "division_by_zero") {
            suggestions.push("Validate divisor before division".to_string());
        }

        if issues.iter().any(|i| i.rule_id == "infinite_loop") {
            suggestions.push("Ensure loops contain proper exit conditions".to_string());
        }

        if issues.iter().any(|i| i.rule_id == "rust_unwrap") {
            suggestions.push("Use proper Result handling instead of unwrap()".to_string());
        }

        if issues.iter().any(|i| i.rule_id == "js_loose_equality") {
            suggestions.push("Use strict equality operators (===)".to_string());
        }

        if issues.iter().any(|i| i.rule_id == "py_bare_except") {
            suggestions.push("Catch specific exception types".to_string());
        }
