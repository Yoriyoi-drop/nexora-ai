...
impl FileUtils {
    ...
    pub fn find_files_by_pattern(path: &Path, pattern: &str) -> Result<Vec<PathBuf>> {
        ...
        let filtered: Vec<PathBuf> = matching_files
            .into_iter()
            .filter(|path| {
                path.file_name()
    ...
    pub fn find_duplicates(path: &Path) -> Result<Vec<Vec<PathBuf>>> {
        ...
        let mut checksum_map: std::collections::HashMap<String, Vec<PathBuf>> =
        ...
        let duplicates: Vec<Vec<PathBuf>> = checksum_map
            .into_values()
...
mod tests {
    ...
    fn test_file_utils() {
        ...
        // Test file extension
        assert_eq!(
            FileUtils::get_file_extension(&test_file),
            Some("txt".to_string())
        );
        assert_eq!(
            FileUtils::get_file_stem(&test_file),
            Some("test".to_string())
        );
        assert_eq!(
            FileUtils::get_file_name(&test_file),
