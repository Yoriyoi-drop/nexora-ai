...
static RE_ALPHA_NUM_UNDERSCORE: Lazy<Regex> =
...
impl ValidationUtils {
    ...
    pub fn validate_int_range(num: i64, min: i64, max: i64) -> Result<()> {
        if num < min || num > max {
            return Err(anyhow::anyhow!(
                "Integer {} is outside valid range [{}, {}]",
                num,
                min,
                max
            ));
    ...
    pub fn validate_allowed_values<T: std::fmt::Display + PartialEq>(
        s: &str,
        allowed: &[T],
    ) -> Result<()> {
