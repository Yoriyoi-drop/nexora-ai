...
impl TextProcessor {
    ...
    fn lemmatize_english(word: &str) -> String {
        ...
        match word_lower.as_str() {
            ...
            w if w.ends_with("ly") && w.len() > 3 => w[..w.len() - 2].to_string(),
            // Comparative/Superlative
            w if w.ends_with("er") && w.len() > 3 => w[..w.len() - 2].to_string(),
            w if w.ends_with("est") && w.len() > 4 => w[..w.len() - 3].to_string(),
            _ => word.to_string(),
        }
    }
    ...
    pub fn extract_keywords_tfidf(texts: &[&str], top_k: usize) -> Vec<(String, f64)> {
        ...
        let mut tfidf_scores: Vec<(String, f64)> = word_freq
            .into_iter()
            .map(|(word, freq)| {
                let tf = freq as f64;
    ...
    pub fn extract_keywords_frequency(
        text: &str,
        min_length: usize,
        top_k: usize,
    ) -> Vec<(String, usize)> {
    ...
    pub fn detect_language(text: &str) -> String {
        ...
        let id_words = [
            "yang", "di", "ke", "dari", "pada", "untuk", "dengan", "adalah", "itu", "ini",
        ];
...
mod tests {
    ...
    fn test_text_processor() {
        ...
        assert_eq!(
            tokens,
            vec!["hello", "world", "this", "is", "a", "test", "text"]
        );
