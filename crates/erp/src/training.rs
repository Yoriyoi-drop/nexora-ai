...
use crate::{CompressedLayer, ERPConfig, ERPEngine, ERPError};
...
impl ERPTrainer {
    ...
    ) -> Result<TrainedERPModel, ERPError> {
    ...
    fn phase1_resonance_mapping(
        &self,
        weights: &[Array2<f32>],
    ) -> Result<ResonanceMappingResult, ERPError> {
    ...
    fn phase2_compression(
        &self,
        weights: &[Array2<f32>],
        resonance_result: &ResonanceMappingResult,
    ) -> Result<CompressedModel, ERPError> {
    ...
    fn phase3_calibration(
        &mut self,
        model: &CompressedModel,
        training_data: &TrainingDataset,
    ) -> Result<CalibratedModel, ERPError> {
        ...
        for epoch in 0..calibration_epochs {
            ...
            if epoch % 2 == 0 {
                let calibration_loss =
                    self.evaluate_calibration_loss(&calibrated_layers, &calibration_batch)?;
                tracing::info!(
                    "Calibration epoch {}: loss = {:.4}",
                    epoch,
                    calibration_loss
                );
            }
    ...
    fn phase4_lora_recovery(
        ...
        let lora_adapters = match &self.training_strategy {
            ...
            TrainingStrategy::Aggressive {
                lora_rank,
                fine_tuning_epochs: _,
            } => self.train_lora_adapters(&model.calibrated_layers, training_data, *lora_rank)?,
        };
    ...
    fn calibrate_layer_gates(
        &mut self,
        layer: &mut CompressedLayer,
        calibration_batch: &CalibrationBatch,
    ) -> Result<(), ERPError> {
        let mut reconstructor =
            crate::reconstruction::ContextReconstructor::new(self.config.clone());
        ...
        for (input, target_output) in &calibration_batch.samples {
            ...
            let gate_gradients =
                self.compute_gate_gradients(layer, input, target_output, &current_gates)?;
        ...
        if sample_count > 0 {
            ...
            let gradient_norm = accumulated_gradients
                .iter()
                .map(|x| x * x)
                .sum::<f32>()
                .sqrt();
            ...
            self.update_gate_weights_from_reconstructor(
                &mut reconstructor,
            ...
            tracing::debug!(
                "Updated gate weights for layer {} with learning rate {:.6}",
                layer.layer_idx,
                learning_rate
            );
        }
    ...
    /// Compute mapping statistics
    fn compute_mapping_statistics(
        &self,
        resonance_groups: &[crate::core::ResonanceGroup],
        weights: &[Array2<f32>],
    ) -> MappingStatistics {
        ...
        MappingStatistics {
            ...
            compressed_neurons,
            num_groups,
            compression_ratio: if total_neurons > 0 {
                compressed_neurons as f32 / total_neurons as f32
            } else {
                0.0
            },
            avg_group_size: if num_groups > 0 {
                compressed_neurons as f32 / num_groups as f32
            } else {
                0.0
            },
        }
    ...
    fn estimate_compression_potential(
        &self,
        resonance_groups: &[crate::core::ResonanceGroup],
        weights: &[Array2<f32>],
        ...
        for group in resonance_groups {
            ...
            compressed_params += group.neurons.len() * weights[group.neurons[0]].ncols() / 2;
            // Estimated residual
        }

        let compression_ratio = if total_params > 0 {
            1.0 - (compressed_params as f32 / total_params as f32)
        } else {
            0.0
        };
    ...
    /// Compute compression metrics
    fn compute_compression_metrics(
        &self,
        original_weights: &[Array2<f32>],
        ...
        CompressionMetrics {
            original_params,
            compressed_params,
            compression_ratio: if original_params > 0 {
    ...
    fn compute_calibration_stats(&self, layers: &[CompressedLayer]) -> CalibrationStats {
        let total_gates: usize = layers
            .iter()
            .map(|l| l.resonance_representations.len())
            .sum();
        let avg_sparsity: f32 = layers
            .iter()
            .flat_map(|l| l.resonance_representations.iter())
            .map(|rep| {
                rep.importance_coeffs.iter().filter(|&&x| x > 0.5).count() as f32
                    / rep.importance_coeffs.len() as f32
            })
            .sum::<f32>()
            / total_gates.max(1) as f32;

        // Compute calibration quality based on multiple factors
        let sparsity_quality = if avg_sparsity > 0.3 && avg_sparsity < 0.7 {
            1.0
        } else {
            0.8
        };
        ...
        let calibration_quality =
            0.4 * sparsity_quality + 0.3 * gate_balance_quality + 0.3 * compression_efficiency;
    ...
    fn compute_gate_balance_quality(&self, layers: &[CompressedLayer]) -> f32 {
        ...
        for layer in layers {
            ...
            for resonance_rep in &layer.resonance_representations {
                let active_count = resonance_rep
                    .importance_coeffs
                    .iter()
                    .filter(|&&x| x > 0.5)
                    .count();
            ...
            if !gate_activations.is_empty() {
                let mean_activation =
                    gate_activations.iter().sum::<f32>() / gate_activations.len() as f32;
                let variance = gate_activations
                    .iter()
                    .map(|x| (x - mean_activation).powi(2))
                    .sum::<f32>()
                    / gate_activations.len() as f32;
    ...
    fn compute_training_stats(
        ...
        let base_time = match &self.training_strategy {
            TrainingStrategy::Conservative { calibration_epochs } => {
                std::time::Duration::from_secs((*calibration_epochs as u64) * 60)
                // 1 min per epoch
            }
            TrainingStrategy::Balanced {
                calibration_epochs,
        ...
        let lora_effectiveness = if lora_adapters.is_empty() {
            ...
            let avg_rank = lora_adapters.iter().map(|a| a.rank).sum::<usize>() as f32
                / lora_adapters.len() as f32;
...
mod tests {
    ...
    fn test_compute_gate_gradients() {
        ...
        let grad = t
            .compute_gate_gradients(&l, &input(&[1.0, 2.0]), &input(&[0.0, 0.0]), &gates)
            .unwrap();
