...
fn create_caffeine_config(enable_atqs: bool, enable_moe: bool) -> CaffeineConfig {
    debug!(
        "Creating Caffeine configuration - ATQS: {}, MoE: {}",
        enable_atqs, enable_moe
    );
    ...
    let config = CaffeineConfig {
        ...
        atqs_config: if enable_atqs {
            Some(create_atqs_config())
        } else {
            None
        },
        has_moe_config: if enable_moe {
            Some(create_moe_config())
        } else {
            None
        },
...
fn create_moe_config() -> HasMoeFFNConfig {
    ...
    info!(
        "MoE config created - num_experts: {}, top_k: {}",
        ...
        group.bench_with_input(
            ...
            |b, &batch_size| {
                b.iter(|| {
                    ...
                    debug!(
                        "Starting benchmark iteration for batch size: {}",
                        batch_size
                    );
...
fn benchmark_individual_components(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("compression", size), size, |b, &size| {
            b.iter(|| {
                ...
                let _result = compression
                    .compress_string(black_box(&input_str))
                    .unwrap_or_else(|e| {
    ...
    for input_size in [4096].iter() {
        // Use size that matches MoE config
...
fn benchmark_memory_usage(c: &mut Criterion) {
    ...
    group.bench_function("full_integration_memory", |b| {
        b.iter(|| {
            ...
            let _result = RUNTIME
                .block_on(caffeine.forward(black_box(&inputs)))
                .unwrap_or_else(|e| {
