...
fn main() -> anyhow::Result<()> {
    ...
    let teacher_model = PolicyModel::new(uuid::Uuid::new_v4(), (512, 512));
    ...
    let prompts =
        nexora_foundation::alignment::sparo::trainer::utils::generate_training_prompts(20);
    ...
    let (sample_traces, sample_feedback) =
        nexora_foundation::alignment::sparo::data::utils::generate_sample_data(50);
    ...
    println!(
        "Final loss: {:.6}",
        training_result.final_state.current_loss
    );
    ...
    println!("\n📈 Metrics Analysis");
    println!("==================");
    let metrics_analysis = nexora_foundation::alignment::sparo::trainer::utils::analyze_metrics(
        &training_result.metrics_history,
    );
    ...
    println!(
        "Convergence rate: {:.2}%",
        metrics_analysis.convergence_rate * 100.0
    );
    println!(
        "Improvement rate: {:.2}%",
        metrics_analysis.improvement_rate * 100.0
    );

    println!("\nComponent balance:");
    println!(
        "  DPO contribution: {:.1}%",
        metrics_analysis.component_balance.dpo_contribution * 100.0
    );
    println!(
        "  KTO contribution: {:.1}%",
        metrics_analysis.component_balance.kto_contribution * 100.0
    );
    println!(
        "  IPO contribution: {:.1}%",
        metrics_analysis.component_balance.ipo_contribution * 100.0
    );
...
fn demo_kto_component() -> anyhow::Result<()> {
    ...
    let labels = vec![IndependentLabel {
        id: uuid::Uuid::new_v4(),
    ...
    println!(
        "  Good: {} ({:.1}%)",
        stats.good_count,
        stats.good_ratio * 100.0
    );
    println!(
        "  Bad: {} ({:.1}%)",
        stats.bad_count,
        stats.bad_ratio * 100.0
    );
...
fn demo_rlvf_component() -> anyhow::Result<()> {
    ...
    for (i, feedback) in step_feedbacks.iter().enumerate() {
        println!(
            "  Step {}: Score = {:.3}, Verifiable = {}",
            i + 1,
            feedback.overall_score,
            feedback.is_verifiable
        );
    }
...
fn demo_spin_component() -> anyhow::Result<()> {
    ...
    println!(
        "  Student win rate: {:.1}%",
        tournament.student_win_rate * 100.0
    );
...
fn demo_rlaif_component() -> anyhow::Result<()> {
    ...
    for (i, fb) in feedback.iter().enumerate() {
        match &fb.feedback_type {
            FeedbackType::Independent {
                step_id,
                is_good,
                confidence,
            } => {
                println!(
                    "  Feedback {}: Independent - Step {} = {}, Confidence = {:.2}",
                    i + 1,
                    step_id,
                    is_good,
                    confidence
            ...
            FeedbackType::Pairwise {
                preferred,
                rejected,
                confidence,
            } => {
                println!(
                    "  Feedback {}: Pairwise - Preferred {} over {}, Confidence = {:.2}",
                    i + 1,
                    preferred,
                    rejected,
                    confidence
